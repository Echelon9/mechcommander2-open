...
void EditorObjectMgr::init( const char* bldgListFileName, const char* objectFileName )
{
	...
	while( lstFile.getLogicalPosition() < lstFile.getLength() )
	{
		...
		if ( ExtractNextString( pCur,  tmpBuffer, bufferLength ) )
		{
			FullPathFileName bPath;
			bPath.init(objectPath, bldg.fileName, ".fit");
			FitIniFile bldgFile;
...
//magic code change here
			...
			{
				if (bldg.type != MECH_TYPE)
				{
					char tmpBuf[1024];
					sprintf(tmpBuf, "NO Object FIT file in Object2 or data/objects: (%ld)%s", bldg.fitID, bldg.fileName);
					MessageBox(NULL, tmpBuf ,"WARNING", MB_OK);
				}
			}
			bldgFile.close();
		}
...
bool EditorObjectMgr::save( PacketFile& PakFile, int whichPacket )
{
	...
	*pTacMapPoints = pointCounter;
	...
	if (!justResaveAllMaps) {
		...
		if (128/*max turrets*/ < numTurrets ) //magic 10012011 *
		{
			CString CStr;
...
bool EditorObjectMgr::load( PacketFile& PakFile, int whichPacket )
{
	...
	for ( int i = 0; i < count; ++i )
	{
		...
		if ( !getBuildingFromID(id, group, index, false ) )
		{
			...
			//char tmp2[64];
			//sprintf( tmp2, "cant load building: %ld", id );
			//MessageBox(NULL,tmp2,"ERROR",MB_OK);
			continue;
		}
...
ObjectAppearance* EditorObjectMgr::getAppearance( EditorObjectMgr::Building* pBuilding )
{
	...
	if ( !appearanceTypeList )
	{
		...
		appearanceTypeList->init(16384000); //magic 10012011*
	}
	...
	appearance->setObjectName( pBuilding->name ); //magic 12092011 OK
...
bool		EditorObjectMgr::saveMechs( FitIniFile& file )
{
	...
	*/
	...
	if (!justResaveAllMaps && (units.Count() > 192)) //magic 18092012 max units on the map is 192
	{
				...
				for ( int l = -1; l < 2; l++ )
				{
					for ( int m = -1; m < 2; m++ )
					{
						...
						for ( int t = 0; t < count; t++ )
						{
							...
							/*if ((AppearanceTypeList::appearanceHeap->totalCoreLeft() < 1000)
								|| (AppearanceTypeList::appearanceHeap->totalCoreLeft() < 0.01 * AppearanceTypeList::appearanceHeap->size()))
							{
...
int			EditorObjectMgr::getNumberOfVariants( int group, int indexInGroup ) const
{
		...
		//return i + 1;		//magic 28072011 ????	
		return i;		//magic 28072011 ????
}
