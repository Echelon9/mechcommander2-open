...
void *EditorObject::operator new (size_t mySize)
{
	...
	result = gos_Malloc(mySize); //magic 01022012
...
void Pilot::initPilots()
{
	...
	for ( int i = 0; i < 2; i++ )
	{
		while( true )
		{
			...
			//magic 10012011 begin
			infos[*counter].fileName = (char*)malloc(strlen( pilotFileName ) + 1);
			//magic 10012011 end
			...
			//magic 10012011 begin
			infos[*counter].name = (char*)malloc(strlen( pilotFileName ) + 1);
			//magic 10012011 end
