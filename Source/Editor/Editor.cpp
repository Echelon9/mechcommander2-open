...
unsigned long systemHeapSize = 65536000;//8192000;
unsigned long guiHeapSize = 8192000;//1023999;
unsigned long tglHeapSize = 262144000;//65536000;
...
void InitializeGameEngine()
{
	...
	systemHeap->init(systemHeapSize,"SYSTEM",true); //magic 30032011

	Stuff::InitializeClasses();

	//MidLevelRenderer::InitializeClasses(1024);
	MidLevelRenderer::InitializeClasses(); //magic 22022011
...
//				Environment.fullScreen = (fullScreen == TRUE);
	...
	//TG_Shape::tglHeap = new UserHeap; //magic 01022012 disabled
	//TG_Shape::tglHeap->init(tglHeapSize,"TinyGeom");
	//TG_Shape::tglHeap->init(tglHeapSize,"TinyGeom",true); //magic 30032011 magic 01022012 disabled

		//Start up the TGL RAM pools.
		colorPool 		= new TG_VertexPool;
		colorPool->init(60000);
		
		vertexPool 		= new TG_GOSVertexPool;
		vertexPool->init(160000);
		
		facePool 		= new TG_DWORDPool;
		facePool->init(160000);
		
		shadowPool 		= new TG_ShadowPool;
		shadowPool->init(80000);
		
		trianglePool 	= new TG_TrianglePool;
		trianglePool->init(80000);
	...
	mcTextureManager->startVertices(3000000); //magic 29052012
	...
	if (TG_Shape::tglHeap)
	{
		*/ //magic 01022012 disabled
		...
		if (trianglePool)
		{
			...
			delete trianglePool;
			//free(trianglePool); //magic 27072011
			trianglePool = NULL;
		}
...
void GetGameOSEnvironment( char* CommandLine )
{
	...
	Environment.applicationName			= "MCO Mission Editor";
