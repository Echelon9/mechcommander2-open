...
extern bool drawBuildingLinks; //by mgto 17022011
...
void Editor::resaveAll (void)
{
	...
	// ARM
	CoInitialize(NULL);
	armProvider = CreateProviderEngine("MCOEditor", versionStamp); //magic 22082011 was MC2Eitor
...
void EditorInterface::addBuildingsToNewMenu()
{
	...
	for ( i = groupCount; i < numTerrains + groupCount; i++ )
	{
		...
		//magic 22022012 begin
		case 10070/*Cement 4*/:
		case 10071/*Cement 4 (angled)*/:
		case 10072/*Cement 4 (crumbled)*/:
		case 10073/*Cement 5*/:
		case 10074/*Cement 5 (angled)*/:
		case 10075/*Cement 5 (crumbled)*/:
		//magic 22022012 end
		//magic 23022012 begin
		case 10076/*Crop 1 (overlay5)*/:
		case 10077/*Crop 2*/:
		case 10078/*Crop 3*/:
		case 10079/*Crop 4*/:
		case 10080/*Crop 5*/:
...
void EditorInterface::handleNewMenuMessage( long specificMessage )
{
	...
	if ( specificMessage >= IDS_OBJECT_200 && specificMessage <= 31400 ) //magic 10012011 !!!
	{
	...
	else 
	{
		...
		switch (specificMessage) {
			...
			case ID_SKY_SKY22 : SetSky(22); break; //magic 27022011
			case ID_SKY_SKY23 : SetSky(23); break; //magic 27022011
			case ID_SKY_SKY24 : SetSky(24); break; //magic 27022011
			case ID_SKY_SKY25 : SetSky(25); break; //magic 27022011
			case ID_SKY_SKY26 : SetSky(26); break; //magic 27022011
			case ID_SKY_SKY27 : SetSky(27); break; //magic 27022011
			case ID_SKY_SKY28 : SetSky(28); break; //magic 27022011
			case ID_SKY_SKY29 : SetSky(29); break; //magic 27022011
			case ID_SKY_SKY30 : SetSky(30); break; //magic 27022011
			case ID_SKY_SKY31 : SetSky(31); break; //magic 27022011
			case ID_SKY_SKY32 : SetSky(32); break; //magic 27022011
			case ID_SKY_SKY33 : SetSky(33); break; //magic 27022011
			case ID_SKY_SKY34 : SetSky(34); break; //magic 27022011
			case ID_SKY_SKY35 : SetSky(35); break; //magic 27022011
			case ID_SKY_SKY36 : SetSky(36); break; //magic 27022011
...
void EditorInterface::handleKeyDown( int Key )
{
	...
	if ( currentBrushID >= IDS_OBJECT_200 && currentBrushID <= IDS_OBJECT_200 + 1200 ) //magic 10012011 !!!
	{
		if (Key == KEY_LBRACKET)
		{
	...
	if ( lastKey != Key ) // only want to do these if something has changed
	{
		...
		/* by mgto  17022011 begin*/
		if (GetAsyncKeyState(KEY_I) && ctrlDn && altDn && !shiftDn)
		{
			drawBuildingLinks ^= true;
			terrainLineChanged = turn;
		}
		/* by mgto 17022011 end*/
	}
...
int	EditorInterface::paintBuildings( int message )
{
	...
	for ( int i = 0; i < groupCount; ++i )
	{
		...
		if ( id <= message && message < id + buildCount )
		{
				...
				//magic 27072011 here are movers check
				//if (((i == 4) || (i == 6)) && (alignment == EDITOR_TEAMNONE))
				//magic 28072011 begin
				/*if (((4 == i) || (6 == i)  || (21 == i)  || (22 == i)  || (23 == i)  || (24 == i) || (25 == i))  && (alignment == EDITOR_TEAMNONE))
				//magic 28072011 end*/
				//magic 22082011 begin
				if (((i == 6) || (i == 7)  || (i == 8)  || (i == 9)  || (i == 11)  || (i == 12)  || (i == 13)) && (alignment == EDITOR_TEAMNONE))
				//magic 22082011 end
				{
...
int EditorInterface::Alignment( int specific )
{
	...
	if ( currentBrushID >= IDS_OBJECT_200 && currentBrushID <= 31400 ) //magic 10012011 !!!
	{
...
inline bool colorMapIsOKFormat (const char *fileName)
{
	...
	//DWORD localColorMapSizeCheck = land->realVerticesMapSide * 25.6; //magic 06032012
...
LRESULT EditorInterface::WindowProc(UINT message, WPARAM wParam, LPARAM lParam) 
{
	...
	if ( (eye || message == WM_MOVE) )
	{
		...
		/*case WM_SIZE: //Magic original code disabled
			{
		...
		case WM_SIZE: //Magic bug fix by rollin
			{
				float w = LOWORD(lParam);
				float h = HIWORD(lParam);
				...
				if (w != g_newWidth && h != g_newHeight)
					...
					if ( (16 < (w - g_newWidth)) || (16 < (h - g_newHeight)) )
					{ // make bigger
					windowSizeChanged = true;
					}

				if ( (-w + g_newWidth) > 20 || (-h + g_newHeight) > 20 )
					{ // make smaller
					windowSizeChanged = true;
					}
...
int EditorInterface::UnitSettings()
{
	...
	/*//magic 27072011 begin
		char buffer[128];
		sprintf( buffer, "unit count %ld", list.Count() );
		MessageBox( buffer );
	//magic 27072011 end OK = 1*/
	if ( list.Count() )
	{
