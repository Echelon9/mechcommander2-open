...
bool EditorData::reassignHeightsFromTGA( const char* fileName, int min, int max )
{
	...
	//if ((theader.width > 200) || (theader.height > 200)) //magic 23022011
	if ((theader.width > 280) || (theader.height > 280)) //magic 24032012
	{
		PAUSE(("TGA file is too large. Largest allowed is 280x280.  Press Continue"));
		return false;
	}
	...
	MemoryPtr tgaData = (MemoryPtr)gos_Malloc(tgaFile.fileSize()); //magic 01022012
	...
	gos_Free(tgaData); //magic 01022012
...
bool EditorData::save( const char* fileName, bool quickSave )
{
	...
	*/
	...
	if (!quickSave)
	{
		...
		_ScenarioMapCellInfo* pInfo = (_ScenarioMapCellInfo *)malloc( sizeof(_ScenarioMapCellInfo) * land->realVerticesMapSide * land->realVerticesMapSide * 12);
		memset( pInfo, 0, sizeof( _ScenarioMapCellInfo ) * land->realVerticesMapSide * land->realVerticesMapSide * 12 );		
...
void EditorData::drawTacMap( BYTE* pDest, long dataSize, int tacMapSize )
{
	...
	TGARecs *tRecs = (TGARecs *)gos_Malloc(sizeof(TGARecs) * 255); //magic 02012012
				...
				strcmp(EditorObjectMgr::instance()->getTGAFileName(pObj->getID()),"NONE") != 0 )
			{
				...
				else
				{
					...
					if (result == NO_ERR)
					{
						...
						tgaRAM = (MemoryPtr)gos_Malloc(header.width * header.height * sizeof(DWORD)); //magic 01022011
	...
	for (long it = 0;it<255;it++)
	{
		if (tRecs[it].tgaData)
		{
			...
			gos_Free(tRecs[it].tgaData); //magic 01022012
			tRecs[it].tgaData = NULL;	
		}
	}
	
	//systemHeap->Free(tRecs);
	gos_Free(tRecs); //magic 01022012
	tRecs = NULL;
