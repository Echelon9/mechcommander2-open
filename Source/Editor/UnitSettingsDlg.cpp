...
void UnitSettingsDlg::OnSelchangeGroup() 
{
		...
		//const char* MechNames[256]; //magic 27072011
		//int count = 256;
		//magic 27072011 begin
		int count = EditorObjectMgr::instance()->getNumberBuildingsInGroup( group );
		const char** MechNames = 0;
		MechNames = (const char**)malloc( sizeof( const char * ) * count );
		//magic 27072011 end
...
void UnitSettingsDlg::OnSelchangeMech() 
{
	...
	//const char* varNames = (const char*) gos_Malloc(varCount);
	if (0 < varCount)
	{
		const char** VariantNames = 0;
		//const char** VariantNames = (const char**)malloc( sizeof( const char * ) * varCount );
		VariantNames = (const char**)malloc( sizeof( const char * ) * varCount );
		//VariantNames = new const char*[varCount];
		//magic 08012011 begin
		//VariantNames = (const char**) gos_Malloc(varCount);
		//*VariantNames = varNames;
		//magic 08012011 end
		EditorObjectMgr::instance()->getVariantNames( group, indexInGroup, VariantNames, varCount );
	
		//char tmp2[64];
		//sprintf( tmp2, "vriants count: %ld", varCount );
		//MessageBox(NULL,tmp2,MB_OK);
...
void UnitSettingsDlg::updateMemberVariables()
{
	...
	int*		 groupIDs = new int[groupCount]; //magic 28072011 activated
	//magic 08012011 begin
	const char** pGroups = (const char**)malloc( sizeof( const char * ) * groupCount );
	//int*		 groupIDs = (int*)malloc(groupCount); //magic 28072011 deactivated - dont do integer like this
	//magic 08012011 end
	...
	free(pGroups);
	pGroups = NULL;
	//free(groupIDs); //magic 28072011 disabled 
	//groupIDs = NULL;
	//magic 08012011 end
	delete [] groupIDs; //magic 28072011 enabled
	...
	if ( group != -1 ) // we found a valid group
	{
		...
		//int count = 256;
		//magic 27072011 begin
		int count = pMgr->getNumberBuildingsInGroup( group );
		const char** MechNames = 0;
		MechNames = (const char**)malloc( sizeof( const char * ) * count );
		//magic 27072011 end
		m_Mech.ResetContent();
