...
bool CPlayBIK::EditDialog() {
	while (true) {
		...
		if( selectAVIFileDialog.DoModal()==IDOK ) {
			...
			TCHAR *CurrentDirectoryBuffer = (TCHAR*)gos_Malloc(CurrentDirectoryBufferLength); //magic 02092012
			...
			//magic 02092012 begin
			if (CurrentDirectoryBuffer)
				gos_Free(CurrentDirectoryBuffer);
			//magic 02092012 end
...
bool CMakeNewSalvageComponents::operator==(const CObjectiveAction &rhs) const {
	bool retval = false;
	const CMakeNewSalvageComponents *pCastedRhs = dynamic_cast<const CMakeNewSalvageComponents *>(&rhs);
	if ((pCastedRhs) && (CObjectiveAction::operator==(rhs))) {
		if (m_purchaseFilePathname == pCastedRhs->m_purchaseFilePathname) {
			retval = true;
		}
...
bool CMakeNewSalvageComponents::Read( FitIniFile* missionFile )
{
	long result = 0;
	ECharString tmpECStr;
	result = sReadIdString(missionFile, "SalvageFileName", tmpECStr);
	if (NO_ERR != result) { return false; }
	m_purchaseFilePathname = tmpECStr.Data();
	return true;
}

bool CMakeNewSalvageComponents::Save( FitIniFile* file )
{
	file->writeIdString( "SalvageFileName", m_purchaseFilePathname.Data());
	return true;
}

bool CMakeNewSalvageComponents::EditDialog() {
	while (true) {
		CFileDialog selectPurchaseFileDialog(TRUE,_T("FIT"),_T("*.FIT"),
						 OFN_HIDEREADONLY|OFN_OVERWRITEPROMPT|OFN_NOCHANGEDIR,
						 _T("Purchase File (*.FIT)|*.FIT|"));
...
EString CMakeNewSalvageComponents::Description() {
	EString retval = "MakeNewSalvageComponents"; /* needs to be put somewhere localizable */
	ESLoadString(IDS_MakeNewComponentSalvage, retval);
	return retval;
}

EString CMakeNewSalvageComponents::InstanceDescription() {
	EString tmpEStr;
	tmpEStr.Format("%s", m_purchaseFilePathname.Data());
	return tmpEStr;
}
...
CObjectiveAction *CObjective::new_CObjectiveAction(action_species_type actionSpecies, int alignment) {
	...
	case MAKE_NEW_COMPONENT_SALVAGE: retval = new CMakeNewSalvageComponents(alignment); break; //magic 03092012
	default:
		assert(false);
		retval = 0;
		break;
