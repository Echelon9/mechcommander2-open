...
void LogisticsScreen::init2( FitIniFile& file, float resXFactor,  float resYFactor, const char* staticName, const char* textName, const char* rectName,
					  const char* buttonName, const char* editName, const char* animObjectName, DWORD neverFlush) //magic 18022012 new function
{
	clear();
	...
	if ( staticName )
	{
		...
		if ( NO_ERR == file.seekBlock( blockName ) )
		{
			...
			if ( staticCount )
			{
				...
				{
					sprintf( blockName, "%s%ld", staticName, i );
					statics[i].init2( &file, blockName, resXFactor, resYFactor ); //magic 18022012
				}
	...
	if ( rectName )
	{
		...
		if ( NO_ERR == file.seekBlock( blockName ) )
		{
			...
			if ( rectCount )
			{
				...
				{
					sprintf( blockName, "%s%ld", rectName, i );
					rects[i].init2( &file, blockName, resXFactor, resYFactor ); // magic 18022012
				}
			}
		}
	}
	...
	if ( buttonName )
	{
		...
		if ( NO_ERR == file.seekBlock( blockName ) )
		{
			...
			if ( buttonCount )
			{
				...
				{
					sprintf( blockName,"%s%ld", buttonName, i );
					buttons[i].init3( file, blockName, resXFactor, resYFactor );
					addChild( &buttons[i] );
				}
			}
	...
	if ( textName )
	{
		...
		if ( NO_ERR == file.seekBlock( blockName ) )
		{
			...
			if ( textCount )
			{
				...
				{
					sprintf( blockName, "%s%ld", textName, i );
					textObjects[i].init2( &file, blockName, resXFactor, resYFactor );
				}
	...
	if ( editName )
	{
		...
		if ( NO_ERR == file.seekBlock( blockName ) )
		{
			...
			if ( editCount )
			{
				...
				{
					sprintf( blockName, "%s%ld", editName, i );
					edits[i].init2( &file, blockName, resXFactor, resYFactor );
				}
	...
	if ( animObjectName )
	{
		...
		if ( NO_ERR == file.seekBlock( blockName ) )
		{
			...
			if ( animObjectsCount )
			{
				...
				{
					sprintf( blockName, "%s%ld", animObjectName, i );
					animObjects[i].init2( &file, blockName, neverFlush, resXFactor, resYFactor );
				}
...
void LogisticsScreen::render()
{
	...
	for ( i = 0; i < rectCount; i++ )
	{
		...
		if ( (rects[i].getColor() & 0xff000000) != 0xff000000 ) //magic 30112011
			rects[i].render();
	}
...
void  LogisticsScreen::moveTo( long xPos, long yPos )
{
	...
	if ( Environment.screenWidth == 1280 )
		aObject::init( xPos, yPos, 1280, 1024 ); 
	if ( Environment.screenWidth == 1440 )
		aObject::init( xPos, yPos, 1440, 900 ); 
	//magic 16092011 end*/
