...
long TERRAIN_TXM_SIZE = 128; //magic 20022012
...
long TerrainTextures::init (char *fileName, char *baseName)
{
	...
	*/ //magic 01022012 disabled
	quickLoad = false; //magic 01022012
	tileCacheReqs = 0;
	tileCacheHits = 0;
	tileCacheMiss = 0;
	...
	gosASSERT(tileHeap != NULL);*/ //magic 18032012 disabled

	/*tileRAMHeap = new UserHeap;
	gosASSERT(tileRAMHeap != NULL);*/ //magic 18032012 disabled
	...
	/*//magic 21022012 begin
	terrainTileCacheSize = 1048576;
	//magic 21022012 end*/

	//result = tileHeap->init(65536,"TextureRAM");
	/*result = tileHeap->init(131072,"TextureRAM",true); //magic 17032011
	gosASSERT(result == NO_ERR);*/ //magic 18032012 disabled

	//result = tileRAMHeap->init(terrainTileCacheSize,"TextureRAM");
	/*result = tileRAMHeap->init(terrainTileCacheSize,"TextureRAM",true); //magic 17032011
	gosASSERT(result == NO_ERR);*/ //magic 18032012 disabled

	//localBaseName = (char *)tileRAMHeap->Malloc(strlen(baseName)+1);
	localBaseName = (char *)gos_Malloc(strlen(baseName)+1); //magic 18032012
	...
	textures = new TerrainTXM[numTxms]; //magic 18032012
	...
	types = new MC_TerrainType[numTypes]; //magic 18032012
	...
	overlays = new MC_OverlayType[numOverlays]; //magic 18032012
	...
	details = new MC_DetailType[numDetails]; //magic 18032012
	...
	transitions = new TransitionType[numTransitions]; //magic 18032012
	...
	for (long i=0;i<numTypes;i++)
	{
		...
		if (result == NO_ERR)
		{
			...
			types[i].terrainName = (char *)gos_Malloc(strlen(tmpy)+1); //magic 18032012
			...
			types[i].maskName = (char *)gos_Malloc(strlen(tmpy)+1); //magic 18032012
	...
	for (i=0;i<numTypes;i++)
	{
		if ((i && (types[i].terrainId != types[0].terrainId)) || !i)
		{
			...
			types[i].textureData = (MemoryPtr *)gos_Malloc(sizeof(MemoryPtr) * MC_MAX_MIP_LEVELS); //magic 18032012
			...
			types[i].maskData = (MemoryPtr *)gos_Malloc(sizeof(MemoryPtr) * MC_MASK_NUM * MC_MAX_MIP_LEVELS); //magic 24012012
		...
		else if (i && (types[i].terrainId == types[0].terrainId))
		{
			...
			MemoryPtr ourRAM = (MemoryPtr)gos_Malloc(16 * 16 * sizeof(DWORD)); //magic 18032012
			...
			/*//magic 18032012 begin
			if (ourRAM)
				gos_Free(ourRAM);
			//magic 18032012 end */ //magic 25092012 disabled
		}
	}
	...
	for (i=0;i<numOverlays;i++)
	{
		...
		overlays[i].overlayName = (char *)gos_Malloc(strlen(tmpy)+1); //magic 18032012
	...
	for (i=0;i<numOverlays;i++)
	{
		...
		overlays[i].overlayData = (MemoryPtr *)gos_Malloc(sizeof(MemoryPtr) * overlays[i].numTextures * MC_MAX_MIP_LEVELS); //magic 24012012
	...
	for (i=0;i<numDetails;i++)
	{
		...
		details[i].detailName = (char *)gos_Malloc(strlen(tmpy)+1); //magic 18032012
	...
	for (i=0;i<numDetails;i++)
	{
		...
		details[i].detailData = (MemoryPtr *)gos_Malloc(sizeof(MemoryPtr) * details[i].numDetails * MC_MAX_MIP_LEVELS); //magic 18032012
...
void TerrainTextures::initializeStatistics()
{
	...
	{
			StatisticFormat( "" );
			StatisticFormat( "MechCommander O Terrain Texture" );
			StatisticFormat( "===============================" );
			StatisticFormat( "" );
...
//---------------------------------------------------------------------------
long TerrainTextures::loadOverlayMemory2 (char *textureName, MemoryPtr ourRam)
{
	textures[nextAvailable].mcTextureNodeIndex = mcTextureManager->loadTexture(textureName,gos_Texture_Alpha,gosHint_DisableMipmap | gosHint_DontShrink);
	if (textures[nextAvailable].mcTextureNodeIndex)
		ourRam = (MemoryPtr)textures[nextAvailable].mcTextureNodeIndex;
...
long TerrainTextures::initTexture (long typeNum)
{
	...
	for (long j=0;j<MC_MAX_MIP_LEVELS;j++)
	{
		...
		if (InEditor || !quickLoad)
		{
			...
			tgaFile.seek(0);
			/*//magic 20022012 begin
			MemoryPtr ourRAM = NULL; //magic 20022012

			if ( (header.image_type == UNC_TRUE) && (header.pixel_depth == 32) )
				ourRAM = (MemoryPtr)gos_Malloc(mipSize * mipSize * sizeof(DWORD)*4); //magic 20022012 
			else
				ourRAM = (MemoryPtr)gos_Malloc(mipSize * mipSize * sizeof(DWORD)); //magic 20022012 
				
			gosASSERT(ourRAM != NULL);
			//magic 20022012 end*/ //magic 20022012 no good cause overlays not loading correctly
			/*//magic 21022012 begin
			//MemoryPtr ourRAM = NULL; //magic 20022012
			if ( (header.image_type == UNC_TRUE) && (header.pixel_depth == 32) )
			{
				//ourRAM = (MemoryPtr)malloc(mipSize * mipSize * sizeof(DWORD)*4); //magic 20022012 
				types[typeNum].textureData[j] = (MemoryPtr)malloc(mipSize * mipSize * sizeof(DWORD)*4);
				//overlays[overlayNum].overlayData[txmNum] = (MemoryPtr)tileRAMHeap->Malloc(mipSize * mipSize * sizeof(DWORD)*4);
				loadTGATexture(&tgaFile,types[typeNum].textureData[j],mipSize,mipSize);
			}
			else
			{
				//ourRAM = (MemoryPtr)malloc(mipSize * mipSize * sizeof(DWORD)); //magic 20022012 
				types[typeNum].textureData[j] = (MemoryPtr)malloc(mipSize * mipSize * sizeof(DWORD));
				//overlays[overlayNum].overlayData[txmNum] = (MemoryPtr)tileRAMHeap->Malloc(mipSize * mipSize * sizeof(DWORD));
				loadTGATexture(&tgaFile,types[typeNum].textureData[j],mipSize,mipSize);
			}
			//gosASSERT(ourRAM != NULL);
			//magic 21022012 end*/
			...
			//gos_Free(ourRAM); //magic 20022012
			tgaFile.close();
		}
...
void TerrainTextures::initMask (long typeNum)
{
	...
	for (long i=0;i<MC_MAX_MIP_LEVELS;i++)
	{
		...
		for (long j=0;j<MC_MASK_NUM;j++)
		{
			...
			*/
			//magic 21022012 begin
			types[typeNum].maskData[j+(i*MC_MASK_NUM)] = (MemoryPtr)malloc(mipSize * mipSize * sizeof(BYTE));
			loadTGAMask(&tgaFile,types[typeNum].maskData[j+(i*MC_MASK_NUM)],mipSize,mipSize);
			//magic 21022012 end
...
long TerrainTextures::initOverlay (long overlayNum, long txmNum, char *txmName)
{
	...
	for (long j=0;j<MC_MAX_MIP_LEVELS;j++)
	{
		...
		if (InEditor || !quickLoad)
		{
			...
			{
				//ourRAM = (MemoryPtr)malloc(mipSize * mipSize * sizeof(DWORD)*4); //magic 20022012 
				overlays[overlayNum].overlayData[txmNum] = (MemoryPtr)malloc(mipSize * mipSize * sizeof(DWORD)*4);
				//overlays[overlayNum].overlayData[txmNum] = (MemoryPtr)tileRAMHeap->Malloc(mipSize * mipSize * sizeof(DWORD)*4);
				loadTGATexture(&tgaFile,overlays[overlayNum].overlayData[txmNum],mipSize,mipSize);
			}
			else
			{
				//ourRAM = (MemoryPtr)malloc(mipSize * mipSize * sizeof(DWORD)); //magic 20022012 
				overlays[overlayNum].overlayData[txmNum] = (MemoryPtr)malloc(mipSize * mipSize * sizeof(DWORD));
				//overlays[overlayNum].overlayData[txmNum] = (MemoryPtr)tileRAMHeap->Malloc(mipSize * mipSize * sizeof(DWORD));
				loadTGATexture(&tgaFile,overlays[overlayNum].overlayData[txmNum],mipSize,mipSize);
			}
			//gosASSERT(ourRAM != NULL);
...
/*
			MemoryPtr ourRAM = (MemoryPtr)tileRAMHeap->Malloc(mipSize * mipSize * sizeof(DWORD));
			//MemoryPtr ourRAM = (MemoryPtr)gos_Malloc(mipSize * mipSize * sizeof(DWORD)); //magic 20022012 
			
			gosASSERT(ourRAM != NULL);
			loadTGATexture(&tgaFile,ourRAM,mipSize,mipSize); //magic 20022012 error whenloading overlay tex ture > 64

			overlays[overlayNum].overlayData[txmNum] = ourRAM; //must do memcpy
			*/
			//gos_Free(ourRAM); //magic 20022012
			//free(ourRAM); //magic 20022012
			//free(tgaBuffer); //magic 20022012
			tgaFile.close();
		}
...
long TerrainTextures::createTransition (DWORD typeInfo, DWORD overlayInfo)
{
	...
	if (nextTransition < MAX_MC2_TRANSITIONS)
	{
		...
		for (long kmp=0;kmp<MC_MAX_MIP_LEVELS;kmp++)
		{
			...
			//MemoryPtr ourRAM = (MemoryPtr)tileRAMHeap->Malloc(mipSize * mipSize * sizeof(DWORD));
			MemoryPtr ourRAM = (MemoryPtr)gos_Malloc(mipSize * mipSize * sizeof(DWORD)); //magic 21022012
			...
			for (long k=0;k<4;k++)
			{
				for (long j=k;j<4;j++)
				{
					if (types[priTypes[j]].terrainPriority < types[priTypes[k]].terrainPriority)
					{
						BYTE t = priTypes[k];
						priTypes[k] = priTypes[j];
						priTypes[j] = t;
					}
				}
			}
			...
			*/ //magic 01022012 disabled	
			...
			gos_Free(ourRAM); //magic 21022012
			//free(lzBuffer); //magic 01022012 disabled
		}
...
}*/ //magic 18032012 disabled original func

void TerrainTextures::update (void) //magic 18032012 new function
{
		...
		if (types)
		{
			for (long i = 0;i < numTypes; i++)
			{
				if (types[i].terrainName)
				{
					gos_Free(types[i].terrainName);
					types[i].terrainName = NULL;
				}
				if (types[i].maskName)
				{
					gos_Free(types[i].maskName);
					types[i].maskName = NULL;
				}
				if (types[i].textureData)
				{
					for (long k=0; k<MC_MAX_MIP_LEVELS;k++)
						if (types[i].textureData[k])
						{
							free(types[i].textureData[k]);
							types[i].textureData[k] = NULL;
						}
					gos_Free(types[i].textureData);
					types[i].textureData = NULL;
				}
				if (types[i].maskData)
				{
					for (long k=0; k<MC_MASK_NUM;k++)
						if (types[i].maskData[k])
						{
							free(types[i].maskData[k]);
							types[i].maskData[k] = NULL;
						}
					gos_Free(types[i].maskData);
					types[i].maskData = NULL;
				}
			}
			delete [] types;
		...
		if (overlays)
		{
			for (long i = 0;i < numOverlays; i++)
			{
				if (overlays[i].overlayName)
				{
					gos_Free(overlays[i].overlayName);
					overlays[i].overlayName = NULL;
				}
				if (overlays[i].overlayData)
				{
					for (long k=0; k<overlays[i].numTextures;k++)
						if (overlays[i].overlayData[k])
						{
							free(overlays[i].overlayData[k]);
							overlays[i].overlayData[k] = NULL;
						}

					gos_Free(overlays[i].overlayData);
					overlays[i].overlayData = NULL;
				}
			}
			delete [] overlays;
			overlays = NULL;
		...
		if (details)
		{
			for (long i=0;i<numDetails;i++)
			{
				if (details[i].detailName)
				{
					gos_Free(details[i].detailName);
					details[i].detailName = NULL;
				}
				if (details[i].detailData)
				{
					gos_Free(details[i].detailData);
					details[i].detailData = NULL;
				}
			}
		}	
