...
#define MAX_WEAPON_NODES	4 //magic 31052011
...
void GVAppearanceType::init (char * fileName)
{
	...
	nodeData = new NodeData[numWeaponNodes+numSmokeNodes+numFootNodes]; //magic 01102011
	...
	for (i=1;i<=numSmokeNodes;i++)
	{
		...
		nodeData[i-1].nodeId = (char *)gos_Malloc(strlen(blockId)+1);  //magic 01102011
	...
	for (i=1;i<=numWeaponNodes;i++)
	{
		...
		nodeData[(i-1)+numSmokeNodes].nodeId = (char *)gos_Malloc(strlen(blockId)+1);
	...
	for (i=1;i<=numFootNodes;i++)
	{
		...
		nodeData[(i-1)+numSmokeNodes+numWeaponNodes].nodeId = (char *)gos_Malloc(strlen(blockId)+1); //magic 01102011
...
void GVAppearanceType::destroy (void)
{
	...
	{
		if (gvAnimData[i]) //magic 27112011
		{ //magic 27112011
			delete gvAnimData[i];
			gvAnimData[i] = NULL;
		} //magic 27112011
	...
	if (nodeData)
	{
		for (i=0;i<getTotalNodes();i++)
		{
			...
			if (nodeData[i].nodeId) //magic 27112011
			{ //magic 27112011
...
void GVAppearance::setObjectName (const char *objName) //magic 12092011 new function
{
