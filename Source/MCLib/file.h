...
class File
{
	...
	//------------------
		...
		public:
			...
			long read (DWORD* buffer, long length); //magic 06112011

			//Used to dig the LZ data directly out of the fastfiles.
			// For textures.
			//long readRAW (unsigned long * &buffer, UserHeapPtr heap);
			long readRAW (unsigned long * &buffer, UserHeapPtr heap = NULL); //magic 04112011
			...
			long write (DWORD* buffer, long bytes); //magic 06112011
