...
void initColorTables (void)
{
	...
	colorRGBLookup = (DWORD **)gos_Malloc(sizeof(DWORD *) * numColorRGBTables); //magic 02102011
	...
	for (long i=0;i<numColorRGBTables;i++)
	{
		...
		colorRGBLookup[i] = new DWORD[MAX_COLOR_ENTRIES]; //magic 02102011
...
/*void destroyColorTables (void)
{
	for (long i=0;i<numColorRGBTables;i++)
	{
		...
		delete [] colorRGBLookup[i]; //magic 02102011
		colorRGBLookup[i] = NULL;
	}

	if ( colorRGBLookup )
		//systemHeap->Free(colorRGBLookup);
		gos_Free(colorRGBLookup); //magic 02102011
	colorRGBLookup = NULL;
}	*/

void destroyColorTables (void) //magic 01122011
{
	if ( colorRGBLookup )
	{
	for (long i=0;i<numColorRGBTables;i++)
	{
		if (colorRGBLookup[i])
		{
		delete [] colorRGBLookup[i];
		colorRGBLookup[i] = NULL;
		} 
	}
		gos_Free(colorRGBLookup); 
	colorRGBLookup = NULL;
	} 
}	
