...
void *TG_TypeMultiShape::operator new (size_t mySize)
{
	...
	void *result = gos_Malloc(mySize); //magic 03102011
	return result;
}
...
TG_MultiShapePtr TG_TypeMultiShape::CreateFrom (void)
{
	...
	newShape->listOfShapes = new TG_ShapeRec[numTG_TypeShapes]; //magic 03102011
...
long TG_TypeMultiShape::LoadBinaryCopy (char *fileName)
{
	...
	if (version == CURRENT_SHAPE_VERSION)
	{
		...
		if (numTextures)
		{
			...
			listOfTextures = new TG_Texture[numTextures]; //magic 03102011
		...
		listOfTypeShapes = new TG_TypeNodePtr[numTG_TypeShapes]; //magic 03102011
...
void TG_TypeMultiShape::destroy (void)
{
	...
	if (listOfTypeShapes)
		...
		delete [] listOfTypeShapes; //magic 03102011
		
	listOfTypeShapes = NULL;

	if (listOfTextures)
		//TG_Shape::tglHeap->Free(listOfTextures);
		delete [] listOfTextures; //magic 03102011
	listOfTextures= NULL;
...
void TG_MultiShape::destroy (void)
{
	for (long i=0;i<numTG_Shapes;i++)
	{
		if (listOfShapes[i].node)
		{
			...
			delete listOfShapes[i].node; //magic 03102011
		}
	}

	if (listOfShapes)
		//TG_Shape::tglHeap->Free(listOfShapes);
		delete [] listOfShapes; //magic 03102011
	listOfShapes = NULL;	
...
long TG_TypeMultiShape::LoadTGMultiShapeFromASE (char *fileName, bool forceMakeBinary, IProviderEngine* armProvider)
{
	...
	else
	{
		...
		listOfTypeShapes = new TG_TypeNodePtr[(numTG_TypeShapes+1)]; //magic 03102011
		gosASSERT(listOfTypeShapes != NULL);
		...
		// FOURTH PASS.  Setup Heirarchy ALWAYS or NO Shape BOUNDS!!!
		{
			...
			for (long i=0;i<numTG_TypeShapes;i++)
			{
					...
					(strnicmp(listOfTypeShapes[i]->getNodeId(),"SpotLight",9) != 0))
				{
					...
					// Scan list of vertices and create minBox, maxBox and extentRadius
					for (long k=0;k<listOfTypeShapes[i]->GetNumTypeVertices();k++)
					{
						Stuff::Vector3D relNodeCenter;
						relNodeCenter = listOfTypeShapes[i]->GetRelativeNodeCenter();
						
						TG_TypeShapePtr typeShape = (TG_TypeShapePtr)listOfTypeShapes[i];
						if (minBox.x > typeShape->listOfTypeVertices[k].position.x + relNodeCenter.x)
							minBox.x = typeShape->listOfTypeVertices[k].position.x + relNodeCenter.x;
													 
						if (maxBox.x < typeShape->listOfTypeVertices[k].position.x + relNodeCenter.x)
							maxBox.x = typeShape->listOfTypeVertices[k].position.x + relNodeCenter.x;
													 
						if (minBox.y > typeShape->listOfTypeVertices[k].position.y + relNodeCenter.y)
							minBox.y = typeShape->listOfTypeVertices[k].position.y + relNodeCenter.y;
													 
						if (maxBox.y < typeShape->listOfTypeVertices[k].position.y + relNodeCenter.y)
							maxBox.y = typeShape->listOfTypeVertices[k].position.y + relNodeCenter.y;
													 
						if (minBox.z > typeShape->listOfTypeVertices[k].position.z + relNodeCenter.z)
							minBox.z = typeShape->listOfTypeVertices[k].position.z + relNodeCenter.z;
													 
						if (maxBox.z < typeShape->listOfTypeVertices[k].position.z + relNodeCenter.z)
							maxBox.z = typeShape->listOfTypeVertices[k].position.z + relNodeCenter.z;
					}
					//magic 28112011 end
				}
			}
			...
			if (childChain[j]->calcedThisFrame != turn)
			{
				...
				else
				{
					...
					else
					{
						...
						//------------------------------------------------------------------
						//Then move the piece of the heirarchy into the frame of the parent
						localShapeOrigin = childChain[j]->localShapeToWorld; //magic 28112011 ????
						childChain[j]->localShapeToWorld.Multiply(localShapeOrigin,childChain[j+1]->localShapeToWorld);
			...
			if (Environment.Renderer != 3)
			{
				for (long iLight=0;iLight<TG_Shape::numLights;iLight++)
				{
					if ((TG_Shape::listOfLights[iLight] != NULL) && (TG_Shape::listOfLights[iLight]->active))
					{
						...
						if (TG_Shape::listOfLights[iLight]->lightType)
...
TG_MultiShapePtr TG_MultiShape::Detach (char *nodeName)
{
	...
	resultShape->listOfShapes = new TG_ShapeRec[curShape]; //magic 03102011
	...
	TG_ShapeRecPtr newListOfShapes = new TG_ShapeRec[(numTG_Shapes - curShape)]; //magic 03102011
...
void _TG_Animation::LoadBinaryCopy (File *binFile)
{
	...
	if (quatRAM)
	{
		...
		quat = new Stuff::UnitQuaternion[numFrames];  //magic 03102011
	...
	if (posRAM)
	{
		...
		pos = new Stuff::Point3D[numFrames];  //magic 03102011
...
long TG_AnimateShape::LoadBinaryCopy (char *fileName)
{
	...
	if (animFileVersion == CURRENT_ANIM_VERSION)
	{
		...
		if (count)
		{
			...
			listOfAnimation = new TG_Animation[count]; //magic 03102011
...
long TG_AnimateShape::LoadTGMultiShapeAnimationFromASE (char *fileName, TG_TypeMultiShapePtr shape, bool skipIfBinary, bool forceMakeBinary)
{
	...
	else
	{
		...
		listOfAnimation = new TG_Animation[shape->GetNumShapes()]; //magic 03102011
		gosASSERT(listOfAnimation != NULL);
			...
			else
			{
				...
				if (strstr(nextLine,ASE_ANIM_POS_HEADER) != NULL)
				{
					...
					if (animScan)
					{
						...
						listOfAnimation[i].pos = new Stuff::Point3D[numFrames]; //magic 03102011
				...
				if (strstr(nextLine,ASE_ANIM_ROT_HEADER) != NULL)
				{
					...
					if (animScan)
					{
						...
						listOfAnimation[i].quat = new Stuff::UnitQuaternion[numFrames]; //magic 03102011
