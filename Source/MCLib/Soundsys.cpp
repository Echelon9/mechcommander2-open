...
void SoundSystem::destroy (void)
{
	...
	if (digitalMusicIds)
	{
		for (long i=0; i<numDMS; i++)
			if (digitalMusicIds[i])
			{
				gos_Free(digitalMusicIds[i]);
				digitalMusicIds[i] = NULL;
			}

		gos_Free(digitalMusicIds);
		digitalMusicIds = NULL;
	}

	if (bettySoundBite)
	{
		gos_Free(bettySoundBite);
		bettySoundBite = NULL;
	}

	if (supportSoundBite)
	{
		gos_Free(supportSoundBite);
	...
	if (digitalMusicLoopFlags)
	{
		delete [] digitalMusicLoopFlags;
		digitalMusicLoopFlags = NULL;
	}
	if (digitalMusicVolume)
	{
		delete [] digitalMusicVolume;
		digitalMusicVolume = NULL;
	}	
...
long SoundSystem::init (char *soundFileName)
{
	if (useSound)
	{
		...
		//gosASSERT(result == NO_ERR);
		//magic 20092011 begin
		if ((result != NO_ERR) || soundHeapSize < 16384000)
			soundHeapSize = 16384000;
		//magic 20092011 end 
		*/ //magic 02102011 disabled
...
/*
		...
		result = soundHeap->init(soundHeapSize,"SOUND",true); //magic 17032011
		gosASSERT(result == NO_ERR);
		*/  //magic 02102011 disabled
		//-----------------------------------------------------------------------
		...
		sounds = new SoundBite[numSoundBites]; //magic 02102011
		...
		for (i=0;i<numDMS;i++)
		{
			...
			digitalMusicIds[i] = (char *)gos_Malloc(30); //magic 02102011
...
void SoundSystem::preloadSoundBite (long soundId)
{
	...
	if (packetSize > 0)
	{
		...
		if (thisSoundBite->biteSize == 0 || thisSoundBite->biteData == NULL)
		{
			...
			thisSoundBite->biteData = (MemoryPtr)gos_Malloc(packetSize); //magic 02102011
...
void SoundSystem::update (void)
{
	...
	if (useSound && bettySoundBite)
	{
		...
		if (sampleStatus != gosAudio_PlayOnce)
		{
			...
			gos_Free(bettySoundBite); //magic 02102011
			bettySoundBite = NULL;
		}
	}
	...
	if (useSound && supportSoundBite)
	{
		...
		if (sampleStatus != gosAudio_PlayOnce)
		{
			...
			gos_Free(supportSoundBite); //magic 02102011
			supportSoundBite = NULL;
		}
	}
...
long SoundSystem::playBettySample (unsigned long bettySampleId)
{
	if (useSound && (bettySoundBite == NULL))	//Playing Betty takes precedence
	{
		...
		bettySoundBite = (MemoryPtr)gos_Malloc(bettySize); //magic 02102011
	...
	if (useSound && (supportSoundBite == NULL))		//Playing Support takes precedence
	{
		...
		else 
		{
			...
			if (supportSize > 0)
			{
				...
				supportSoundBite = (MemoryPtr)gos_Malloc(supportSize); //magic 02102011
