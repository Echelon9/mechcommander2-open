...
long TerrainTextures::init (char *fileName, char *baseName)
{
	...
	*/ //magic 24012012 disabled
	...
	gosASSERT(result == NO_ERR);*/ //magic 24012012 disabled
	...
	localBaseName = (char *)gos_Malloc(strlen(baseName)+1); //magic 24012012
	...
	textures = new TerrainTXM[numTxms]; //magic 24012012
	...
	types = new MC_TerrainType[numTypes]; //magic 24012012
	...
	overlays = new MC_OverlayType[numOverlays]; //magic 24012012
	...
	details = new MC_DetailType[numDetails]; //magic 24012012
	...
	transitions = new TransitionType[numTransitions]; //magic 24012012
	...
	for (long i=0;i<numTypes;i++)
	{
		...
		if (result == NO_ERR)
		{
			...
			types[i].terrainName = (char *)gos_Malloc(strlen(tmpy)+1); //magic 24012012
			...
			types[i].maskName = (char *)gos_Malloc(strlen(tmpy)+1); //magic 24012012
	...
	for (i=0;i<numTypes;i++)
	{
		if ((i && (types[i].terrainId != types[0].terrainId)) || !i)
		{
			...
			types[i].textureData = (MemoryPtr *)gos_Malloc(sizeof(MemoryPtr) * MC_MAX_MIP_LEVELS); //magic 24012012
		...
		else if (i && (types[i].terrainId == types[0].terrainId))
		{
			...
			MemoryPtr ourRAM = (MemoryPtr)gos_Malloc(16 * 16 * sizeof(DWORD));
	...
	for (i=0;i<numOverlays;i++)
	{
		...
		overlays[i].overlayName = (char *)gos_Malloc(strlen(tmpy)+1); //magic 24012012
	...
	for (i=0;i<numDetails;i++)
	{
		...
		details[i].detailName = (char *)gos_Malloc(strlen(tmpy)+1); //magic 24012012
	...
	for (i=0;i<numDetails;i++)
	{
		...
		details[i].detailData = (MemoryPtr *)gos_Malloc(sizeof(MemoryPtr) * details[i].numDetails * MC_MAX_MIP_LEVELS); //magic 24012012
...
long TerrainTextures::initTexture (long typeNum)
{
	...
	for (long j=0;j<MC_MAX_MIP_LEVELS;j++)
	{
	 ...
	 //magic 21022012 disabled
			tgaFile.seek(0);
	
			//MemoryPtr ourRAM = (MemoryPtr)tileRAMHeap->Malloc(mipSize * mipSize * sizeof(DWORD));
			/*MemoryPtr ourRAM = (MemoryPtr)gos_Malloc(mipSize * mipSize * sizeof(DWORD)); //magic 24012012
			gosASSERT(ourRAM != NULL);
			...
			types[typeNum].textureData[j] = (MemoryPtr)gos_Malloc(mipSize * mipSize * sizeof(DWORD));
...
void TerrainTextures::initMask (long typeNum)
{
	...
	for (long i=0;i<MC_MAX_MIP_LEVELS;i++)
	{
		...
		for (long j=0;j<MC_MASK_NUM;j++)
		{
			...
			MemoryPtr ourRAM = (MemoryPtr)gos_Malloc(mipSize * mipSize * sizeof(BYTE)); //magic 24012012
			gosASSERT(ourRAM != NULL);
...
long TerrainTextures::initOverlay (long overlayNum, long txmNum, char *txmName)
{
	...
	for (long j=0;j<MC_MAX_MIP_LEVELS;j++)
	{
		...
		/*	MemoryPtr ourRAM = (MemoryPtr)gos_Malloc(mipSize * mipSize * sizeof(DWORD)); //magic 24012012
			...
			//magic 21022012 begin
			overlays[overlayNum].overlayData[txmNum] = (MemoryPtr)gos_Malloc(mipSize * mipSize * sizeof(DWORD));
...
}*/ //magic 24012012 disabled original func
void TerrainTextures::update (void) //magic 24012012 new function
{
...
void TerrainTextures::destroy (void)
{
	...
	//magic 24012012 end
	//delete tileHeap; //magic 24012012 disabled
	//tileHeap = NULL; //magic 24012012 disabled
}
