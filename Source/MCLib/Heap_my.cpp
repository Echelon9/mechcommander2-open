...
//---------------------------------------------------------------------------
long HeapManager::createHeap (unsigned long long memSize)
{
...
//---------------------------------------------------------------------------
long HeapManager::commitHeap (unsigned long long commitSize)
{
	if (commitSize == 0)
	...
	if (result == heap)
	{
		...
		// do its own unwind.
		unsigned long long currentEbp;
		unsigned long long prevEbp;
		...
		prevEbp = *((unsigned long long *)currentEbp);
		retAddr = *((unsigned long long *)(currentEbp+4));
		whoMadeMe = retAddr;
...
//---------------------------------------------------------------------------
long HeapManager::decommitHeap (unsigned long long decommitSize)
{
	long result = 0;
	...
	unsigned long long decommitAddress = decommitSize;
...
//---------------------------------------------------------------------------
long UserHeap::init (unsigned long long memSize, char *heapId, bool useGOS)
{
	if (heapId)
	{
	...
	if (!useGOS)
	{
		...
		unsigned long long heapTop = (unsigned long long)heap;
		heapTop += memSize;
		heapTop -= 16;				
		heapTop &= ~3;				//Force top to be DWORD boundary. //magic 26022011 disabled
	
		unsigned long long heapBottom = (unsigned long long)heap;
		...
		unsigned long long length = heapTop-heapBottom;
...
//---------------------------------------------------------------------------
unsigned long long UserHeap::totalCoreLeft (void)
{
	...
	long long heapBlockSize = -(long long)allocatedBlockSize;
...
//---------------------------------------------------------------------------
unsigned long long UserHeap::coreLeft (void)
{
	unsigned long result = 0;
...
//---------------------------------------------------------------------------
void * UserHeap::Malloc (unsigned long long memSize)
{
	void * result = NULL;
	if (gosHeap)
	{
...
//---------------------------------------------------------------------------
void * UserHeap::calloc (unsigned long long memSize)
{
...
void UserHeap::walkHeap (bool printIt, bool skipAllocated)
{
	...
	while (walker->blockSize != 1)
	{
		...
		if (walker->upperBlock)
		{
			...
			bSize += (unsigned long long)walker->upperBlock;
			valid = (bSize == (unsigned long long)walker);
		}

		if (valid)
		{
			bSize = (unsigned long long)walker + (walker->blockSize & ~1);
