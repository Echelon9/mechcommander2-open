...
//---------------------------------------------------------------------------
void tgaDecomp(DWORD* dest, DWORD* source, TGAFileHeader *tga_header, bool dd) //magic 06112011
{
	//---------------------------------------------------------------------
	// Display Image based on Format 10 (0x0a).  RLE true color.
...
//---------------------------------------------------------------------------
void flipTopToBottom (DWORD* buffer, BYTE depth, long width, long height, bool dd) //magic 06112011
{
	//-----------------------------------------------------------
	// ScanLine by Scanline
	DWORD* tmpBuffer = (DWORD*)malloc(width * height * (depth>>3));
	DWORD* tmpPointer = tmpBuffer;

	//----------------------------------
	// Point to last scanline.
	long pixelWidth = width * (depth >> 3);
	DWORD* workBuffer = buffer + ((height - 1) * pixelWidth);
...
//---------------------------------------------------------------------------
static const int g_textureCache_BufferSize = 65536; //16384/*64*64*sizeof(DWORD)*/; //magic 21022012
	...
	switch (header->image_type)
	{
		...
		case UNC_TRUE:
		{
			...
			if (header->pixel_depth == 32)
				memcpy(ourRAM,image,width * height * 4); //magic 20022012 problem loading size bigger then ourRAM when overlay texture > 64
				//memcpy(ourRAM,image,width * height); //magic 21022012 wont load overlays at all
				//memcpy(ourRAM,image,sizeof(image)); //magic 21022012 no good
			else
