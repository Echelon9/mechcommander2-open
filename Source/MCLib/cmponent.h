...
typedef union {
	...
	struct {
		...
		unsigned int		minWRange; //min weapon range
		unsigned int		maxWRange; //max weapon range
		//magic 11022012 end
	} weapon;
	struct {
...
class MasterComponent {
	...
	public:
		...
		//magic 14092011 begin
		float getArmorPoints (void) { 
			return(stats.armor.armorPoints);
		}
		//magic 14092011 end
...
//magic 11022012 begin
		long getMinWeaponRange (void) {
			return(stats.weapon.minWRange);
		}

		void setMinWeaponRange (long weaponRange) {
			stats.weapon.minWRange = weaponRange;
		}

		long getMaxWeaponRange (void) {
			return(stats.weapon.maxWRange);
		}

		void setMaxWeaponRange (long weaponRange) {
			stats.weapon.maxWRange = weaponRange;
		}
//magic 11022012 end
