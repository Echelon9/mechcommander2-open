...
const float					Terrain::worldUnitsPerVertex = 128.0; //magic 21022011 ?
...
//UserHeapPtr					Terrain::terrainHeap = NULL;			//Setup at load time.  //magic 01102011X3 disabled
...
#define						MAX_TERRAIN_HEAP_SIZE		65536000 //magic 29052012 //16384000//1024000 //magic 11032012
...
void Terrain::initMapCellArrays (void)
{
	if (!tileRowToWorldCoord)
	{
		...
		tileRowToWorldCoord = new float[realVerticesMapSide]; //magic 01102011X2
	...
	if (!tileColToWorldCoord)
	{
		...
		tileColToWorldCoord = new float[realVerticesMapSide]; //magic 01102011 
	...
	if (!cellToWorldCoord)
	{
		...
		cellToWorldCoord = new float[MAPCELL_DIM];  //magic 01102011
	...
	if (!cellColToWorldCoord)
	{
		...
		cellColToWorldCoord = new float[realVerticesMapSide * MAPCELL_DIM];  //magic 01102011
	...
	if (!cellRowToWorldCoord)
	{
		...
		cellRowToWorldCoord = new float[realVerticesMapSide * MAPCELL_DIM];  //magic 01102011
					...
					float percentRange )
{
	...
	//int tmp_old = pakFile->getPacketSize();
	long tmp = pakFile->getPacketSize();

	/*//Magic Debug file begin
	char tmp2[256];
	FitIniFile MtestFile;
	MtestFile.open("data\\packet_size.fit", CREATE);
	sprintf( tmp2, "long_packetsize %ld", tmp );
	MtestFile.writeBlock( tmp2 );
	sprintf( tmp2, "sizeof_postcomvertex %ld", sizeof(PostcompVertex) );
	MtestFile.writeBlock( tmp2 );
	MtestFile.close();
	...
	if (!justResaveAllMaps && 
		(realVerticesMapSide != 280) && //magic 23022011
		(realVerticesMapSide != 260) && //magic 23022011
		(realVerticesMapSide != 240) && //magic 21022011
		(realVerticesMapSide != 220) && //magic 22022011
		(realVerticesMapSide != 200) && //magic 23022011
		(realVerticesMapSide != 180) && //magic 23022011
		(realVerticesMapSide != 160) && //magic 21022011
		(realVerticesMapSide != 140) && //magic 22022011
	...
	if (file)
	{
		if (file->seekBlock("ColorMap") == NO_ERR)
		{
			...
			if (file->readIdString("ColorMapName",mapName,1023) == NO_ERR)
			{
				...
				//colorMapName = (char *)gos_Malloc(strlen(mapName)+1); //magic 26092011
					...
					float percentRange)
{
	...
	if (verticesPerMapSide > 300)
		verticesPerMapSide /= 12.8; //reverse 24032012
		//verticesPerMapSide /= 25.6; //magic 06032012
		
	realVerticesMapSide = verticesPerMapSide;
	halfVerticesMapSide = realVerticesMapSide >> 1;
	//halfVerticesMapSide = realVerticesMapSide / 2;
	...
	FitIniFile MtestFile;
	MtestFile.open("data\\packet_size.fit", CREATE);
	sprintf( tmp2, "realVerticesMapSide %ld", realVerticesMapSide );
	MtestFile.writeBlock( tmp2 );
	sprintf( tmp2, "halfVerticesMapSide %ld", halfVerticesMapSide );
	MtestFile.writeBlock( tmp2 );
	sprintf( tmp2, "blocksMapSide %ld", blocksMapSide );
	MtestFile.writeBlock( tmp2 );
	sprintf( tmp2, "verticesBlockSide %ld", verticesBlockSide );
	MtestFile.writeBlock( tmp2 );
	sprintf( tmp2, "worldUnitsMapSide %ld", worldUnitsMapSide );
	MtestFile.writeBlock( tmp2 );
	sprintf( tmp2, "worldUnitsPerVertex %ld", worldUnitsPerVertex );
	MtestFile.writeBlock( tmp2 );
	sprintf( tmp2, "Stuff::SMALL %f", Stuff::SMALL );
	MtestFile.writeBlock( tmp2 );
	//MtestFile.close();
	//Magic debug file end*/
	...
	//terrainHeapSize = MAX_TERRAIN_HEAP_SIZE;  //magic 01102011X3 disabled
	/*//magic debug file
	sprintf( tmp2, "oneOverWorldUnitsMapSide %f", oneOverWorldUnitsMapSide );
	MtestFile.writeBlock( tmp2 );
	sprintf( tmp2, "Terrain::numObjBlocks %ld", Terrain::numObjBlocks );
	MtestFile.writeBlock( tmp2 );
	MtestFile.close();
	//Magic debug file end*/
	//-----------------------------------------------------------------
	...
	if( !terrainHeap )
	{
		...
		terrainHeap->init(terrainHeapSize,"TERRAIN",true); //magic 17032011
	}
	*/  //magic 01102011X3 disabled
	...
	if ( !terrainTextures2 && pakFile)
	{
		...
		//terrainName = (char *)gos_Malloc(strlen(name)+1); //magic 26092011
	...
	objBlockInfo = new ObjBlockInfo[numObjBlocks]; //magic 01102011
	...
	objVertexActive = new bool[realVerticesMapSide * realVerticesMapSide]; //magic 01102011
	gosASSERT(objVertexActive != NULL);
	
	//memset(objVertexActive,0,sizeof(bool)*numObjBlocks);
	memset(objVertexActive,0,sizeof(bool)*realVerticesMapSide * realVerticesMapSide); //magic 06032012
	
	//moverBlockList = (long *)terrainHeap->Malloc(sizeof(long) * numberBlocks);
	moverBlockList = new long[numberBlocks]; //magic 01102011
	gosASSERT(moverBlockList != NULL);
	
	//usedBlockList = (long *)terrainHeap->Malloc(sizeof(long) * numberBlocks);
	usedBlockList = new long[numberBlocks]; //magic 01102011
	...
	vertexList = new Vertex[visibleVertices * visibleVertices]; //magic 01102011
	...
	quadList = new TerrainQuad[visibleVertices * visibleVertices]; //magic 01102011
...
void Terrain::resetVisibleVertices (long maxVisibleVertices)
{
	...
	delete [] vertexList; //magic 01102011
	vertexList = NULL;

	//terrainHeap->Free(quadList);
	delete [] quadList; //magic 01102011
	quadList = NULL;
	...
	vertexList = new Vertex[visibleVerticesPerSide * visibleVerticesPerSide]; //magic 01102011
	gosASSERT(vertexList != NULL);
	...
	quadList = new TerrainQuad[visibleVerticesPerSide * visibleVerticesPerSide]; //magic 01102011
	gosASSERT(quadList != NULL);
...
void Terrain::destroy (void)
{
	...
	if (tileRowToWorldCoord)
	{
		...
		delete [] tileRowToWorldCoord; //magic 01102011
	...
	if (tileColToWorldCoord)
	{
		...
		delete [] tileColToWorldCoord; //magic 01102011
	...
	if (cellToWorldCoord)
	{
		...
		delete [] cellToWorldCoord; //magic 01102011
	...
	if (cellColToWorldCoord)
	{
		...
		delete [] cellColToWorldCoord; //magic 01102011
	...
	if (cellRowToWorldCoord)
	{
		...
		delete [] cellRowToWorldCoord; //magic 01102011
	...
	if (moverBlockList)
	{
		...
		delete [] moverBlockList; //magic 01102011
		moverBlockList = NULL;
	}
	...
	if (usedBlockList)
	{
		...
		delete [] usedBlockList; //magic 01102011
		usedBlockList = NULL;
	}
	...
	if (objBlockInfo)
	{
		...
		delete [] objBlockInfo; //magic 01102011
		objBlockInfo = NULL;
	}
	...
	if (objVertexActive)
	{
		...
		delete [] objVertexActive; //magic 01102011
