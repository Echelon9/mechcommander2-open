...
#define COLOR_TXM_HEAP_SIZE				32768 //4096 //magic 23022011
#define COLOR_MAP_TEXTURE_SIZE			256 //magic 05112011 512 too big
...
void TerrainColorMap::destroy (void)
{
	...
	if (txmRAM)
	{
		for (unsigned long i=0;i<numTextures;i++)
		{
			...
			free(txmRAM[i].ourRAM); //magic 05112011
...
void TerrainColorMap::recalcLight(char *fileName)
{
	...
	*/ //magic 05112011 disabled	
	...
	if (colorMapInfo.image_type == UNC_TRUE)
	{
		...
		ColorMap = (MemoryPtr)malloc(colorMapInfo.width * colorMapInfo.width * sizeof(DWORD)); //magic 05112011
		...
		for (unsigned long i=0;i<numTextures;i++)
		{
			...
			txmRAM[i].ourRAM = (MemoryPtr)malloc(sizeof(DWORD) * COLOR_MAP_TEXTURE_SIZE * COLOR_MAP_TEXTURE_SIZE); //magic 05112011
