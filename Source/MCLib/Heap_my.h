...
typedef struct _GlobalHeapRec
{
	...
	unsigned long long  totalCoreLeft;
	unsigned long long	coreLeft;
} GlobalHeapRec;
...
class HeapManager
{
	...
	protected:
		...
		unsigned long long  		committedSize;
		unsigned long long			whoMadeMe;
...
//		void	ConstructBlkInfoLine (PVMQUERY pVMQ, LPTSTR szLine, int nMaxLen);
	...
	public:
		...
		long createHeap (unsigned long long memSize);
		long commitHeap (unsigned long long commitSize = 0);
		long decommitHeap (unsigned long long decommitSize = 0);
...
class UserHeap : public HeapManager
{
	...
	public:
		...
		UserHeap (void);
		long init (unsigned long long memSize, char *heapId = NULL, bool useGOS = false);
		
		~UserHeap (void);
		void destroy (void);
		
		unsigned long long totalCoreLeft (void);
		unsigned long long coreLeft (void);
		unsigned long long size (void) { return heapSize;}
		
		void *Malloc (unsigned long long memSize);
		long Free (void *memBlock);
		
		void *calloc (unsigned long long memSize);
