...
long CraterManager::init (long numCraters, unsigned long craterTypeSize, char *craterFileName)
{
	...
	//craterPosHeapSize = numCraters * (sizeof(CraterData) + allocatedBlockSize); 	//magic 05112011 disabled
	unsigned long craterPosHeapSize = numCraters * (sizeof(CraterData)); 	//magic 05112011
	...
	*/ 	//magic 05112011 disabled
	...
	craterTextureHandles = new DWORD[numCraterTextures]; //magic 05112011
	//memset((MemoryPtr)craterTextureHandles,0xff,sizeof(DWORD) * numCraterTextures);
	memset(craterTextureHandles,0xff,sizeof(DWORD) * numCraterTextures); //magic 07112011

	//craterTextureIndices = (DWORD *)malloc(sizeof(DWORD) * numCraterTextures);
	craterTextureIndices = new DWORD[numCraterTextures]; //magic 05112011
	//memset((MemoryPtr)craterTextureIndices,0xff,sizeof(DWORD) * numCraterTextures);
	memset(craterTextureIndices,0xff,sizeof(DWORD) * numCraterTextures); //magic 07112011
	...
	for (long i=0;i<numCraterTextures;i++)
	{
		...
		if (!i)
			...
			craterTextureIndices[i] = mcTextureManager->loadTexture(craterPath, gos_Texture_Alpha, gosHint_DisableMipmap); //magic 25092011
		else
			craterTextureIndices[i] = mcTextureManager->loadTexture(craterPath, gos_Texture_Alpha, gosHint_DisableMipmap);

	}

	//magic 07112011 begin
	craterTextures = 0;
	char craterName2[1024];
	sprintf(craterName2,"defaults\\feet0000");
	FullPathFileName craterPath2;
	craterPath2.init(texturePath,craterName2,".tga");
	craterTextures = mcTextureManager->loadTexture(craterPath2, gos_Texture_Alpha, gosHint_DisableMipmap);
	//magic 07112011 end
	/*//magic 22092011 begin
	char craterName[1024];
	sprintf(craterName,"defaults\\feet0000");
	FullPathFileName craterPath;
	craterPath.init(texturePath,craterName,".tga");
	craterTextureIndices[0] = mcTextureManager->loadTexture(craterPath, gos_Texture_Keyed, gosHint_DisableMipmap);

	sprintf(craterName,"defaults\\feet0001");
	FullPathFileName craterPath1;
	craterPath1.init(texturePath,craterName,".tga");
	craterTextureIndices[1] = mcTextureManager->loadTexture(craterPath1, gos_Texture_Alpha, gosHint_DisableMipmap);
	//magic 22092011 end*/
	//-----------------------------------------------------
	// Setup pointer and initial values
	//craterList = (CraterDataPtr)craterPosHeap->getHeapPtr(); 	//magic 05112011 disabled
	craterList = new CraterData[numCraters]; 	//magic 05112011
	//memset(craterPosHeap->getHeapPtr(),-1,craterPosHeapSize); 	//magic 05112011 disabled
	//memset((MemoryPtr)craterList,-1,sizeof(CraterData) * numCraters); //magic 05112011
	memset(craterList,-1,sizeof(CraterData) * numCraters); //magic 07112011
...
void CraterManager::destroy (void)
{
	...
	*/ //magic 05112011
	...
	if (craterTextureHandles)
	{
		delete [] craterTextureHandles; //magic 05112011
	//magic 05112011 end
	...
	{
		delete [] craterTextureIndices; //magic 05112011
	//magic 05112011 end
	...
	craterTextures = 0; //magic 07112011
}			
	...
	for (long i=0;i<(long)maxCraters;i++,currentCrater++)
	{
		if (currentCrater->craterShapeId != -1)
		{
			if (currentCrater->craterShapeId > TURKINA_FOOTPRINT)	//We are standard crater.
			{
				...
				//craterTextureHandles[0] = craterTextureIndices[0]; //magic 28092011
				craterTextureHandles[0] = craterTextures; //magic 07112011
				//mcTextureManager->addTriangle(craterTextureIndices[0],MC2_ISCRATERS | MC2_DRAWALPHA | MC2_ISTERRAIN);
				//mcTextureManager->addTriangle(craterTextureIndices[0],MC2_ISCRATERS | MC2_DRAWALPHA | MC2_ISTERRAIN);
				mcTextureManager->addTriangle(craterTextures,MC2_ISCRATERS | MC2_DRAWALPHA | MC2_ISTERRAIN); //magic 07112011
				mcTextureManager->addTriangle(craterTextures,MC2_ISCRATERS | MC2_DRAWALPHA | MC2_ISTERRAIN); //magic 07112011
			}
			...
			if (onScreen1 || onScreen2 || onScreen3 || onScreen4)
			{
				...
				else
				{
					...
					if (currCrater->craterShapeId > TURKINA_FOOTPRINT)	//We are standard crater.
					{
						...
						//mcTextureManager->addVertices(craterTextureIndices[0],gVertex,MC2_ISCRATERS | MC2_DRAWALPHA | MC2_ISTERRAIN);
						//mcTextureManager->addVertices(craterTextureIndices[0],sVertex,MC2_ISCRATERS | MC2_DRAWALPHA | MC2_ISTERRAIN);
						mcTextureManager->addVertices(craterTextures,gVertex,MC2_ISCRATERS | MC2_DRAWALPHA | MC2_ISTERRAIN); //magic 07112011
						mcTextureManager->addVertices(craterTextures,sVertex,MC2_ISCRATERS | MC2_DRAWALPHA | MC2_ISTERRAIN); //magic 07112011
					}
					else		//We are a footprint
					{
						...
						mcTextureManager->addVertices(craterTextureIndices[1],gVertex,MC2_ISCRATERS | MC2_DRAWALPHA);
						mcTextureManager->addVertices(craterTextureIndices[1],sVertex,MC2_ISCRATERS | MC2_DRAWALPHA);
					}
				}
			}
		}
