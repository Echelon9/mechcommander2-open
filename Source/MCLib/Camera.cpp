...
float Camera::MaxClipDistance	= 4000.0f; //magic 18022011 lowering num renderable objects when lower value
...
//	gosASSERT(result == NO_ERR);
	...
	if ( !worldLights ) // only do the alloc once
	{
		...
		worldLights = new TG_LightPtr[MAX_LIGHTS_IN_WORLD]; //magic 02102011
		...
		worldLights[0] = new TG_Light; //magic 02102011
	...
	if ( !activeLights ) // only do the alloc once
	{
		...
		activeLights = new TG_LightPtr[MAX_LIGHTS_IN_WORLD]; //magic 02102011
	...
	if ( !terrainLights ) // only do the alloc once
	{
		...
		terrainLights = new TG_LightPtr[MAX_LIGHTS_IN_WORLD]; //magic 02102011
	...
	worldLights[1] = new TG_Light; //magic 02102011
...
void Camera::destroy (void)
{
	if (worldLights)
	{
		...
		delete worldLights[1]; //magic 02102011
		worldLights[1] = NULL;

		//systemHeap->Free(worldLights[0]);
		delete worldLights[0]; //magic 02102011
		worldLights[0] = NULL;

		//systemHeap->Free(worldLights);
		delete [] worldLights; //magic 02102011
		worldLights = NULL;
	}
	...
	if (activeLights)
	{
		...
		delete [] activeLights; //magic 02102011
		activeLights = NULL;
	}
	...
	if (terrainLights)
	{
		...
		delete [] terrainLights; //magic 02102011
		terrainLights = NULL;
	}
}
...
void Camera::zoomValue (float value)
{
	...
	else
	{
		...
		if (cameraAltitude > 3000.0)
		{
			if (projectionAngle < 55.0)
				projectionAngle = 55.0;//55
		}
		else if (cameraAltitude > 2000.0)
		{
			if (projectionAngle < 45.0)
				projectionAngle = 45.0;//45
		}
		else if (cameraAltitude > 1000.0)
		{
			if (projectionAngle < 35.0)
				projectionAngle = 35.0;
		}
		//magic 18022011 end*/
...
void Camera::ZoomIn (float amount)
{
	...
	else
	{
		...
		{
			if (projectionAngle < 55.0)
				projectionAngle = 55.0;//75
		}
		else if (cameraAltitude > 2000.0)
		{
			if (projectionAngle < 45.0)
				projectionAngle = 45.0;//55
		}
		else if (cameraAltitude > 1000.0)
		{
			if (projectionAngle < 35.0)
