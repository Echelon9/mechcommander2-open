	...
	else
	{
		...
		else		//45Pixel mechs dealt with here.
		{
			if (!tempBuffer)
				...
				tempBuffer = (MemoryPtr)gos_Malloc(MAX_X * MAX_Y); //magic 02102011
...
void VFXShapeElement::drawShape (void)
{
	...
	if (actualHeight == -1)
	{
		if ((xRes < 16) && (yRes < 16))
		{
			if (!shapeBuffer16)
				...
				shapeBuffer16 = (MemoryPtr)gos_Malloc(16*16); //magic 02102011
		...
		else if ((xRes < 32) && (yRes < 32))
		{
			if (!shapeBuffer32)
				...
				shapeBuffer32 = (MemoryPtr)gos_Malloc(32*32);  //magic 02102011
		...
		else if ((xRes < 64) && (yRes < 64))
		{
			if (!shapeBuffer64)
				...
				shapeBuffer64 = (MemoryPtr)gos_Malloc(64*64); //magic 02102011
		...
		else if ((xRes < 128) && (yRes < 128))
		{
			if (!shapeBuffer128)
				...
				shapeBuffer128 = (MemoryPtr)gos_Malloc(128*128); //magic 02102011
		...
		else if ((xRes < 256) && (yRes < 256))
		{
			if (!shapeBuffer256)
				...
				shapeBuffer256 = (MemoryPtr)gos_Malloc(256*256); //magic 02102011
	...
	if (!tempBuffer)
		//tempBuffer = (MemoryPtr)systemHeap->Malloc(MAX_X * MAX_Y);
		tempBuffer = (MemoryPtr)gos_Malloc(MAX_X * MAX_Y);
