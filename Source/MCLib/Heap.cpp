...
long HeapManager::createHeap (unsigned long memSize)
{
	...
	//heap = (MemoryPtr)gos_Malloc(memSize); //magic 26112011
	//heap = (MemoryPtr)HeapAlloc(heap,HEAP_GENERATE_EXCEPTIONS,memSize); //magic 26112011
	/*//magic 17032011 begin
	heap = (MemoryPtr)HeapCreate(0,0,0);
	if (heap == NULL) 
	{
        //_tprintf(TEXT("Failed to create a new heap with LastError %d.\n"), GetLastError());
		STOP(("Failed HeapCreate with LastError %d.\n", GetLastError()));
...
long HeapManager::commitHeap (unsigned long commitSize)
{
	...
	//MemoryPtr result = (MemoryPtr)gos_Malloc(commitSize); //magic 26112011
	/*//magic 17032011 begin
	MemoryPtr result = (MemoryPtr)HeapAlloc(heap,HEAP_GENERATE_EXCEPTIONS,commitSize);
	if (result == NULL) 
	{
        //_tprintf(TEXT("Failed to create a new heap with LastError %d.\n"), GetLastError());
		STOP(("Failed HeapAlloc with LastError %d.\n", GetLastError()));
        return COULDNT_COMMIT;
    }
	//magic 17032011 end*/

	if (result == heap)
	//if (result) //magic 17032011 no good
	{
...
long HeapManager::decommitHeap (unsigned long decommitSize)
{
	...
	if (result == FALSE)
		result = GetLastError();
	/*//magic 17032011 begin
	if (HeapFree(heap,0,(void *)committedSize)) 
	{
        _tprintf(TEXT("Failed to decommit heap with LastError %d.\n"),
                 GetLastError());
        return 1;
    }
	//magic 17032011 end*/
...
long UserHeap::init (unsigned long memSize, char *heapId, bool useGOS)
{
	...
	{
		//gosHeap = gos_CreateMemoryHeap(heapId,memSize); //magic 29032011 not solution for invalid allocation 0 bytes for support pallete
		gosHeap = gos_CreateMemoryHeap(heapId,0); //magic 17032011
		...
		//heapName = NULL; //magic 19032011 disabled nothing with support VEHICLES null alloc
		heapState = NO_ERR; 
...
#ifdef SAFE_HEAP //magic 05082011 disabled - no good
		or		eax,eax		   				//return 0 on error
		je      Dealloc_Done
#endif //magic 05082011 disabled
