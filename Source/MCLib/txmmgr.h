...
#define MC_MAXTEXTURES				8192 //4096 //magic 25112011 more then 8192 does no good
#define CACHED_OUT_HANDLE			0xFFFFFACE		//If this value is in gosTextureHandle, texture is cached out and must be cached in.
#define MAX_MC2_GOS_TEXTURES		1500 //750 //magic 21022011
#define TEXTURE_CACHE_SIZE			(200*1024*1024) //(120*1024*1024) - magic 06032012	// magic 04112011 was (80*1024*1024)	//Extra space here to facilitate editting //magic 17022011 was 40*
#define MAX_CACHE_SIZE				(200*1024*1024) //(120*1024*1024) - magic 06032012	//Actual amount map must run in! //magic 17022011 was 40*
#define MC_MAXFACES					500000 //200000//50000 //magic 21022011- magic 06032012
//#define MAX_LZ_BUFFER_SIZE			((512*512*4) + 1024) //magic 21022011 was ((256*256*4) + 1024) //size of the texture for loading 512x512 textures
#define MAX_LZ_BUFFER_SIZE			((1024*1024*4) + 1024) //magic 13022012
...
class MC_TextureManager
{
	...
	protected:
		...
		//UserHeapPtr						textureCacheHeap;			//Heap used to cache textures from vidCard to system RAM. //magic 04112011 disabled
		//UserHeapPtr						textureStringHeap;			//Heap used to store filenames of textures so no dupes. //magic 04112011 disabled
	...
	public:
		...
		void init (void)
		{
			...
			//textureCacheHeap = NULL; //magic 04112011 disabled
			//textureStringHeap = NULL; //magic 04112011 disabled
		...
		void startVertices (long maxVertices = 3000000); //magic 29052012 (720000) 17022011 was 30000
		...
		void addVertices (DWORD nodeId, gos_VERTEX *data, DWORD flags)
		{
			...
			if (nodeId < MC_MAXTEXTURES)
			{
					...
					masterTextureNodes[nodeId].vertexData->flags == flags)
				{
					...
					if (vertices < (masterTextureNodes[nodeId].vertexData->vertices + masterTextureNodes[nodeId].vertexData->numVertices)) //magic 25112011 disabled
					//if ((vertices) && (vertices < (masterTextureNodes[nodeId].vertexData->vertices + masterTextureNodes[nodeId].vertexData->numVertices))) //magic 25112011
					{
						#if 0
						if ((data[0].u > 64.0f) ||
							(data[0].u < -64.0f) || 
						 ...
						 masterTextureNodes[nodeId].vertexData2->flags == flags)
				{
					...
					#endif
					//gosASSERT(oldVertices < oldStart); //magic 25112011 disabled
					...
					if (vertices < (masterTextureNodes[nodeId].vertexData2->vertices + masterTextureNodes[nodeId].vertexData2->numVertices)) //magic 25112011 disabled
					//if ((vertices) && (vertices < (masterTextureNodes[nodeId].vertexData2->vertices + masterTextureNodes[nodeId].vertexData2->numVertices))) //magic 25112011 nije resenje
					{
						#if 0
						if ((data[0].u > 64.0f) ||
							(data[0].u < -64.0f) || 
						...
						#endif
//magic 17022011 here crushes on memcpy	- in mission2 defined totalvertices to 100000					
						memcpy(vertices,data,sizeof(gos_VERTEX) * 3); //magic 25112011 disabled
						/*//magic 25112011 begin
						errno_t err;
						err = memcpy_s(vertices,sizeof(gos_VERTEX) * 3,data,sizeof(gos_VERTEX) * 3); //magic 25112011
						if (err)
						{
					...
					//if (vertices) //magic mozda radi probati ponovo
						 ...
						 masterTextureNodes[nodeId].vertexData3->flags == flags)
				{
					...
					if (vertices < (masterTextureNodes[nodeId].vertexData3->vertices + masterTextureNodes[nodeId].vertexData3->numVertices)) //magic 25112011 disabled
					//if ((vertices) && (vertices < (masterTextureNodes[nodeId].vertexData3->vertices + masterTextureNodes[nodeId].vertexData3->numVertices))) //magic 25112011
					{
						#if 0
						if ((data[0].u > 64.0f) ||
							(data[0].u < -64.0f) || 
