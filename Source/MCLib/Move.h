...
//#define	USE_FORESTS			1 //magic 26072011

#define	MAX_GLOBALMAP_DOORS		20000 //10000 magic 06032012
#define	MAX_GLOBALMAP_AREAS		20000 //10000 magic 06032012
...
#define	OVERLAYINDEX_NULL		1 //magic 05102012
...
#define MAX_GLOBAL_PATH					200 //50 //magic 06032012

#define	NUM_MOVE_LEVELS					2

#define	MAX_WALL_OBJECTS				4000 //2000 //magic 05122011
#define	MAX_GATES_OBJECT				2000 //1000 //magic 05122011
#define	MAX_CELL_COORDS					20000 //10000 //5000 //magic 05122011 10000 no good
#define	FILL_STACK_SIZE					1810000 //810000 //magic 21022011
...
class MissionMap {
	...
	public:
		...
		void* operator new (size_t mySize); //magic 24032012 disabled
		void operator delete (void* us);  //magic 24032012 disabled
...
//---------------------------------------------------------------------------
#define	MAX_STEPS_PER_MOVEPATH 400 //200 //magic 05122011

typedef struct _PathStep {
	//short					cell[2];							// obvious
	long					cell[2];	//magic 07032012
...
typedef struct _GlobalMapDoor {
	...
	char					direction[2];
	//short					numLinks[2];
	long					numLinks[2]; //magic 07032012
	DoorLinkPtr				links[2];
	//------------------
	// Pathfinding  data
	long					cost;
...
typedef struct _GlobalMapDoorOld {
	//----------------
	// Map layout data
	short					row;
	short					col;
...
typedef struct _GlobalMapArea {
	...
	AreaType				type;
	//short					numDoors;
	long					numDoors; //magic 07032012
	long					ownerWID;
	char					teamID;
	bool					offMap;
	bool					open;
	//short*					cellsCovered;
	long*					cellsCovered; //magic 07032012
} GlobalMapArea;
/*//magic 09032012 begin
typedef struct _GlobalMapAreaOld {
	//----------------
	// Map layout data
	short					sectorR;
	short					sectorC;
...
#define	MAX_SPECIAL_AREAS		1500 //3000 //1500 //magic 16032012 - 3000 causes stack owerflow on mission save in editor
#define	MAX_SPECIAL_SUB_AREAS	25 //25
#define	MAX_CELLS_PER_SUB_AREA	49 //49
#define	MAX_OFFMAP_AREAS		60 //60
#define	MAX_GAME_OBJECT_CELLS	64 //64
...
typedef struct _GameObjectFootPrint {
	...
	//magic 07032012 begin
	long				cellPositionRow;
	long				cellPositionCol;
	long				preNumCells;
	long				numCells;
	long				cells[MAX_GAME_OBJECT_CELLS][2];
	//magic 07032012 end
} GameObjectFootPrint;
...
typedef struct _GlobalSpecialAreaInfo {
	...
	//magic 07032012 begin
	long					numSubAreas;
	long					subAreas[MAX_SPECIAL_SUB_AREAS];
	long					numCells;
	long					cells[MAX_CELLS_PER_SUB_AREA][2];
	//magic 07032012 end
...
class GlobalMap {
	...
	public:
		...
		long						offMapAreas[MAX_OFFMAP_AREAS]; //magic 07032012
...
/*//magic 09032012 begin
void oldToNewGlobalMapDoor(GlobalMapDoor* data, GlobalMapDoorOld* dataOld)
{

	data->row = dataOld->row;
	data->col = dataOld->col;
	data->length = dataOld->length;					// in cells
	data->open = dataOld->open;
	data->teamID = dataOld->teamID;
	data->area[0] = dataOld->area[0];
	data->area[1] = dataOld->area[1];
	data->areaCost[0] = dataOld->areaCost[0];
	data->areaCost[1] = dataOld->areaCost[1];

	data->direction[0] = dataOld->direction[0];
	data->direction[1] = dataOld->direction[1];

	data->numLinks[0] = dataOld->numLinks[0];
	data->numLinks[1] = dataOld->numLinks[1];

	data->links[0] = dataOld->links[0];
	data->links[1] = dataOld->links[1];

	data->cost = dataOld->cost;
	data->parent = dataOld->parent;
	data->fromAreaIndex = dataOld->fromAreaIndex;
	data->flags = dataOld->flags;
	data->g = dataOld->g;
	data->hPrime = dataOld->hPrime;
	data->fPrime = dataOld->fPrime;
}

void oldToNewDoorInfo(DoorInfo* data, DoorInfoOld* dataOld)
{
	data->doorIndex = dataOld->doorIndex; 
	data->doorSide = dataOld->doorSide;
} 

void oldToNewGlobalMapArea(GlobalMapArea* data, GlobalMapAreaOld* dataOld)
{
	data->sectorR = dataOld->sectorR;
	data->sectorC = dataOld->sectorC;
	//
	data->doors = dataOld->doors;
	//oldToNewDoorInfo(data->doors, dataOld->doors);
	data->type = dataOld->type;
	//
	data->numDoors = dataOld->numDoors;
	data->ownerWID = dataOld->ownerWID;
	data->teamID = dataOld->teamID;
	data->offMap = dataOld->offMap;
	data->open = dataOld->open;
	//short*					cellsCovered;
	//long*					cellsCovered; //ovde mora memcpy
	memcpy(data->cellsCovered,dataOld->cellsCovered, sizeof(long));
}
//magic 09032012 end*/
};
...
typedef struct _MoveMapNode {
	...
	long				adjCells[NUM_ADJ_CELLS]; //magic 07032012
...
#define	DISTANCE_TABLE_DIM 160 //magic 07032012
...
class MoveMap {
	...
	public:
		...
		void* operator new (size_t mySize);  //magic 24032012 diasbled
		void operator delete (void* us); //magic 24032012 disabled
