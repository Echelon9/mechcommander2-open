...
void Mech3DAppearanceType::init (char * fileName)
{
	...
	if (numJumpNodes+numWeaponNodes+numSmokeNodes+numFootNodes)
	{
		...
		nodeData = new NodeData [(numJumpNodes+numWeaponNodes+numSmokeNodes+numFootNodes)]; //magic 01102011
		...
		for (i=0;i<numSmokeNodes;i++)
		{
			...
			nodeData[i].nodeId = (char *)gos_Malloc(strlen(smokeName)+1); //magic 01102011
...
//			gosASSERT(result == NO_ERR);
			//magic 04082011 begin
			if (result != NO_ERR)
				weaponType = 0;
			//magic 04082011 end
			//nodeData[i+numSmokeNodes].nodeId = (char *)AppearanceTypeList::appearanceHeap->Malloc(strlen(weaponName)+1); 
			nodeData[i+numSmokeNodes].nodeId = (char *)gos_Malloc(strlen(weaponName)+1); //magic 01102011
			gosASSERT(nodeData[i+numSmokeNodes].nodeId != NULL);
//magic 04082011 ovde treba if (nodeData[i+numSmokeNodes].nodeId)			
		...
		for (i=0;i<numJumpNodes;i++)
		{
			...
			nodeData[i+numSmokeNodes+numWeaponNodes].nodeId = (char *)gos_Malloc(strlen(jumpName)+1); //magic 01102011
		...
		for (i=0;i<numFootNodes;i++)
		{
			...
			nodeData[i+numSmokeNodes+numWeaponNodes+numJumpNodes].nodeId = (char *)gos_Malloc(strlen(footName)+1); //magic 01102011
...
void Mech3DAppearanceType::destroy (void)
{
	...
	for (i=0;i<getTotalNodes();i++)
	{
		...
		gos_Free(nodeData[i].nodeId); //magic 01102011
		nodeData[i].nodeId = NULL;
	}

	//AppearanceTypeList::appearanceHeap->Free(nodeData);
	delete [] nodeData; //magic 01102011
...
void Mech3DAppearance::init (AppearanceTypePtr tree, GameObjectPtr obj)
{
	...
	if (mechType)
	{
 		...
 		if (mechType->numWeaponNodes)
		{
			...
			nodeUsed = new long[mechType->numWeaponNodes]; //magic 01102011
			...
			nodeRecycle = new float[mechType->numWeaponNodes]; //magic 01102011
...
void Mech3DAppearance::LoadPaintSchemata (void)
{
	...
	paintSchemata = new PaintSchemata[numPaintSchemata]; //(PaintSchemataPtr)gos_Malloc(numPaintSchemata * sizeof(PaintSchemata)); //magic 01102011
		...
		if (visible)
		{
			...
			if ((objectNameId != -1) && (selected & DRAW_TEXT) && !sensorLevel )
			{
				...
				else
				{
					...
					drawTextHelp(objectName, color);//magic 12092011
				}
				if ( strlen( pilotName ) )
				{
...
long Mech3DAppearance::renderShadows (void)
{
	...
	float distanceToEye = 0.0;
		if (eye->usePerspective)
		{
			...
			if (distanceToEye >2500.0) //magic 1500 works fine magic 09112011 2500
			{
				drawShadow = false;
			}
		}
	//magic26112011 end
	//if (inView && visible)
	if (inView && visible && drawShadow) //magic 26112011
	{
		//---------------------------------------------
		// Call Multi-shape render stuff here.
...
long Mech3DAppearance::update (bool animate) 
{
	...
	MtestFile.open("data\\mech3dtest.fit", CREATE);
	//Magic 60 end*/
	...
	if (currentStateGoal != -1)
	{
		...
		if (atTransitionToNextGesture)	//This is only set when we are ready for the next gesture if these is one!
		{
			...
			if (newGestureId == -1)		//We have reached our goal.
			{
				...
				atTransitionToNextGesture = false;
				/*//Magic 58 begin				
				if ((oldStateGoal == MECH_STATE_FALLEN_FORWARD) || (oldStateGoal == MECH_STATE_FALLEN_BACKWARD))
				{
					if (oldStateGoal == MECH_STATE_FALLEN_FORWARD)
					{
						//currentGestureId = GestureFallenForward;//pravi gresku
						currentStateGoal = 1;
						//inReverse = true;
					}
					if (oldStateGoal == MECH_STATE_FALLEN_BACKWARD)
					{
						//currentGestureId = GestureFallenBackward;//pravi gresku
						currentStateGoal = 1;
						//inReverse = true;
					}
					oldStateGoal = 1;
	...
	if ((status == OBJECT_STATUS_DESTROYED) || (status == OBJECT_STATUS_DISABLED))
	{
		...
		if (currentStateGoal < 7)
		{
			...
			{
				setGestureGoal(7);
				//currentGestureId = GestureFallForward; //ne radi - probati -1
			}
			else
			{
				setGestureGoal(8);
				//currentGestureId = GestureFallBackward; //ne radi - probati -1
			}
		}
	}
	...
	if ((currentGestureId == GestureJump) && !inJump)
	{
		...
		//Magic 60 begin
		Stuff::Vector3D resultDistance;
		resultDistance.x = ownerPosition.x - targetPosition.x;
		resultDistance.y = ownerPosition.y - targetPosition.y;
		resultDistance.z = 0.0;
		float realDistance = (resultDistance.GetLength() * metersPerWorldUnit);
		if (realDistance > 8.0)
		{//Magic 60 end
	...
	//if (((currentGestureId == GestureFallenForward) || (currentGestureId == GestureFallenBackward)) && ((status != OBJECT_STATUS_DESTROYED) || (status != OBJECT_STATUS_DISABLED)))
	{
		fallen = TRUE;
		/*//Magic 58 begin				
		if ((status != OBJECT_STATUS_DESTROYED) && (status != OBJECT_STATUS_DISABLED))
		{
			if (currentGestureId == GestureFallenForward)
			{
				currentGestureId = 15;
				inReverse = true;
				oldStateGoal = MECH_STATE_FALLEN_FORWARD;
				
			}
			if (currentGestureId == GestureFallenBackward)
			{
				currentGestureId = 14;
				inReverse = true;
				oldStateGoal = MECH_STATE_FALLEN_BACKWARD;
				
			}
			//oldStateGoal = 1;
			currentStateGoal = 1; 
			transitionState++; = 0;
			//atTransitionToNextGesture = true;//M59
			currentFrame = 0;
	...
	else	//Normal Frame Increment
	{
		...
		if (animate)
		{
			...
			if ((frameInc != 0.0f) || (singleStepMode))
			{
				...
				{
					if ((currentStateGoal == -1) && (hitGestureId == -1) && (currentGestureId != GestureIdle) && (currentGestureId != GestureFallBackward) && (currentGestureId != GestureFallForward) && (currentGestureId != GestureFallenBackward) && (currentGestureId != GestureFallenForward))
						//Are we just Looping or transitioning? probati ovde
					{
						currentFrame -= mechType->getNumFrames(currentGestureId);
						//currentGestureId = GestureIdle; //Magic 59 ne valja
						//currentStateGoal = 1;//ne valja
						//oldStateGoal = 1; ne valja
					}
					else
					{
						//-------------------------------------------
...
void Mech3DAppearance::destroy (void)
{
	...
	delete [] paintSchemata; //magic 01102011
	paintSchemata = NULL;
	...
	if (InEditor)
	{
		...
		delete [] nodeUsed; //magic 01102011X
		nodeUsed = NULL;

		//AppearanceTypeList::appearanceHeap->Free(nodeRecycle);
		delete [] nodeRecycle; //magic 01102011X
		nodeRecycle = NULL;
	}
}
...
void Mech3DAppearance::copyTo (MechAppearanceData *data)
{
	...
	if (mechType->numWeaponNodes > MAX_NODE_USED) //magic 20072011
	...
	//magic 20072011 begin
	//memcpy(data->nodeUsed,nodeUsed,MAX_NODE_USED);		  
	//memcpy(data->nodeRecycle,nodeRecycle,MAX_NODE_USED); 
	//magic 28112011 begin
	//memcpy(data->nodeUsed,nodeUsed,sizeof(long)*MAX_NODE_USED);		 //magic 30052012 disabled 
	//memcpy(data->nodeRecycle,nodeRecycle,sizeof(float)*MAX_NODE_USED); //magic 30052012 disabled
	//magic 28112011 end
	//magic 20072011 end
	//magic 30052012 begin
	//memcpy(data->nodeUsed,nodeUsed,sizeof(long)*MAX_NODE_USED);		  
	//memcpy(data->nodeRecycle,nodeRecycle,sizeof(float)*MAX_NODE_USED); 

	for (int i=0; i<mechType->numWeaponNodes; i++)
	{
		data->nodeUsed[i] = nodeUsed[i];
		data->nodeRecycle[i] = nodeRecycle[i];
	}
	for (int i=mechType->numWeaponNodes; i<MAX_NODE_USED; i++)
	{
		data->nodeUsed[i] = 0;
		data->nodeRecycle[i] = 0.25f;
	}
...
//	status = data->status;
	...
	//magic 20072011 begin
	//memcpy(nodeUsed,data->nodeUsed,MAX_NODE_USED);		 //magic 30052012 disabled
	//memcpy(nodeRecycle,data->nodeRecycle,MAX_NODE_USED); //magic 30052012 disabled
	//magic 28112011 begin
	//memcpy(nodeUsed,data->nodeUsed,sizeof(long)*MAX_NODE_USED);		//puca quickload 
	//memcpy(nodeRecycle,data->nodeRecycle,sizeof(float)*MAX_NODE_USED); //puca quickload error float a ne long
	//magic 28112011 end
	//magic 20072011 end
	//magic 30052012 begin
	//memcpy(nodeUsed,data->nodeUsed,sizeof(long)*MAX_NODE_USED);		
	//memcpy(nodeRecycle,data->nodeRecycle,sizeof(float)*MAX_NODE_USED); 

	for (int i=0; i<mechType->numWeaponNodes; i++) 
	{
	/*	nodeUsed[i] = data->nodeUsed[i];
		nodeRecycle[i] = data->nodeRecycle[i];
	}
	for (int i=mechType->numWeaponNodes; i<MAX_NODE_USED; i++)
	{*/ //cause crush after quickload better to reset nodes when quickload.
		nodeUsed[i] = 0;
		nodeRecycle[i] = 0.25f;
	}
	//magic 30052012 end
...
//-----------------------------------------------------------------------------
void Mech3DAppearance::fallForward (void) //magic 06092011 new function
{
	if ((currentGestureId == GestureStand) ||
		(currentGestureId == GestureWalk) || 
		(currentGestureId == GestureReverse) || 
	...
	{
		hitGestureId = currentGestureId;
		currentGestureId = GestureFallForward;
...
//-----------------------------------------------------------------------------
void Mech3DAppearance::fallBackward (void) //magic 06092011 new function
{
	if ((currentGestureId == GestureStand) ||
		(currentGestureId == GestureWalk) || 
		(currentGestureId == GestureReverse) || 
	...
	{
		hitGestureId = currentGestureId;
		currentGestureId = GestureFallBackward;
...
void Mech3DAppearance::setObjectName (const char *objName) //magic 12092011 new function
{
	strcpy_s(objectName, objName);
	//objectName = objName;
}