...
long File::open (const char* fName, FileMode _mode, long numChild)
{
	...
	fileName = (char *)gos_Malloc(fNameLength+1); //magic 02102011
		...
		if (handle == INVALID_HANDLE_VALUE)
		{
			...
			if (!fastFile)
			{
				...
				else
				{
					...
					childList = new FilePtr[maxChildren]; //magic 02102011
...
/*void File::close (void)
{
	...
	//if ((parent == NULL) && (fileName != NULL))
	{
		...
		gos_Free(fileName); //magic 02102011
	...
	//fileName = NULL;
	//length = 0; //magic disabled sholdnt be here
	...
	if (maxChildren)
	{
		if (childList)
		{
			for (long i=0;i<(long)maxChildren;i++)
			{
				...
				{
					childList[i]->close();
					childList[i] = NULL; //magic 06032012
				}
			}
		}
		...
		if (childList)
			...
			delete [] childList; //magic 02102011
	}
...
}*/ //magic 06032012 disabled original
...
void File::close (void)
{
	...
	//if ((parent == NULL) && (fileName != NULL))
	if ((fileName) && (!parent)) //magic 06032012
	{
		//systemHeap->Free(fileName);
		gos_Free(fileName); //magic 02102011
		/*//magic 06032012 begin
...
//---------------------------------------------------------------------------
long File::read (DWORD* buffer, long length)
{
	long result = 0;
...
long File::readRAW (unsigned long * &buffer, UserHeapPtr heap)
{
	...
	if (fastFile && heap && fastFile->isLZCompressed())
	{
		...
		buffer = (unsigned long *)gos_Malloc(lzSizeNeeded); //magic 04112011
...
//---------------------------------------------------------------------------
long File::write (DWORD* buffer, long bytes)
{
	long result = 0;
