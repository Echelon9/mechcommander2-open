...
long MasterComponent::initEXCEL (char* dataLine, float baseSensorRange) {
	...
	field = strtok(NULL, ","); //magic 28112011 double must be lo load from compbas
	size = atof(field);
	...
	field = strtok(NULL, ","); //logistics icon 1 - ignore
	field = strtok(NULL, ","); //logistics icon 2 - ignore
	field = strtok(NULL, ","); //logistics icon x - ignore
	field = strtok(NULL, ","); //logistics icon y - ignore

	field = strtok(NULL, ","); // plus this to read ranges
	field = strtok(NULL, ","); // plus this to read ranges
	
	field = strtok(NULL, ","); //wepons min range
	int minWRange = 0;
	if ( field )
		minWRange = atoi(field);

	field = strtok(NULL, ","); //wepons max range
	int maxWRange = 0;
	if ( field )
		maxWRange = atoi(field);
	//magic 11022012 end
	switch (form) {
		...
		case COMPONENT_FORM_ECM:
			...
			//field = strtok(NULL, ","); //magic 11022012 disabled should not be here
		...
		case COMPONENT_FORM_WEAPON_BALLISTIC:
			...
			stats.weapon.minWRange = minWRange; //magic 11022012
			stats.weapon.maxWRange = maxWRange; //magic 11022012
			break;
		...
		case COMPONENT_FORM_BULK:
			stats.armor.armorPoints = damage; //magic 14092011
			break;
		default:
			return(-2);
	}
...
long MasterComponent::saveEXCEL (FilePtr componentFile, unsigned char masterId, float baseSensorRange) 
{
	...
	switch (form) 
	{
		...
		default:
			...
			//sprintf(piece,"%4.1f",stats.armor.armorPoints);
			sprintf(piece,"%4.1f",(float)stats.armor.armorPoints); //magic 28112011
			strcat(dataLine,piece);
			strcat(dataLine,comma);
			//magic 14092011 end
		...
		case COMPONENT_FORM_HEATSINK:
			...
			sprintf(piece,"%4.1f",(float)stats.heatsink.dissipation); //magic 28112011
			strcat(dataLine,piece);
			strcat(dataLine,comma);
			sprintf(piece,"na,na,na,na,na,na,na,na,,,,,");
			strcat(dataLine,piece);
...
//			strcat(dataLine,comma);
		...
		case COMPONENT_FORM_JUMPJET:
			...
			sprintf(piece,"%6.2f",(float)stats.jumpjet.rangeMod); //magic 28112011
			strcat(dataLine,piece);
			strcat(dataLine,comma);
			sprintf(piece,"na,na,na,na,na,na,na,na,,,,,");
			strcat(dataLine,piece);
...
long MasterComponent::loadMasterList (char* fileName, long listSize, float baseSensorRange) {
	...
	//char dataLine[512];
	char dataLine[1025]; //magic 11022012
	long lineLength;
	

	//lineLength = componentFile.readLine((MemoryPtr)dataLine, 511);
	lineLength = componentFile.readLine((MemoryPtr)dataLine, 1024); //magic 11022012
