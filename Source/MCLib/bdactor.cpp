...
void BldgAppearanceType::init (char * fileName)
{
	...
	if (result == NO_ERR)
	{
		...
		nodeData = new NodeData[MAX_WEAPON_NODES]; //magic 01102011
		...
		for (i=0;i<MAX_WEAPON_NODES;i++)
		{
			...
			nodeData[i].nodeId = (char *)gos_Malloc(strlen(weaponName)+1); //magic 01102011
...
void BldgAppearanceType::destroy (void)
{
	...
	if (nodeData)
	{
		for (long i=0;i<MAX_WEAPON_NODES;i++)
		{
...
void BldgAppearance::init (AppearanceTypePtr tree, GameObjectPtr obj)
{
	...
	if (appearType)
	{
 		...
 		if (appearType->numWeaponNodes)
		{
			...
			nodeUsed = new long[appearType->numWeaponNodes]; //magic 01102011
			gosASSERT(nodeUsed != NULL);
			//magic 09112011 begin
			for (long i=0;i<appearType->numWeaponNodes;i++)
				nodeUsed[i] = 0;
			//magic 09112011 end
			//memset(nodeUsed,0,sizeof(long) * appearType->numWeaponNodes); //magic 09112011 disabled
			
			//nodeRecycle = (float *)AppearanceTypeList::appearanceHeap->Malloc(sizeof(float) * appearType->numWeaponNodes);
			nodeRecycle = new float[appearType->numWeaponNodes]; //magic 01102011
...
void BldgAppearance::setObjStatus (long oStatus)
{
	if (status != oStatus)
	{
		if ((oStatus == OBJECT_STATUS_DESTROYED) || (oStatus == OBJECT_STATUS_DISABLED))
		{
			...
			stopActivity();
			/*//magic 09112011 begin
			markMoveMap(true,NULL);
			//magic 09112011 end ovde radi */ //magic 27082012 disabled -cause mission crush on load first in editor when there is destroyed building
		}
...
//				if (bdAnimationState != -1)
		...
		if (bldgShape)
		{
			//magic 26072011 begin
			//markMoveMap(true,NULL);//magic 26072011
			//toggleForestClump();
			//magic 26072011 end
			//-------------------------------------------------
			// Load the texture and store its handle.
...
//-----------------------------------------------------------------------------
void BldgAppearance::setObjStatus (long oStatus, bool impassableWhenDestroyed)
{
	if (status != oStatus)
	{
		if ((oStatus == OBJECT_STATUS_DESTROYED) || (oStatus == OBJECT_STATUS_DISABLED))
		{
			...
			stopActivity();
			/*//magic 09112011 begin
			markMoveMap(impassableWhenDestroyed,NULL);
			//magic 09112011 end ovde radi*/ //magic 16032012 disabled
		}
...
long BldgAppearance::render (long depthFixup)
{
	if (inView)
	{
		...
		if ( selected & DRAW_TEXT )
		{
			...
			{
				//har tmpString[255];
				//cLoadString(objectNameId, tmpString, 254); //magic 09092011 disabled

				//strcpy_s(tmpString,objectName); //magic 19032011

				//drawTextHelp(tmpString, color);
				drawTextHelp(objectName, color);//magic 09092011
			}
		}
...
long BldgAppearance::renderShadows (void)
{
	...
	float distanceToEye = 0.0;
		if (eye->usePerspective)
		{
			...
			//float object_angle = fabs(objectCenter.z) * clip_distance;
			//float extent_angle = bldgShape->GetExtentRadius() / distanceToEye;
			if (distanceToEye >2500.0) //magic 1500 works OK magic 09112011 2500
			{
				//In theory, we would return here.  Object is NOT on screen.
				drawShadow = false;
			}
	...
	if (inView && visible && !appearType->spinMe && drawShadow)
	{
		//---------------------------------------------
		// Call Multi-shape render stuff here.
		if (bldgShadowShape)
			...
			if (strnicmp(tempBldgShape->GetNodeId(i),"_PAB",4) != 0)
			{
				for (long j=0;j<tempBldgShape->GetNumVerticesInShape(i);j++)
				{
					...
					if (appearType->isForestClump) {
						...
						if (vertexPos.z >= 1.0f)
						//if (vertexPos.z <= 1.0f) //magic 16032012
							curCell->setPassable(passable);
						//magic 16032012 begin

						/*else
							//if (vertexPos.z < 0.0f)
								curCell->setPassable(true);*/
						//magic 16032012 end
					}
				}
			}
...
void BldgAppearance::setObjectName (const char *objName) //magic 09092011 new function
{
	strcpy_s(objectName, objName);
	//objectName = objName;
}

void TreeAppearance::setObjectName (const char *objName) //magic 09092011 new function
{
	strcpy_s(objectName, objName);
	//objectName = objName;
}
