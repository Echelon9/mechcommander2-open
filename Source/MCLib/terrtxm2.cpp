...
#define COLOR_MAP_HEAP_SIZE				163840000//20480000 //magic 21022011
#define COLOR_TXM_HEAP_SIZE				65536 //32768 //4096 //magic 11032012
#define COLOR_MAP_TEXTURE_SIZE			256 //magic 09032012 512 is OK back to 256 for now
...
{
	ColorMap = NULL;
	//colorMapHeap = NULL; //magic 05112011 disabled
	//colorMapRAMHeap = NULL; //magic 05112011 disabled
...
void TerrainColorMap::destroy (void)
{
	if (textures)
	{
		...
		delete [] textures; //magic 05112011
		textures = NULL;
	}
	...
	if (txmRAM)
	{
		for (unsigned long i=0;i<numTextures;i++)
		{
			...
			gos_Free(txmRAM[i].ourRAM); //magic 05112011
			//delete [] txmRAM[i].ourRAM; //magic 05112011
			txmRAM[i].ourRAM = NULL;
			}
		}
		
		//colorMapRAMHeap->Free(txmRAM);
		delete [] txmRAM; //magic 05112011
		txmRAM = NULL;
	}
	...
	if (ColorMap)
	{
		...
		gos_Free(ColorMap);
		//delete [] ColorMap; //magic 05112011
		ColorMap = NULL;
	}
	/*
 	if (colorMapHeap)
...

{
	...
	MemoryPtr ourColor = (MemoryPtr)ColorMap + (startCol + startRow) * sizeof(DWORD);
...
/*void TerrainColorMap::getColorMapData (DWORD* ourRAM, long index, long width, bool dd) //magic 06112011
{
	long numWide = width / COLOR_MAP_TEXTURE_SIZE;
	...
	DWORD* ourColor = ColorMap + (startCol + startRow) * sizeof(DWORD); //magic 06112011
		
	for (long i=0;i<COLOR_MAP_TEXTURE_SIZE;i++)
	{
		memcpy(ourRAM,ourColor,COLOR_MAP_TEXTURE_SIZE * sizeof(DWORD)); //magic 06112011 access violation writing location for dword
		ourRAM += COLOR_MAP_TEXTURE_SIZE * sizeof(DWORD);
		ourColor += width * sizeof(DWORD);
	}
}*/
...
//void saveTGAFile(DWORD* ColorMap, char * fileName, long pixelWidth)
{
...
//---------------------------------------------------------------------------
void saveBaseTGAFile(MemoryPtr ColorMap, char * fileName, long pixelWidth) //magic 24032012 NEW function
{
	TGAFileHeader colorMapInfo;
	colorMapInfo.image_id_len = 0;
	colorMapInfo.color_map = 0;
...
inline bool textureIsOKFormat (const char *fileName)
{
	...
	if (result == NO_ERR)
	{
		...
		if (((tgaHeader.image_type == UNC_TRUE) || (tgaHeader.image_type == RLE_TRUE)) &&
			...
			(tgaHeader.width == 128) ||
			(tgaHeader.width == 256) ||
			(tgaHeader.width == 512) ||//magic 17032012
			(tgaHeader.width == 1024) //magic 17032012
			))
			return true;
...
void TerrainColorMap::resetDetailTexture (const char *fileName)
{
	...
	{
		char msg[2048];
		sprintf(msg,"Texture %s is not 24bit or 32bit, 32x32, 64x64, 128x128, 256x256, 515x512 or 1024x1024 TGA File.  Not loaded!",fileName); //magic 17032012
...
void TerrainColorMap::recalcLight(char *fileName)
{
	...
	colorMapRAMHeap->init(COLOR_MAP_HEAP_SIZE,"ColorMap",true); //magic 17032011
	*/ //magic 05112011 disabled	
	...
	//DWORD* tgaFileImage = (DWORD*)malloc(colorMapFile.fileSize());
	gosASSERT(tgaFileImage != NULL);
	...
	{
		MemoryPtr loadBuffer = tgaFileImage + sizeof(TGAFileHeader);
		//DWORD* loadBuffer = tgaFileImage + sizeof(TGAFileHeader);
		...
		ColorMap = (MemoryPtr)gos_Malloc(colorMapInfo.width * colorMapInfo.width * sizeof(DWORD)); //magic 05112011
		
		//ColorMap = (MemoryPtr)new DWORD[colorMapInfo.width * colorMapInfo.width]; //magic 05112011
		//ColorMap = new DWORD[colorMapInfo.width * colorMapInfo.width]; //magic 05112011
		...
		txmRAM = new ColorMapRAM[numTextures]; //magic 05112011
		...
		for (unsigned long i=0;i<numTextures;i++)
		{
			...
			txmRAM[i].ourRAM = (MemoryPtr)gos_Malloc(sizeof(DWORD) * COLOR_MAP_TEXTURE_SIZE * COLOR_MAP_TEXTURE_SIZE); //magic 05112011
			//txmRAM[i].ourRAM = (MemoryPtr)new DWORD[COLOR_MAP_TEXTURE_SIZE * COLOR_MAP_TEXTURE_SIZE]; //magic 05112011
			//txmRAM[i].ourRAM = new DWORD[COLOR_MAP_TEXTURE_SIZE * COLOR_MAP_TEXTURE_SIZE]; //magic 05112011
			gosASSERT(txmRAM[i].ourRAM != NULL);
				
			getColorMapData(txmRAM[i].ourRAM,i, colorMapInfo.width);
			//getColorMapData(txmRAM[i].ourRAM,i, colorMapInfo.width,false); //magic 06112011
	...
	//colorMapRAMHeap->Free(ColorMap);
	//gos_Free(ColorMap); //magic 05112011
	//magic 05112011 begin
	if (ColorMap)
	{
		gos_Free(ColorMap); //magic 05112011
	//delete [] ColorMap; //magic 05112011
...
void TerrainColorMap::resetBaseTexture (char *fileName)
{
	...
	*/ //magic 05112011
	...
	MemoryPtr tgaFileImage = (MemoryPtr)malloc(colorMapFile.fileSize());
	//DWORD* tgaFileImage = (DWORD*)malloc(colorMapFile.fileSize()); //magic 06112011
	gosASSERT(tgaFileImage != NULL);
	
	colorMapFile.read(tgaFileImage,colorMapFile.fileSize());
	//colorMapFile.read((MemoryPtr)tgaFileImage,colorMapFile.fileSize());
	...
	if (colorMapInfo.image_type == UNC_TRUE)
	{
		...
		if (colorMapInfo.pixel_depth == 24)
		{
			...
			MemoryPtr cMap = ColorMap;
			MemoryPtr lMap = loadBuffer;
			//DWORD* cMap = ColorMap; //magic 06112011
			//DWORD* lMap = loadBuffer; //magic 06112011
			for (long i = 0;i<(colorMapInfo.width * colorMapInfo.width);i++)
			{
				*cMap = *lMap;		//Red
				cMap++;
		...
		textures = new ColorMapTextures[numTextures]; //magic 05112011
...
void TerrainColorMap::getScaledColorMap (MemoryPtr bfr, long dWidth)
{
	...
	if (!ColorMap)
	{
		...
		*/ //magic 05112011 disabled
		...
		{
			MemoryPtr loadBuffer = tgaFileImage + sizeof(TGAFileHeader);
			//DWORD* loadBuffer = tgaFileImage + sizeof(TGAFileHeader); //magic 06112011
			if (colorMapInfo.width != colorMapInfo.height)
				STOP(("Color Map is not a perfect Square"));
			...
			//ColorMap = (MemoryPtr)new DWORD[colorMapInfo.width * colorMapInfo.width]; //magic 05112011
			//ColorMap = new DWORD[colorMapInfo.width * colorMapInfo.width]; //magic 06112011
			gosASSERT(ColorMap != NULL);
...
long TerrainColorMap::init (char *fileName)
{
	...
	if (!colorMapStarted)
	{
		...
		colorMapHeap->init(COLOR_TXM_HEAP_SIZE,"ColorTXM",true); //magic 17032011
		*/ //magic 05112011
		/*
		colorMapRAMHeap = new UserHeap;
		//colorMapRAMHeap->init(COLOR_MAP_HEAP_SIZE,"ColorMap");
		...
		*/ //magic 05112011 disabled
		...
		if (result == NO_ERR)
		{
			...
			//ColorMap = (DWORD*)DecodeJPG(burnInJpg, jpgData, fileSize, &jpgColorMapWidth, &jpgColorMapHeight, false, NULL); //magic 06112011
			...
			for (unsigned long i=0;i<numTextures;i++)
			{
				...
				//getColorMapData(txmRAM[i].ourRAM,i, jpgColorMapWidth,false); //magic 06112011
		...
		else
		{
			...
			if (colorMapInfo.image_type == UNC_TRUE)
			{
				...
				for (unsigned long i=0;i<numTextures;i++)
				{
					...
					getColorMapData(txmRAM[i].ourRAM,i, colorMapInfo.width);
					//getColorMapData(txmRAM[i].ourRAM,i, colorMapInfo.width, false);//magic 06112011 - no good access violation writing location
	...
	}*/ //magic 02122011 disabled

	//magic 02122011 begin
	if (txmRAM)
	{
	for (unsigned long i=0;i<numTextures;i++)
	{
