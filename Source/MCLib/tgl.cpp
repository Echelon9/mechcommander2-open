...
//UserHeapPtr 			TG_Shape::tglHeap = NULL; //magic 03102011 disabled
...
TG_ShapePtr TG_TypeNode::CreateFrom (void)
{
	...
	newShape = new TG_Shape; //magic 03102011
...
TG_ShapePtr TG_TypeShape::CreateFrom (void)
{
	...
	if (numTypeVertices)
	{
		...
		newShape->listOfShadowVertices = new TG_ShadowVertex[numTypeVertices * MAX_SHADOWS]; //magic 03102011
...
void TG_TypeShape::LoadBinaryCopy (File &binFile)
{
	...
	if (numTypeVertices)
	{
		...
		listOfTypeVertices = new TG_TypeVertex[numTypeVertices]; //magic 03102011
	...
	if (numTypeTriangles)
	{
		...
		listOfTypeTriangles = new TG_TypeTriangle[numTypeTriangles]; //magic 03102011
	...
	if (numTextures)
	{
		...
		listOfTextures = new TG_TinyTexture[numTextures]; //magic 03102011
...
void TG_TypeShape::destroy (void)
{
	if (listOfTypeVertices)
		...
		delete [] listOfTypeVertices; //magic 03102011
	listOfTypeVertices = NULL;

	if (listOfTypeTriangles)
		//TG_Shape::tglHeap->Free(listOfTypeTriangles);
		delete [] listOfTypeTriangles; //magic 03102011
	listOfTypeTriangles = NULL;
...
void TG_Shape::destroy (void)
{
	...
	if (listOfShadowVertices)
		...
		delete [] listOfShadowVertices; //magic 03102011
...
long TG_TypeShape::ParseASEFile (BYTE *aseBuffer, char *fileName)
{
	...
	long *vertexNormalCount = new long[numTypeVertices]; //magic 03102011
	...
	// Face Data Next.
	//listOfTypeTriangles = (TG_TypeTrianglePtr)TG_Shape::tglHeap->Malloc(sizeof(TG_TypeTriangle) * numTypeTriangles);
	listOfTypeTriangles = new TG_TypeTriangle[numTypeTriangles]; //magic 03102011 //24102011
	gosASSERT(listOfTypeTriangles != NULL);
	...
	delete [] vertexNormalCount; //magic 03102011
	return(0);
}	
...
long TG_TypeShape::LoadTGShapeFromASE (char *fileName)
{
	...
	BYTE *aseContents = (BYTE *)gos_Malloc(aseFileSize); //magic 03102011
	...
	gos_Free(aseContents); //magic 03102011
	aseContents = NULL;
...
long TG_Shape::RenderShadows (long startFace)
{
	...
	if (eye->getCameraAltitude()> 2000.0) //magic 18022011
		return(startFace);
	//magic 18022011 end*/ //magic disabled 26112011 set in mech3d, bdactor and gvactor
