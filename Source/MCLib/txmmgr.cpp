...
void MC_TextureManager::start (void)
{
	...
	masterTextureNodes = (MC_TextureNode *)gos_Malloc(nodeRAM); //magic 02102011
	...
	masterVertexNodes = (MC_VertexArrayNode *)gos_Malloc(nodeRAM);
...
/*
	...
	textureCacheHeap->init(TEXTURE_CACHE_SIZE,"TXMCache",true); //magic 17032011
	textureCacheHeap->setMallocFatals(false);
	*/ //magic 04112011 disabled
	/*
	textureStringHeap = new UserHeap;
	//textureStringHeap->init(512000,"TXMString");
	textureStringHeap->init(1024000,"TXMString",true); //magic 17032011
	*/ //magic 04112011 disabled

	if (!textureManagerInstrumented)
	{
		StatisticFormat( "" );
		StatisticFormat( "MechCommander O Texture Manager" );
		StatisticFormat( "===============================" );
		StatisticFormat( "" );
	...
	indexArray = new WORD[MC_MAXFACES]; //magic 02102011
...
void MC_TextureManager::destroy (void)
{
	...
	if (masterTextureNodes) //magic 18032012
	{
		gos_Free(masterTextureNodes); //magic 02102011
		masterTextureNodes = NULL;
	}
	//systemHeap->Free(masterVertexNodes);
	if (masterVertexNodes) //magic 18032012
	{
		gos_Free(masterVertexNodes); //magic 02102011
		masterVertexNodes = NULL;
	}
	/*//magic 02102011 begin
	if (indexArray)
		delete [] indexArray;
	//magic 02102011 end*/ //game crush on exit
/*
...
void MC_TextureManager::flush (bool justTextures)
{
	...
	//magic 26032012 begin
	if (gosFX::LightManager::Instance)
	{
	//magic 26032012
		delete gosFX::LightManager::Instance;
		gosFX::LightManager::Instance = NULL;
	...
	//MidLevelRenderer::TGAFilePool *pool = new MidLevelRenderer::TGAFilePool("data\\tgl\\256\\"); //magic 26092011
...
void MC_TextureManager::removeTextureNode (DWORD textureNode)
{
	if (textureNode != 0xffffffff)
	{
		...
		if (masterTextureNodes[textureNode].textureData)
		{
			...
			gos_Free(masterTextureNodes[textureNode].textureData); //magic 04112011
			...
			if (masterTextureNodes[textureNode].nodeName)
			{
				...
				gos_Free(masterTextureNodes[textureNode].nodeName); //magic 04112011
...
void MC_TextureManager::removeTexture (DWORD gosHandle)
{
	...
	if ( i < MC_MAXTEXTURES && masterTextureNodes[i].numUsers == 0)
	{
		...
		if (masterTextureNodes[i].textureData)
		{
			...
			gos_Free(masterTextureNodes[i].textureData); //magic 04112011
			...
			if (masterTextureNodes[i].nodeName)
			{
				...
				gos_Free(masterTextureNodes[i].nodeName); //magic 04112011
...
DWORD MC_TextureManager::textureFromMemory (DWORD *data, gos_TextureFormat key, DWORD hints, DWORD width, DWORD bitDepth)
{
	...
	if (!lzBuffer1)
	{
		...
		lzBuffer1 = (MemoryPtr)gos_Malloc(MAX_LZ_BUFFER_SIZE); //magic 04112011
		gosASSERT(lzBuffer1 != NULL);
		
		//lzBuffer2 = (MemoryPtr)textureCacheHeap->Malloc(MAX_LZ_BUFFER_SIZE);
		lzBuffer2 = (MemoryPtr)gos_Malloc(MAX_LZ_BUFFER_SIZE); //magic 04112011
		gosASSERT(lzBuffer2 != NULL);
	}
	...
	if (!masterTextureNodes[i].textureData )
		...
		masterTextureNodes[i].textureData = (DWORD *)gos_Malloc(txmCompressSize); //magic 04112011
		//masterTextureNodes[i].textureData = (DWORD *)gos_Malloc(txmSize); //magic 13032012
	...
	else
	{
		...
		/*//magic 13032012 begin
		memcpy(masterTextureNodes[i].textureData,lzBuffer1,txmSize);
...
DWORD MC_TextureManager::loadTexture (const char *textureFullPathName, gos_TextureFormat key, DWORD hints, DWORD uniqueInstance, DWORD nFlush)
{
	...
	masterTextureNodes[i].nodeName = (char *)gos_Malloc(strlen(textureFullPathName) + 1); //magic 04112011
	...
	long result = textureFile.readRAW(masterTextureNodes[i].textureData); //,textureCacheHeap); //04112011
	if (!result)
	{
		...
		if (masterTextureNodes[i].textureData == NULL)
			...
			memcpy(masterTextureNodes[i].textureData,lzBuffer2,txmCompressSize);
			//memcpy(masterTextureNodes[i].textureData,lzBuffer1,txmSize); //magic 13032012

		masterTextureNodes[i].lzCompSize = txmCompressSize;
		//masterTextureNodes[i].lzCompSize = txmSize; //magic 13032012
	}
	else
	{
...
long MC_TextureManager::saveTexture (DWORD textureIndex, const char *textureFullPathName)
{
...

	{
...

		{
			...
			//long origSize = masterTextureNodes[textureIndex].lzCompSize; //magic 13032012
...
DWORD MC_TextureNode::get_gosTextureHandle (void)	//If texture is not in VidRAM, cache a texture out and cache this one in.
{
	...
	else
	{
		...
		if (width > 0xf0000000)
		{
			...
			//long origSize = lzCompSize; //magic 13032012
		...
		else
		{
			...
			memcpy(pTextureData.pTexture,MC_TextureManager::lzBuffer2,txmSize); //magic 13032012 disabled
			//memcpy(pTextureData.pTexture,(MemoryPtr)textureData,txmSize); //magic 13032012
...
void MC_TextureNode::destroy (void)
{
	...
	if (nodeName) //magic 18032012
	{
		gos_Free(nodeName); //magic 04112011
		nodeName = NULL;  //magic 18032012
	}
	//mcTextureManager->textureCacheHeap->Free(textureData);
	if (textureData) //magic 18032012
	{
		gos_Free(textureData); //magic 04112011
		textureData = NULL;  //magic 18032012
	}
