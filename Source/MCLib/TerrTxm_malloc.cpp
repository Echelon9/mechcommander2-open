...
long TerrainTextures::init (char *fileName, char *baseName)
{
	...
	*/ //magic 24012012 disabled
	...
	localBaseName = (char *)malloc(strlen(baseName)+1); //magic 24012012
	...
	for (long i=0;i<numTypes;i++)
	{
		...
		if (result == NO_ERR)
		{
			...
			types[i].terrainName = (char *)malloc(strlen(tmpy)+1); //magic 24012012
			...
			types[i].maskName = (char *)malloc(strlen(tmpy)+1); //magic 24012012
	...
	for (i=0;i<numTypes;i++)
	{
		if ((i && (types[i].terrainId != types[0].terrainId)) || !i)
		{
			...
			types[i].textureData = (MemoryPtr *)malloc(sizeof(MemoryPtr) * MC_MAX_MIP_LEVELS); //magic 24012012
			...
			types[i].maskData = (MemoryPtr *)malloc(sizeof(MemoryPtr) * MC_MASK_NUM * MC_MAX_MIP_LEVELS); //magic 24012012
		...
		else if (i && (types[i].terrainId == types[0].terrainId))
		{
			...
			MemoryPtr ourRAM = (MemoryPtr)malloc(16 * 16 * sizeof(DWORD));
	...
	for (i=0;i<numOverlays;i++)
	{
		...
		overlays[i].overlayName = (char *)malloc(strlen(tmpy)+1); //magic 24012012
	...
	for (i=0;i<numOverlays;i++)
	{
		...
		overlays[i].overlayData = (MemoryPtr *)malloc(sizeof(MemoryPtr) * overlays[i].numTextures * MC_MAX_MIP_LEVELS); //magic 24012012
	...
	for (i=0;i<numDetails;i++)
	{
		...
		details[i].detailName = (char *)malloc(strlen(tmpy)+1); //magic 24012012
	...
	for (i=0;i<numDetails;i++)
	{
		...
		details[i].detailData = (MemoryPtr *)malloc(sizeof(MemoryPtr) * details[i].numDetails * MC_MAX_MIP_LEVELS); //magic 24012012
...
long TerrainTextures::initTexture (long typeNum)
{
	...
	for (long j=0;j<MC_MAX_MIP_LEVELS;j++)
	{
		...
		if (InEditor || !quickLoad)
		{
			...
			MemoryPtr ourRAM = (MemoryPtr)malloc(mipSize * mipSize * sizeof(DWORD)); //magic 24012012
			gosASSERT(ourRAM != NULL);
...
void TerrainTextures::initMask (long typeNum)
{
	...
	for (long i=0;i<MC_MAX_MIP_LEVELS;i++)
	{
		...
		for (long j=0;j<MC_MASK_NUM;j++)
		{
			...
			MemoryPtr ourRAM = (MemoryPtr)malloc(mipSize * mipSize * sizeof(BYTE)); //magic 24012012
			gosASSERT(ourRAM != NULL);
...
void TerrainTextures::update (void) //magic 24012012 new function
{
		...
		if (types)
		{
			for (long i = 0;i < numTypes; i++)
			{
				...
				{
					free(types[i].terrainName);
					types[i].terrainName = NULL;
				}
				if (types[i].maskName)
				{
					free(types[i].maskName);
					types[i].maskName = NULL;
				}
				if (types[i].textureData)
				{
					free(types[i].textureData);
					types[i].textureData = NULL;
				}
				if (types[i].maskData)
				{
					free(types[i].maskData);
					types[i].maskData = NULL;
				}
			}
			delete [] types;
		...
		if (overlays)
		{
			for (long i = 0;i < numOverlays; i++)
			{
				...
				{
					free(overlays[i].overlayName);
					overlays[i].overlayName = NULL;
				}
				if (overlays[i].overlayData)
				{
					free(overlays[i].overlayData);
		...
		if (details)
		{
			for (long i=0;i<numDetails;i++)
			{
				...
				{
					free(details[i].detailName);
					details[i].detailName = NULL;
				}
				if (details[i].detailData)
				{
					free(details[i].detailData);
					details[i].detailData = NULL;
				}
			}
		}	
