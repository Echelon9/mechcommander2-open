...
//extern UserHeapPtr systemHeap;  //magic 02102011 disabled
...
*/
		...
		case TACTICAL_ORDER_REFIT:
			...
			//if (target && ((target->getObjectClass() == BATTLEMECH) && ((MoverPtr)target)->needsRefit() || (target->getResourcePoints() > 0))) //magic 05072011
			//if (target && ((target->getObjectClass() == BATTLEMECH) && ((MoverPtr)target)->needsRefit() || ((target->getResourcePoints() > 0) && (((GroundVehiclePtr)warrior->getVehicle())->getRefitPoints() < ((GroundVehiclePtr)warrior->getVehicle())->getTotalRefitPoints())))) //magic 07072011
			//if (target && ((target->getObjectClass() == BATTLEMECH) && ((MoverPtr)target)->needsRefit() || ((target->getResPoints() > 0) && (((GroundVehiclePtr)warrior->getVehicle())->getRefitPoints() < ((GroundVehiclePtr)warrior->getVehicle())->getTotalRefitPoints())))) //magic 02082011
			if (target && ((target->getObjectClass() == BATTLEMECH) && ((MoverPtr)target)->needsRefit() || ((target->getResourcePoints() > 0) && (((GroundVehiclePtr)warrior->getVehicle())->getRefitPoints() < ((GroundVehiclePtr)warrior->getVehicle())->getTotalRefitPoints())))) //magic 09092012
			//if (target && ((target->getObjectClass() == BATTLEMECH) && ((MoverPtr)target)->needsRefit() || ((target->getResourcePoints() > 0) && (((GroundVehiclePtr)warrior->getVehicle())->getRefitPoints() < ((GroundVehiclePtr)warrior->getVehicle())->getTotalRefitPoints())))) //magic 08092012
			{
				...
				if (((warrior->getVehicle()->getObjectClass() == GROUNDVEHICLE) &&
					(((GroundVehiclePtr)warrior->getVehicle())->getRefitPoints() > 0.0) &&
					(warrior->getVehicle()->refitBuddyWID == 0)) || 
					//((((GroundVehiclePtr)warrior->getVehicle())->getRefitPoints() <= 0.0)  && (target->getResourcePoints() > 0)))
					//((((GroundVehiclePtr)warrior->getVehicle())->getRefitPoints() <= 0.0)  && (target->getResPoints() > 0))) //magic 02082011
					//((((GroundVehiclePtr)warrior->getVehicle())->getRefitPoints() <= 0.0)  && (target->getResourcePoints() > 0))) //magic 02082012
					//((( ((GroundVehiclePtr)warrior->getVehicle())->getTotalRefitPoints() > ((GroundVehiclePtr)warrior->getVehicle())->getRefitPoints() ) ) && (target->getResourcePoints() > 0))) //magic 08092012
					//((target->getObjectClass() == BUILDING)) && (target->getResourcePoints() > 0)) //magic 08092012
					//(target->getResourcePoints() > 0)) //magic 08092012
					(target->getResourcePoints() > 0)) //magic09092012
				//magic 06072011 end
				{
					...
					if ((result == TACORDER_FAILURE) && (target->getObjectClass() == BUILDING))
					{
				...
				switch(stage) 
				{
					...
					/*case 3:	// do the deed
						...
						if (scenarioTime > time + Mover::refitTime)
						{
							...
							if (target->getObjectClass() == BUILDING)
							{
								float resourcePAvailable = target->getResourcePoints();
								//warrior->getVehicle()->refit(resourcePAvailable, pointsUsed);
									//magic 05072011 begin
								if (target->getResourcePoints() > 0)
								{
									float curRP, maxRP;
									curRP = warrior->getVehicle()->getRefitPoints();
									maxRP = warrior->getVehicle()->getTotalRefitPoints();
									float logisticsRP = LogisticsData::instance->getResourcePoints()/20.0f; //magic 06072011
									if (curRP < maxRP)
									{
										float difRP = maxRP - curRP;
										float numResourcePoints = target->getResourcePoints()/20.0f;
										//magic 06072011 begin
										...
										{
											warrior->getVehicle()->addRefitPoints(difRP);
											//numResourcePoints -= difRP;
											difRP *= 20.0f;
											target->addResourcePoints(-difRP);
											LogisticsData::instance->addResourcePoints(-difRP);
										}
										else
										{
											warrior->getVehicle()->addRefitPoints(numResourcePoints);
											numResourcePoints *= 20.0f;
											target->addResourcePoints(-numResourcePoints);
											LogisticsData::instance->addResourcePoints(-numResourcePoints);
										}
	
										result1 = TACORDER_SUCCESS;
									}
								}
								//stage += result;
								//if (result == 0)
							...
							result1 = ((MoverPtr)target)->refit(pointsAvailable, pointsUsed); //magic 08072011
							...
							//magic 31032011 disabled above if
							} //magic 08072011
							stage += result1;
							if (result1 == 0)
								time = scenarioTime;
					...
					case 3:	// do the deed
						...
						if (scenarioTime > time + Mover::refitTime)
						{
							...
							if (target->getObjectClass() == BUILDING)
							{
								///if (target->getResPoints() > 0) //magic 29072011
								if (target->getResourcePoints() > 20.0f) //magic 08092012
								{
									float curRP, maxRP;
									curRP = warrior->getVehicle()->getRefitPoints();
									maxRP = warrior->getVehicle()->getTotalRefitPoints();
									...
									{
										float difRP = maxRP - curRP;
										//float numResourcePoints = target->getResPoints(); //magic 29072011
										float numResourcePoints = target->getResourcePoints() / 20.0f; //magic 08092012
										//magic 06072011 begin
											if (difRP > logisticsRP)
												difRP = logisticsRP;
										//magic 06072011 end
										...
										{
											warrior->getVehicle()->addRefitPoints(difRP);
											//target->addResPoints(difRP);
											target->addResourcePoints(-difRP); //magic 08092012
											difRP *= 20.0f;
											LogisticsData::instance->addResourcePoints(-difRP);
										}
										else
										{
											warrior->getVehicle()->addRefitPoints(numResourcePoints);
											//target->addResPoints(numResourcePoints);
											target->addResourcePoints(-numResourcePoints); //magic 08092012
											numResourcePoints *= 20.0f;
											LogisticsData::instance->addResourcePoints(-numResourcePoints);
										}
										result1 = TACORDER_SUCCESS;
				...
				switch(stage) {
					...
					case 3:	// do the deed
						if (scenarioTime > time + Mover::refitTime)
						{
							...
							//magic 08072011 begin
							if (target->getFlag(OBJECT_FLAG_MECHBAY))
							//magic 08072011 end
							//float pointsAvailable = target->getRefitPoints(); 
								pointsAvailable = target->getRefitPoints(); // maogic 08072011
