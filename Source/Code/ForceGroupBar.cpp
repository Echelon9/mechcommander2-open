...
void ForceGroupBar::removeMover (Mover* mover) {
	...
	for (long i = 0; i < iconCount; i++)
		...
		//sortForceGroup(); //magic 28092012 NOT here
...
void ForceGroupBar::update( )
{
	...
	//Magic 79 promenjen sort
	//qsort( icons, iconCount, sizeof( ForceGroupIcon* ), ForceGroupIcon::sort ); //Magic 81 disabled

	/*for ( int i = 0; i < iconCount; i++ )
	{
		icons[i]->setLocationIndex( i );
	}*/ //Magic 81 disabled moved to sort function 
...
void ForceGroupBar::reInit(  ) //magic 04032012
	//new function if screenWidth is les then 1280 force group must go in 2 rows
{
	
	for ( int i = 0; i < 16; i++ )
		ForceGroupIcon::reInit( i );
	...
	else
	{
		for ( int i = 0; i < iconCount; i++ )
		{
			if ( icons[i] && icons[i]->unit->getPilot() == pPilot )
			{
				...
				aviPath.init( moviePath, pVideo, ".avi" ); //magic 26042011
...
void ForceGroupBar::formation (int formationID)
{

	int rosterFirst = iconCount - 1;
	for ( int i = 0; i < iconCount; i++ )
	{
		if (icons[i]->unit->isSelected())
		{
			int tmpIndex = icons[i]->getLocationIndex();
			if (tmpIndex < rosterFirst)
				rosterFirst = tmpIndex;
		}
	}

	Stuff::Vector3D pointPos = icons[rosterFirst]->unit->getPosition();
	float pointRot = icons[rosterFirst]->unit->getRotation();
	pointPos.z = land->getTerrainElevation(pointPos);
	float gap = 0;
	for (i = (rosterFirst+1); i < iconCount; i++)
		if (icons[i]->unit->isSelected())
		{
			Stuff::Vector3D newPosition;
			gap += 200 * 5.01f;
			//icons[i]->unit->setRotation(pointRot);
			formationID = 0;
			switch (formationID)
			{
				case 0:
					//icons[i]->unit->relativePosition(90,20,1);
						/*float newRotate = 0.0;
					...
					//icons[i]->unit->rotate(pointRot)?
					//newPosition.Add(pointPos, 100);
					newPosition.x = pointPos.x;
					newPosition.y = pointPos.y + gap;
					newPosition.z = land->getTerrainElevation(newPosition);
					icons[i]->unit->getPilot()->coreMoveTo(newPosition,TACORDER_PARAM_RUN);
					//icons[i]->unit->setPosition(newPosition);
					//icons[i]->unit->updateDynamics();
...
void ForceGroupBar::sortForceGroup ()
{
	qsort( icons, iconCount, sizeof( ForceGroupIcon* ), ForceGroupIcon::sort );
	for ( int i = 0; i < iconCount; i++ )
	{
		icons[i]->setLocationIndex( i );
		icons[i]->unit->setTeamRosterIndex(i);
	}
}

Mover* ForceGroupBar::getFirstOfSelection ()
{
	int rosterFirst = iconCount - 1;
	for ( int i = 0; i < iconCount; i++ )
	{
	...
	//Mover* pMover = icons[rosterFirst]->unit;
	return icons[rosterFirst]->unit;
}

void ForceGroupBar::rosterPlayerOrder (long* rosterOrder, int rosterCount)
//long* ForceGroupBar::rosterGroupOrder ()
{
	//long* rosterOrder;
	//int j = 0;
	for ( int i = 0; i < iconCount; i++ )
	{
		//if (icons[i]->unit->isSelected())
		//{
			//rosterOrder[j] = icons[i]->unit->getWatchID();
			rosterOrder[i] = icons[i]->unit->getWatchID();
			//j++;
		//}
	}
	//rosterCount = j;
	rosterCount = iconCount;
	//return rosterOrder;
}
//*************************************************************************************************
