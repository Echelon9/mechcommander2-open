...
aText*		ActivePilotListItem::s_sensorText = NULL;
aText*		ActivePilotListItem::s_jumpingText = NULL;

long		ActivePilotListItem::s_itemCount = 0;
aRect*		ActivePilotListItem::s_outline[9]; //magic 12042011 was 5
aObject*	ActivePilotListItem::s_icons[9]; //magic 12042011 was 8
aRect*		ActivePilotListItem::s_area = NULL;
aRect*		ActivePilotListItem::s_iconRect = NULL;		
AttributeMeter*		ActivePilotListItem::s_attributeMeters[4] = { 0,0,0,0};
...
void	ActivePilotListItem::init( FitIniFile* file )
{
	...
	//s_area = new aRect; //magic 29112011 double
	//s_rankText = new aText; //magic 29112011 double
	s_gunneryText = new aText;
	s_pilotingText = new aText;
	s_sensorText = new aText;
	s_jumpingText = new aText;
	...
	s_sensorText->init( file, "ActiveSensorText" );
	s_jumpingText->init( file, "ActiveJumpingText" );
	...
	char buffer[256];
	//for ( i = 0; i < 5; i++ )
	for ( i = 0; i < 9; i++ ) //magic 12042011 fixed num of rects in MCUI_MR_layout.fit
	{
	...
	for ( i = 0; i < 5; i++ )
	{
		...
		s_icons[i+4] = new aObject; //magic 12042011
		sprintf( buffer, "ActiveDutyBoxRankIcon%ld", i );
		s_icons[i+4]->init( file, buffer );
	}
		...
		if ( pTmp )
		{
			...
			if ( !bDone )
			{
				if ( 8 + specSkills < areaLeft.textCount )
					//areaLeft.textObjects[8+specSkills].setText(IDS_SPECIALTY + skillID); //Magic 80 disabled
					areaLeft.textObjects[8+specSkills].setText(pilot->getSkillText(skillID));
				selSkillAnim.begin(); 
			}
		}
...
void PilotPromotionArea::setPilot( LogisticsPilot* pPilot, PilotIcon* pIcon )
{
	...
	sprintf( buffer, "%ld", (long)pPilot->getSensorSkill() );
	areaLeft.textObjects[6].setText( buffer ); //novi text box

	sprintf( buffer, "%ld", (long)pPilot->getJumping() );
	areaLeft.textObjects[13].setText( buffer ); //Novi text box
