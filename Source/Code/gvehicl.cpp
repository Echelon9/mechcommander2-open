...
void GroundVehicleType::init (void)  {
	...
	mineSweeper = false;
	refitPoints = 0;
	maxRefitPoints = 0; //magic 08092012
	recoverPoints = 0;
...
/*
		...
		mFullFileName.init(objectPath, "movobj", ".fit");
		//FitIniFile objTypeFile;

		FitIniFile* objTypeFile = new FitIniFile;
		gosASSERT(objTypeFile != NULL);
		

		long result = objTypeFile->open(mFullFileName);

		if (result == NO_ERR)
		{
			result = objTypeFile->seekBlock(tmp1);
			if (result != NO_ERR)
				Fatal(objTypeNum, " GroundVehicleType.init1: can't create object ");
			
			//result = objTypeFile->readIdLong("ObjectTypeNum",objectTypeNum);
			//if (result != NO_ERR)
			//	Fatal(objTypeNum, " ObjectTypeManager.load6: can't create object ");
			
			result = objTypeFile->readIdString("CSVname",tmp1,16);
			if (result != NO_ERR)
				Fatal(objTypeNum, " GroundVehicleType.init2: can't create object ");
...
long GroundVehicleType::init (FilePtr objFile, unsigned long fileSize, const char* vehicleName) {
	...
	if (result != NO_ERR)
	{
		...
		FullPathFileName mFullFileName;
		mFullFileName.init(objectPath, vehicleName, ".fit");
		//FitIniFile objTypeFile;
		//MtestFile.writeBlock( mFullFileName );
		//MtestFile.close();

		long result1 = vehicleFile.open(mFullFileName);
		if (result1 != NO_ERR)
		{
			Fatal(objTypeNum, " GroundVehicleType.init1: can't open vehicle file ");
		}
	}
	else
		vehicleFile.open(objFile, fileSize);
	...
	if (result != NO_ERR)
		moveType = MOVETYPE_GROUND;
	//MtestFile.writeIdLong( "Movetype %ld", moveType );

	result = vehicleFile.readIdFloat("LOSFactor",LOSFactor);
	if (result != NO_ERR)
		LOSFactor = 1.0f;
	//MtestFile.writeIdFloat( "Los factor %ld", LOSFactor );
		
 	result = vehicleFile.readIdUChar("Chassis", chassis);
	if (result != NO_ERR)
		Fatal(result, " GroundVehicle:Init - Unable to find Chassis ");
	//MtestFile.writeIdUChar( "chassis ", chassis );
	//MtestFile.close();
	//------------------------------
	// Check Special Vehicle Info
...
long GroundVehicle::init (FitIniFile* vehicleFile) 
{
	...
	result = vehicleFile->readIdString( "Name", name, 64 );
	if ( result != NO_ERR )
		sprintf( name, "NONAME" );

	sprintf( longName, "%s %.0ft 'Vehicle", name, tonnage );
	//magic 18122101 end
	...
	for (long curLocation = 0; curLocation < NUM_GROUNDVEHICLE_LOCATIONS; curLocation++) 
	{
		...
		unsigned char tmpMaxArmor; //magic 01082011
		//result = vehicleFile->readIdUChar("MaxArmorPoints", armor[curLocation].maxArmor);
		result = vehicleFile->readIdUChar("MaxArmorPoints", tmpMaxArmor); //magic 01082011
		if (result != NO_ERR)
			return(result);

		armor[curLocation].maxArmor = tmpMaxArmor; //magic 01082011
	...
	if (canRefit) 
	{
		...
		(((GroundVehicleTypePtr)objType)->refitPoints / 2); //magic 01082011
		//((GroundVehicleTypePtr)objType)->refitPoints;
	}  //magic 08092012 disabled no more turret armor refit points
	//magic 01082011 begin
	GroundVehicleTypePtr objType = (GroundVehicleTypePtr)ObjectManager->getObjectType(typeHandle);
	if (((GroundVehicleTypePtr)objType)->resourcePoints > 0) 
	{
		//GroundVehicleTypePtr objType = (GroundVehicleTypePtr)ObjectManager->getObjectType(typeHandle);
		armor[GROUNDVEHICLE_LOCATION_TURRET].curArmor = armor[GROUNDVEHICLE_LOCATION_TURRET].maxArmor =
		(((GroundVehicleTypePtr)objType)->resourcePoints) / 20.0f;
...
//----------------------------------------------------------------------------------
long GroundVehicle::init (const char* vehicleName) 
{
	char* BodyLocationBlockString[NUM_GROUNDVEHICLE_LOCATIONS] = {
		"Front",
		"Left",
	...
	//magic open vehicle.fit file
		...
		long result = vehicleFile->open(mFullFileName);
		/*sprintf( tmp2, "result open file name: %ld", result );
		MtestFile.writeBlock( tmp2 );*/
if (result == NO_ERR)
{
	//--------------------------
...
long GroundVehicle::handleWeaponHit (WeaponShotInfoPtr shotInfo, bool addMultiplayChunk) {
	...
	if (shotInfo->masterId == -5) {
		if (!getRefitPoints())
			//disable(POWER_USED_UP);
			//magic 28032011 begin
		{
			//canRefit = false; //magic disabled 06072011
			soundSystem->playBettySample( BETTY_REPAIR_GONE ); //magic 06072011
			//armor[GROUNDVEHICLE_LOCATION_TURRET].curArmor = armor[GROUNDVEHICLE_LOCATION_TURRET].maxArmor = 5;
		}
			//magic 28032011 end
	}
...
long GroundVehicle::fireWeapon (GameObjectPtr target, float targetTime, long weaponIndex, long attackType, long aimLocation, Stuff::Vector3D* targetPoint, float &dmgDone) {
	...
	//magic 31052011 begin
	//sourceHotSpot = appearance->getWeaponNode(MECH3D_WEAPONTYPE_ANY);
	//magic 31052011 end
	...
	if (!MPlayer)
	{
		...
		if (sourceHotSpot < 2 || sourceHotSpot > 5) //magic 31052011
			return (3);
...
float maxRear, curRear, armorRear, maxTurret, curTurret, armorTurret, armorEffect, pilotEffect, result;
		...
		if (isRefit() || mineLayer ) //magic 08092012 da li odavde skloniti isRefit?
		{
...
void GroundVehicle::Save (PacketFilePtr file, long packetNum)
{
	...
	file->writePacket(packetNum,(MemoryPtr)&data,sizeof(GroundVehicleData),STORAGE_TYPE_RAW); //magic 11032012
}
...
{
	//if (canRefit) 
	if (canRefit && (getRefitPoints() > 0) ) //magic 06072011 
	{
		armor[GROUNDVEHICLE_LOCATION_TURRET].curArmor -= pointsToBurn;
		//((GroundVehicleTypePtr)getObjectType())->refitPoints -= pointsToBurn; //magic 08092012
		...
		/*//magic 08092012 begin
		if ( ((GroundVehicleTypePtr)getObjectType())->refitPoints <= 0.0f )
		{
			((GroundVehicleTypePtr)getObjectType())->refitPoints = 0.f;
			if (getCommanderId() == Commander::home->getId())
				soundSystem->playBettySample( BETTY_REPAIR_GONE );
		}
		//magic 08092012 end*/
...
bool  GroundVehicle::addRefitPoints (float pointsToAdd) //new magic 05072011
{
	float difRefitPoints;
	difRefitPoints = armor[GROUNDVEHICLE_LOCATION_TURRET].maxArmor - armor[GROUNDVEHICLE_LOCATION_TURRET].curArmor;
	//difRefitPoints = ((GroundVehicleTypePtr)getObjectType())->maxRefitPoints - ((GroundVehicleTypePtr)getObjectType())->refitPoints; //magic 08092012

	if (canRefit) 
	{
		if (difRefitPoints < pointsToAdd)
			armor[GROUNDVEHICLE_LOCATION_TURRET].curArmor += difRefitPoints;
			//((GroundVehicleTypePtr)getObjectType())->refitPoints += difRefitPoints; //magic 08092012
		else
			armor[GROUNDVEHICLE_LOCATION_TURRET].curArmor += pointsToAdd;
			//((GroundVehicleTypePtr)getObjectType())->refitPoints += pointsToAdd; //magic 08092012
...
bool  GroundVehicle::addResPoints (float numAdd) //new magic 29072011
{
		
	//if ((getResPoints() > 0) ) //magic 06072011
	//{
		armor[GROUNDVEHICLE_LOCATION_TURRET].curArmor += numAdd;
		if ( armor[GROUNDVEHICLE_LOCATION_TURRET].curArmor <= 0.0f )
		{
			armor[GROUNDVEHICLE_LOCATION_TURRET].curArmor = 0.f;
			if (getCommanderId() == Commander::home->getId())
		...
		//else
		//	if ( armor[GROUNDVEHICLE_LOCATION_TURRET].curArmor > armor[GROUNDVEHICLE_LOCATION_TURRET].maxArmor )
		//		armor[GROUNDVEHICLE_LOCATION_TURRET].curArmor = armor[GROUNDVEHICLE_LOCATION_TURRET].maxArmor;
