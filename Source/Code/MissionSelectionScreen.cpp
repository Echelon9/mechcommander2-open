...
void MissionSelectionScreen::begin()
{
	...
	if ( str && strlen( str ) )
	{
		...
		videoName.init( moviePath, str, ".bik" ); //magic 26042011 to be changed
...
int MissionSelectionScreen::handleMessage( unsigned long msg, unsigned long who )
{
	...
	{
	case MB_MSG_NEXT:
		//Magic61 begin load mechs and components from purchase file
		if (LogisticsData::instance->isLoadFromSave())
		{
			LogisticsData::instance->loadFromSave = false;
		}
		else
		{
			LogisticsData::instance->initPurVariants();
			//LogisticsData::instance->setLoadFromSave(false);
			//LogisticsData::instance->loadFromSave = false;
		}
		//Magic61 end
		status = NEXT;
