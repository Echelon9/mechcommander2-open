...
float LogisticsComponent::MAX_HEAT = 30.f;
float LogisticsComponent::MAX_DAMAGE = 25.f;
...
int LogisticsComponent::init( char* dataLine )
{
	...
	name = flavorText  = new char[strlen( pBuffer ) + 1];
	strcpy( name, pBuffer );
	//magic 10022012 end
	...
	if ( *pBuffer && (pBuffer[0] != '0') )
	{
		...
		//iconFileName = (char *)gos_Malloc(strlen( pBuffer ) + 1); //magic 26092011
	...
	if ( *pBuffer )
	{
		...
		//pictureFileName = (char *)gos_Malloc(strlen( pBuffer ) + 1); //magic 26092011
	...
	minWRange = extractInt( pLine );
	maxWRange = extractInt( pLine );
	//magic 11022012 end
	...
	//name = flavorText = (char *)gos_Malloc(strlen( nameBuffer ) + 1); //magic 26092011
	strcpy( name, nameBuffer );
	*/ //magic 10022012 disabled - dont load name from dll - loaded above
	return ID;
}
...
//long LogisticsComponent::save( FitIniFile& file, long counter )
//{
//	char tmp[256];
//	
//	sprintf( tmp, "IComponent%ld", counter );
//
//	//file.writeBlock( tmp );
//
//	file.writeIdLong( tmp, iComponent->getID() );
//
//	return 0;
//}
//void LogisticsComponent::setComponentsInventory( LogisticsComponent iComponent )
//{
