...
void WeaponShotInfo::init (GameObjectWatchID _attackerWID, long _masterId, float _damage, long _hitLocation, float _entryAngle) {
	...
	//Assert((damage >= 0.0) && (damage <= 255.0), (long)damage, " WeaponShotInfo.init: damage out of range "); //magic 22112011 disabled
...
inline bool GameObject::lineOfSight (GameObjectPtr target, float startExtRad, bool checkVisibleBits) 
{
	...
	//radius table has all 9s. Table was intended to use altitude level to increase visual range
	//dont know why, simple math would be OK
	//float radius = visualRangeTable[altLevel] * (maxVisualRange / 300.0); //OK
	float radius = (8 * (maxVisualRange / 300.0)) + (altLevel / 100.0);
	//magic 23022012 end	
...
void GameObject::destroy (void) {
	...
	//Never need to call this.  Heap destruct will get this!! //magic 18032012 not the case any more
