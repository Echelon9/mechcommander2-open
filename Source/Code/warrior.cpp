...
char *SpecialtySkillsTable[NUM_SPECIALTY_SKILLS] = {
	...
	"SRMSpecialist",
	"SmallArmsSpecialist",
	//Magic78 "SensorProfileSpecialist", //Magic obsolete as of 20.09.2010.
	...
	//Magic78 "LongJump",						//Jump Jet Specialty
...
//extern UserHeapPtr missionHeap; //magic 01102011 disabled
...
SpecialtySkillType	MechWarrior::skillTypes[NUM_SPECIALTY_SKILLS] = 
{
	...
	//Magic78 OTHER_SPECIALTY,		//	"SensorProfileSpecialist", //Magic obsolete as of 20.09.2010.
	...
	//Magic78 OTHER_SPECIALTY,		//	"LongJump",				
	...
	for (long i = 0; i < size; i++) {
		switch (list[i].type) {
			...
			case TARGET_PRIORITY_TURRET: {
				...
				for (long k = 0; k < ObjectManager->getNumTurrets(); k++) {
					GameObjectPtr turret = (GameObjectPtr)ObjectManager->getTurret(k);
...
long MechWarrior::addQueuedTacOrder (TacticalOrder tacOrder) {
	...
	//magic 12022012 begin
	//tacOrderQueue[numTacOrdersQueued]
	//magic 12022012 end
	numTacOrdersQueued++;
...
long MechWarrior::checkSkill (long skillId, float factor) {
	...
	if (successMargin >= 0)//Magic 55 && skillId != MWS_SENSORS)
	{
...
bool MechWarrior::isJumping (MoverPtr myVehicle) {

	//MoverPtr myVehicle = getVehicle();
	if (myVehicle)
		return(myVehicle->isJumping(myVehicle));
	return(false);
}
...
long MechWarrior::calcMovePath (long selectionIndex, unsigned long moveParams) {
	...
	bool jumping = isJumping(myVehicle);
			...
			else 
			{
				...
				else 
				{
					...
					if (!lineOfFire && myVehicle->getWeaponIndirectFire(weaponIndex))
					{
						...
						*/ //magic 07112011 disabled
						lineOfFire = true; //magic 07112011 Indirect fire weapons can fire if in range! 
					}
		...
		if (moveOrders.timeOfLastStep < (scenarioTime - MoveTimeOut)) {
			...
			if (curTacOrder.isMoveOrder() || curTacOrder.isWayPathOrder()) {
				if (curTacOrder.time < (scenarioTime - MoveTimeOut)) {
					radioMessage(RADIO_MOVE_BLOCKED, true);
					clearCurTacOrder();
					incNumSkillUses(COMBAT_STAT_MISSION, MWS_PILOTING);//M56
				}
			}
...
//					setMoveStateGoal(MOVESTATE_FORWARD);
		...
		if (!isYielding() && !isJumping(myVehicle) && !getMoveTwisting() && !isWaitingForPoint() && (getMovePath()->numSteps == 0)) {
			//---------------------------------------------
...
long MechWarrior::coreAttack (GameObjectPtr target, unsigned long params) {
	...
	bool run = ((params & TACORDER_PARAM_RUN) != 0); //Magic activated 24112010
	bool jump = ((params & TACORDER_PARAM_JUMP) != 0);
...
//Magic 24112010 begin
	if ((target) && (getVehicle()->distanceFrom(target->getPosition()) > getVehicle()->getMaxFireRange()))
	{
		coreScan(NULL, params);
		target = ObjectManager->getByWatchID(coreScanTargetWID);
	}
...
long MechWarrior::handleNewMover (void) {
	...
	//radioMessage(RADIO_SENSOR_CONTACT,true); //magic
	return(NO_ERR);
}
...
void MechWarrior::calcRank (void)
{
	...
	//skillAverage /= 2.0f;
	//Magic 55 begin
	float skillAverage = skillRank[MWS_GUNNERY] + skillRank[MWS_PILOTING] + skillRank[MWS_SENSORS] + skillRank[MWS_JUMPING];
	skillAverage /= 4.0f;
	//Magic 55 end
...
void MechWarrior::updateMissionSkills()
{
	...
	float gunPoints = (float)skillPoints[MWS_GUNNERY] * 0.01f;
	if (gunPoints > 3)
		gunPoints = 3;
	skillRank[MWS_GUNNERY] += gunPoints;
	...
	skillPoints[MWS_PILOTING] = (getNumSkillUse(COMBAT_STAT_MISSION, MWS_PILOTING) * 0.05f) + (getNumSkillSuccesses(COMBAT_STAT_MISSION, MWS_PILOTING) * 0.1f);
	if (skillPoints[MWS_PILOTING] > 3)
		skillPoints[MWS_PILOTING] = 3;
	...
	//Magic 55 begin
	skillPoints[MWS_SENSORS] = (getNumSkillUse(COMBAT_STAT_MISSION, MWS_SENSORS) * 0.02f) + (getNumSkillSuccesses(COMBAT_STAT_MISSION, MWS_SENSORS) * 0.1f);
	if (skillPoints[MWS_SENSORS] > 3)
		skillPoints[MWS_SENSORS] = 3;
	skillRank[MWS_SENSORS] += skillPoints[MWS_SENSORS];
	if (skillRank[MWS_SENSORS] > MaxPilotSkill)
		skillRank[MWS_SENSORS] = MaxPilotSkill;

	if (skillPoints[MWS_JUMPING] > 3)
		skillPoints[MWS_JUMPING] = 3;
	skillRank[MWS_JUMPING] += skillPoints[MWS_JUMPING];
	if (skillRank[MWS_JUMPING] > MaxPilotSkill)
		skillRank[MWS_JUMPING] = MaxPilotSkill;	
	//Magic 55 end
...
/*	//Magic Debug file begin
	...
	sprintf( tmp2, "skill rank sensors %ld", (long)skillRank[MWS_SENSORS] );
	MtestFile.writeBlock( tmp2 );
	sprintf( tmp2, "skill points sensors %ld", (long)skillPoints[MWS_SENSORS] );
	MtestFile.writeBlock( tmp2 );
	sprintf( tmp2, "skill use sensors %ld", getNumSkillUse(COMBAT_STAT_MISSION, MWS_SENSORS) );
	MtestFile.writeBlock( tmp2 );
	sprintf( tmp2, "skill success sensors %ld", getNumSkillSuccesses(COMBAT_STAT_MISSION, MWS_SENSORS) );
	MtestFile.writeBlock( tmp2 );
	sprintf( tmp2, "skill rank jumping %ld", (long)skillRank[MWS_JUMPING] );
	MtestFile.writeBlock( tmp2 );
	sprintf( tmp2, "skill points jumping %ld", (long)skillPoints[MWS_JUMPING] );
	MtestFile.writeBlock( tmp2 );
	sprintf( tmp2, "skill rank piloting %ld", (long)skillRank[MWS_PILOTING] );
	MtestFile.writeBlock( tmp2 );
	sprintf( tmp2, "skill points piloting %ld", (long)skillPoints[MWS_PILOTING] );
	MtestFile.writeBlock( tmp2 );
	sprintf( tmp2, "skill use piloting %ld", getNumSkillUse(COMBAT_STAT_MISSION, MWS_PILOTING) );
	MtestFile.writeBlock( tmp2 );
	sprintf( tmp2, "skill success piloting %ld", getNumSkillSuccesses(COMBAT_STAT_MISSION, MWS_PILOTING) );
	MtestFile.writeBlock( tmp2 );
	sprintf( tmp2, "skill rank gun %ld", (long)skillRank[MWS_GUNNERY] );
	MtestFile.writeBlock( tmp2 );
	sprintf( tmp2, "skill points gun %ld", (long)skillPoints[MWS_GUNNERY] );
	MtestFile.writeBlock( tmp2 );
...
long MechWarrior::calcTacOrder (long goalAction,
	...
	if (currentAction == GOAL_ACTION_MOVE) {
		...
		GlobalPathStep globalPath[MAX_GLOBAL_PATH]; //magic 06032012
		long numSteps = 0;
		//MessageBox(NULL,"numSteps = 0","1",0);
		if (currentTarget)
...
void MechWarrior::save (PacketFilePtr file, long packetNum) 
{
	...
	file->writePacket(packetNum,(MemoryPtr)&warriorData,sizeof(warriorData),STORAGE_TYPE_RAW); //magic 11032012
}
...
const char* MechWarrior::getSpecSkillText( int skillID )
{
	gosASSERT( skillID <= NUM_SKILLS );
	
	if ( SpecialtySkillsTable[skillID] )
	{
		return SpecialtySkillsTable[skillID];
	}
...
//---------------------------------------------------------------------------
/*long MechWarrior::magicAttack (GameObjectPtr target, unsigned long params)
{

		GameObjectPtr targetID = target;
		//WorldPosition		attackPoint;
		float				range;
		float				angle;
		float				RandomAngle;
//		int					curTacCode;
//		static int[15]		tacOrderParams;
//		float				time;
		int					targetClass;
		float				MinWeaponRange;
		float				weaponRanges[3];
//		int					weaponList[30];
		int					numFunctionalWeapons;
		int					targetObjectStatus;
//		int					result;
		int					curParams;
//		int					tacticState;
//		float				game_angle;
		int		 			param_tactic = params;
		static int	 		random_num;
//		int[32]				enemyList;
		int					numTargets;
		bool				CoreRun, CoreJump;
		MoverPtr			myVehicle = getVehicle();
		GameObjectPtr		WhoIM = ObjectManager->get(-1);//getObject(-1);
		Stuff::Vector3D		newPos;
	
		curParams = 0;
		
						random_num = RandomNumber(7);
						switch (random_num)
						{
							case 0:
								CoreRun = true;
		...
		if (CoreRun)
		{
			curParams = TACORDER_PARAM_RUN;
			CoreRun = false;
		}
		if (CoreJump)
		{
			curParams = TACORDER_PARAM_JUMP;
			CoreJump = false;
		}

		//numFunctionalWeapons = getWeapons(weaponList, 1); //magic zamena sa ovim dole
		numFunctionalWeapons = myVehicle->numFunctionalWeapons;
		if (numFunctionalWeapons == 0)
			//clearTacOrder;
			return(1);
		

		//getWeaponRanges(-1, weaponRanges);//magic zamena sa ovim dole

		weaponRanges[0] = myVehicle->getMinFireRange();
		weaponRanges[1] = myVehicle->getOptimalFireRange();
		weaponRanges[2] = myVehicle->getMaxFireRange();


		numTargets = SensorManager->getTeamSensor(myVehicle->getTeamId())->numContacts;
		//myVehicle->getTeamId()

//			numTargets = getContacts(enemyList, CONTACT_CRITERIA_VISUAL_OR_SENSOR + CONTACT_CRITERIA_NOT_DISABLED, 300); //CONTACT_CRITERIA_ENEMY 

			//magic 23112010 disabled if (getenemycount(-1) > 50) //OK radi ali treba pronaci funkciju koja broji friendly_in_area od 300 i ubaciti umesto 16 (*2)
		if (param_tactic == 0)//************
		{
			if (numTargets > 10) //OK
				param_tactic = TACORDER_PARAM_TACTIC_TURRET;//TACORDER_PARAM_HIT_AND_RUN;
			else
			{
				if (weaponRanges[2] > 180.0)
				{
						random_num = RandomNumber(6);
						switch (random_num)
						{
							case 0:
								param_tactic = TACORDER_PARAM_TACTIC_FLANK_RIGHT;
							break;
							case 1:
								param_tactic = TACORDER_PARAM_TACTIC_FLANK_LEFT;
							break;
							case 2:
								param_tactic = TACORDER_PARAM_TACTIC_FLANK_REAR;
							break;
							case 3:
								param_tactic = TACORDER_PARAM_RANGE_OPTIMAL;
							break;
							case 4:
								param_tactic = TACORDER_PARAM_RANGE_OPTIMAL;
							break;
				...
				{
					if (weaponRanges[2] > 110.0)
					{
						random_num = RandomNumber(5);
						switch (random_num)
						{
							case 0:
								param_tactic = TACORDER_PARAM_TACTIC_FLANK_RIGHT;
							...
							break;
							case 3:
								param_tactic = TACORDER_PARAM_TACTIC_JOUST;
							break;
							case 4:
								param_tactic = TACORDER_PARAM_RANGE_OPTIMAL;
							break;
						...
						param_tactic = TACORDER_PARAM_OBLITERATE;
				}
			}
		}	//*****
		...
		if (!targetID)
		{
			//targetID = getLastScan; //magic23112010 zamenjeno sa dole
			GameObjectPtr lastScanTarget = ObjectManager->getByWatchID(getCoreScanTargetWID());
			if (lastScanTarget)
				targetID = lastScanTarget;//->getPartId();
			
			if (targetID)
				//magic disabled 23112010 if (distancetoobject(-1,targetID) > 500)// OR (distancetoobject(-1,targetID) > (weaponRanges[2]))) then
				if (myVehicle->distanceFrom(targetID->getPosition()) > 500)
				{
					coreScan(0, CONTACT_CRITERIA_VISUAL_OR_SENSOR + CONTACT_CRITERIA_ENEMY + CONTACT_CRITERIA_NOT_DISABLED);
					targetID = ObjectManager->getByWatchID(coreScanTargetWID);
					if (!targetID)
						return 1;
				}
			else
			{
				//magic 23112010 disabled targetID = newScan(0, CONTACT_CRITERIA_VISUAL_OR_SENSOR + CONTACT_CRITERIA_ENEMY + CONTACT_CRITERIA_NOT_DISABLED);
				coreScan(0, CONTACT_CRITERIA_VISUAL_OR_SENSOR + CONTACT_CRITERIA_ENEMY + CONTACT_CRITERIA_NOT_DISABLED);
				targetID = ObjectManager->getByWatchID(coreScanTargetWID);
				if (!targetID)
					return 1;
		...
		//GameObjectPtr obj = getObject(targetId);
		if (targetID)
			targetObjectStatus = targetID->getStatus();//->getStatus();

		//targetObjectStatus = objectStatus(targetID);
		if ((targetObjectStatus == OBJECT_STATUS_DISABLED) ||
		    (targetObjectStatus == OBJECT_STATUS_DESTROYED)) //||
			//(targetObjectStatus == OBJECT_STATUS_WITHDRAWN))
			return(1);


		targetClass = targetID->getObjectClass();

		//targetClass = objectClass(targetID);		  
		if ((targetClass != BATTLEMECH) && (targetClass != GROUNDVEHICLE) && (targetClass != ELEMENTAL))
		{
			setCurrentTarget(targetID);
			//SetTarget(-1,TargetID);

			if (myVehicle->distanceFrom(targetID->getPosition()) > (weaponRanges[2]))
			//if (distancetoobject(-1,targetID) > (weaponRanges[2]))
				coreMoveToObject(targetID, curParams);
			else
				coreAttack(targetID, TACORDER_PARAM_PURSUE + curParams);

			return(1);
		}

//*************************NEW
//if (getenemycount(-1) > 1)
		if (numTargets > 1)
			//targetID = CheckTargetList(targetID,weaponRanges[2]);
			targetID = findClosestTarget(targetID,weaponRanges[2]);
...
//		case TACORDER_PARAM_PURSUE:
//				SetTarget(-1,TargetID);
//				
//				if (range < weaponRanges[2] - 15.0) then
//					coreAttack(targetID, TACORDER_PARAM_PURSUE + curparams);
//				else
//					coreMoveToObject(targetID, TACORDER_PARAM_RUN);
//
//				endif;
//			endcase;

//		case TACORDER_PARAM_WAIT:
//			endcase;

		case TACORDER_PARAM_OBLITERATE: //SWARM

				//SetTarget(-1,TargetID);
				setCurrentTarget(targetID);

				if (WhoIM->getObjectClass() == ELEMENTAL)

					if (myVehicle->distanceFrom(targetID->getPosition()) > 25.0)  // 1.0
					{
						//getRelativePositionToObject(TargetID, RandomAngle, 9.0, 2, AttackPoint);
						RandomAngle = (float)RandomNumber(180);
						random_num = RandomNumber(1);
						if (random_num == 0)
							RandomAngle *= -1;
						newPos = myVehicle->relativePosition(RandomAngle, 9.0, 2);
						curParams = curParams + TACORDER_PARAM_DONT_KEEP_MOVING;
						coreMoveTo(newPos, curParams);

					}

				else

					if (myVehicle->distanceFrom(targetID->getPosition()) > 2.0) // 1.0
					{
						//getRelativePositionToObject(TargetID, RandomAngle, 15.0, 2, AttackPoint);					
						RandomAngle = (float)RandomNumber(180);
						random_num = RandomNumber(1);
						if (random_num == 0)
							RandomAngle *= -1;
						newPos = myVehicle->relativePosition(RandomAngle, 15.0, 2);
						curParams = curParams + TACORDER_PARAM_DONT_KEEP_MOVING;
						coreMoveTo(newPos, curParams);
					}
...
//			break;
		...
		case TACORDER_PARAM_RANGE_OPTIMAL:
				...
				//if (weaponRanges[2] > 180.0) then
				//	MinWeaponRange = 100.0;
				//else
				//	if (weaponRanges[2] > 110.0) then
				//		MinWeaponRange = 60.0;
				//	else
				//		MinWeaponRange = 0.0;
				//	endif;
				//endif;
				range = myVehicle->distanceFrom(targetID->getPosition());
				if (range < weaponRanges[2] - 15.0)
					coreAttack(targetID, TACORDER_PARAM_PURSUE + curParams);
				else
				{
					curParams = curParams + TACORDER_PARAM_DONT_KEEP_MOVING;
		...
		case TACORDER_PARAM_TACTIC_FLANK_RIGHT:
				...
				//getRelativePositionToObject(targetID, ANGLE_WEST, (10 + MinWeaponRange), 2, attackPoint);
				//curparams = setFlag(curparams, TACORDER_PARAM_DONT_KEEP_MOVING, true);
				//coreMoveTo(attackPoint, curparams);
				newPos = myVehicle->relativePosition(-90.0, (MinWeaponRange+10), 2);
				curParams = curParams + TACORDER_PARAM_DONT_KEEP_MOVING;
				coreMoveTo(newPos, curParams);
		...
		case TACORDER_PARAM_TACTIC_FLANK_LEFT:
				...
				//getRelativePositionToObject(targetID, ANGLE_EAST, (10 + MinWeaponRange), 2, attackPoint);
				//curparams = setFlag(curparams, TACORDER_PARAM_DONT_KEEP_MOVING, true);
				//coreMoveTo(attackPoint, curparams);
				newPos = myVehicle->relativePosition(90.0, (MinWeaponRange+10), 2);
				curParams = curParams + TACORDER_PARAM_DONT_KEEP_MOVING;
		...
		case TACORDER_PARAM_TACTIC_FLANK_REAR:
				...
				//getRelativePositionToObject(targetID, ANGLE_NORTH, (10 + MinWeaponRange), 2, attackPoint);
				//curparams = setFlag(curparams, TACORDER_PARAM_DONT_KEEP_MOVING, true);
				//coreMoveTo(attackPoint, curparams);
				newPos = myVehicle->relativePosition(180.0, (MinWeaponRange+10), 2);
				curParams = curParams + TACORDER_PARAM_DONT_KEEP_MOVING;
				coreMoveTo(newPos, curParams);
...
//			break;
		...
		case TACORDER_PARAM_TACTIC_JOUST:
				...
				//tacticState = targetID->getPartId();//?
				...
				if ((angle >= -22.5) && (angle <= 22.5))
					//getRelativePositionToObject(targetID, ANGLE_SOUTH, 50.0, 2, attackPoint);
					newPos = myVehicle->relativePosition(0.0, 50.0, 2);
				else
					if ((angle > 22.5) && (angle <= 67.5))
						//getRelativePositionToObject(targetID, ANGLE_SOUTHWEST, 50.0, 2, attackPoint);
						newPos = myVehicle->relativePosition(-45.0, 50.0, 2);
					else
						if ((angle > 67.5) && (angle <= 112.5))
							//getRelativePositionToObject(targetID, ANGLE_WEST, 50.0, 2, attackPoint);
							newPos = myVehicle->relativePosition(-90.0, 50.0, 2);
						else
							if ((angle > 112.5) && (angle < 157.5))
								//getRelativePositionToObject(targetID, ANGLE_NORTHWEST, 50.0, 2, attackPoint);
								newPos = myVehicle->relativePosition(-135.0, 50.0, 2);
							else
								if (angle > 157.5)
									//getRelativePositionToObject(targetID, ANGLE_NORTH, 50.0, 2, attackPoint);
									newPos = myVehicle->relativePosition(180.0, 50.0, 2);
								else
									if ((angle < -22.5) && (angle >= -67.5)) 
										//getRelativePositionToObject(targetID, ANGLE_SOUTHEAST, 50.0, 2, attackPoint);
										newPos = myVehicle->relativePosition(45.0, 50.0, 2);
									else
										if ((angle < -67.5) && (angle >= -112.5))
											//getRelativePositionToObject(targetID, ANGLE_EAST, 50.0, 2, attackPoint);
											newPos = myVehicle->relativePosition(90.0, 50.0, 2);
										else
											if ((angle < -112.5) && (angle >= -157.5))
												//getRelativePositionToObject(targetID, ANGLE_NORTHEAST, 50.0, 2, attackPoint);
												newPos = myVehicle->relativePosition(135.0, 50.0, 2);
											else
												if (angle < -157.5)
													//getRelativePositionToObject(targetID, ANGLE_NORTH, 50.0, 2, attackPoint);
													...
													gosASSERT( !"Bad Relative Position in Tactic." );
		...
		case TACORDER_PARAM_TACTIC_TURRET://TACORDER_PARAM_HIT_AND_RUN:
				...
				if (range > 40.0)
					...
					if ((angle >= -22.5) && (angle <= 22.5))
						//getRelativePositionToObject(-1, SOUTH, (range - 30.0), 2, AttackPoint);
						newPos = myVehicle->relativePosition(0.0, 30.0, 2);
					else
						if ((angle > 22.5) && (angle <= 67.5))
							//getRelativePositionToObject(-1, SOUTHWEST, (range - 30.0), 2, AttackPoint);						
							newPos = myVehicle->relativePosition(-45.0, 30.0, 2);
						else
							if ((angle > 67.5) && (angle <= 112.5))
								//getRelativePositionToObject(-1, WEST, (range - 30.0), 2, AttackPoint);
								newPos = myVehicle->relativePosition(-90.0, 30.0, 2);
							else
								if ((angle > 112.5) && (angle < 157.5))
									//getRelativePositionToObject(-1, NORTHWEST, (range - 30.0), 2, AttackPoint);
									newPos = myVehicle->relativePosition(-135.0, 30.0, 2);
								else
									if (angle > 157.5)
										//getRelativePositionToObject(-1, NORTH, (range - 30.0), 2, AttackPoint);
										newPos = myVehicle->relativePosition(180.0, 30.0, 2);
									else
										if ((angle < -22.5) && (angle >= -67.5)) 
											//getRelativePositionToObject(-1, SOUTHEAST, (range - 30.0), 2, AttackPoint);
											newPos = myVehicle->relativePosition(45.0, 30.0, 2);
										else
											if ((angle < -67.5) && (angle >= -112.5)) 
												//getRelativePositionToObject(-1, EAST, (range - 30.0), 2, AttackPoint);
												newPos = myVehicle->relativePosition(90.0, 30.0, 2);
											else
												if ((angle < -112.5) && (angle >= -157.5))
													//getRelativePositionToObject(-1, NORTHEAST, (range - 30.0), 2, AttackPoint);
													newPos = myVehicle->relativePosition(135.0, 30.0, 2);
												else
													if (angle < -157.5)
														//getRelativePositionToObject(-1, NORTH, (range - 30.0), 2, AttackPoint);
					...
					if ((angle >= -22.5) && (angle <= 22.5))
						//getRelativePositionToObject(-1, NORTH, (150.0 - range), 2, AttackPoint);
						newPos = myVehicle->relativePosition(180.0, (150.0 - range), 2);
					else
						if ((angle > 22.5) && (angle <= 67.5))
							//getRelativePositionToObject(-1, NORTHEAST, (150.0 - range), 2, AttackPoint);						
							newPos = myVehicle->relativePosition(135.0, (150.0 - range), 2);
						else
							if ((angle > 67.5) && (angle <= 112.5))
								//getRelativePositionToObject(-1, EAST, (150.0 - range), 2, AttackPoint);
								newPos = myVehicle->relativePosition(90.0, (150.0 - range), 2);
							else
								if ((angle > 112.5) && (angle < 157.5))
									//getRelativePositionToObject(-1, SOUTHEAST, (150.0 - range), 2, AttackPoint);
									newPos = myVehicle->relativePosition(45.0, (150.0 - range), 2);
								else
									if (angle > 157.5)
										//getRelativePositionToObject(-1, SOUTH, (150.0 - range), 2, AttackPoint);
										newPos = myVehicle->relativePosition(0.0, (150.0 - range), 2);
									else
										if ((angle < -22.5) && (angle >= -67.5))
											//getRelativePositionToObject(-1, NORTHWEST, (150.0 - range), 2, AttackPoint);
											newPos = myVehicle->relativePosition(-135.0, (150.0 - range), 2);
										else
											if ((angle < -67.5) && (angle >= -112.5))
												//getRelativePositionToObject(-1, WEST, (150.0 - range), 2, AttackPoint);
												newPos = myVehicle->relativePosition(-90.0, (150.0 - range), 2);
											else
												if ((angle < -112.5) && (angle >= -157.5))
													//getRelativePositionToObject(-1, SOUTHWEST, (150.0 - range), 2, AttackPoint);
													newPos = myVehicle->relativePosition(180.0, (-45.0 - range), 2);
												else
													if (angle < -157.5)
														//getRelativePositionToObject(-1, SOUTH, (150.0 - range), 2, AttackPoint);
														newPos = myVehicle->relativePosition(0.0, (150.0 - range), 2);
...
/*GameObjectPtr MechWarrior::findClosestTarget (GameObjectPtr target, float range)
{
	int distToCurrentTarget = getVehicle()->distanceFrom(target->getPosition());
	int scanCrit = CONTACT_CRITERIA_VISUAL_OR_SENSOR + CONTACT_CRITERIA_ENEMY + CONTACT_CRITERIA_NOT_DISABLED;
	//if (!pilot->getTeam())
	//	return(0);
	// for now, just returns the closest contact...
	if (distToCurrentTarget > range)
	{
		GameObjectPtr obj = NULL;
		long contactList[MAX_MOVERS];
		long numContacts = CurObject->getContacts(contactList, scanCrit, CONTACT_SORT_DISTANCE);
		float smallestDistance = distToCurrentTarget;//999999.0;
		for (long i = 0; i < numContacts; i++)
		{
			GameObjectPtr contact = ObjectManager->get(contactList[i]);
			if (!contact->isMarine() || (contact->getTeam() != Team::home))
			{
				float dist = getVehicle()->distanceFrom(contact->getPosition());
				if (dist < smallestDistance)
				{
					obj = contact;
					smallestDistance = dist;
