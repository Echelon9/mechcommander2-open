...
long SensorSystem::calcContactStatus (MoverPtr mover) {
	...
	if (mover->getFlag(OBJECT_FLAG_SENSOR) && !mover->isDisabled()) 
	{
		...
		bool moverInvisible = (mover->getStatus() == OBJECT_STATUS_SHUTDOWN) && !owner->hasActiveProbe(); //magic 13022012 
		//magic 13022012 detection of shutdown mechs if sensor owner has active probe
		//if (!moverInvisible && !moverNotContact)
		if (!moverInvisible && !moverNotContact && !inFriendlyECMRange(mover)) //magic 16022012
		{
		...
		{
			//Target is shutdown, can ONLY be visual cause this platform has no probe. if it has a probe solved in moverInisible.
			float startRadius = 0.0f;
			if (!owner->isMover())
				startRadius = owner->getAppearRadius();
...
bool SensorSystem::inFriendlyECMRange (MoverPtr mover) //magic 16022012 new function 
{
	if (!mover)
		return false;

	//bool inFriendlyECMRange = false; //magic 16022012
	long numMovers = ObjectManager->getNumMovers();		
	for (long i1 = 0; i1 < numMovers; i1++) 
	{
		MoverPtr mover1 = (MoverPtr)ObjectManager->getMover(i1);
		if (mover1->getExists() && (mover1->getTeamId() == mover->getTeamId()) && (mover1->getEcmRange() > 0)) 
		{
			//float distanceToMover = mover->distanceFrom(mover1->getPosition());
			//float sensorRange = getEffectiveRange();
			if (mover->distanceFrom(mover1->getPosition()) < mover1->getEcmRange())
			{
				return true;
			}
		}
...
long SensorSystem::getSensorQuality (void)
{
	...
	if (owner && owner->getPilot())
	{
		...
		//Magic 77 begin
		sSkill = owner->getPilot()->getSkill(MWS_SENSORS);
		if (sSkill >= 70)
			return(CONTACT_SENSOR_QUALITY_4);
		else if (sSkill >= 60)
			return(CONTACT_SENSOR_QUALITY_3);
		else if (sSkill >= 50)
			return(CONTACT_SENSOR_QUALITY_2);
		else
...
void SensorSystem::addContact (MoverPtr mover, bool visual, bool hasProbe) //magic 13022012 new function
{
...
void SensorSystem::modifyContact (MoverPtr mover, bool visual, bool hasProbe) {
...
void SensorSystem::updateScan (bool forceUpdate) {
	...
	if (1/*(nextScanUpdate < scenarioTime) || forceUpdate*/) 
	{
		...
		long currentScan = scanBattlefield();
		if ((currentScan) > -1)		//No returns size of largest contact.
		//magic 03082011 end
		{
			if (owner->isMover() && 
		...
		if (mover->getExists() && (mover->getTeamId() != owner->getTeamId())) 
		{
			...
			if (isContact(mover)) 
			{
				...
				else
				{
					...
					modifyContact(mover, contactStatus == CONTACT_VISUAL ? true : false, owner->hasActiveProbe()); //magic 13022012
					//modifyContact(mover, contactStatus == CONTACT_VISUAL ? true : false, hasProbe); //magic 13022012
			...
			else 
			{
				if (contactStatus != CONTACT_NONE) 
				{
					...
					addContact(mover, contactStatus == CONTACT_VISUAL ? true : false, owner->hasActiveProbe()); //magic 13022012
					//addContact(mover, contactStatus == CONTACT_VISUAL ? true : false, hasProbe); //magic 13022012
					getLargest(currentLargest,mover,contactStatus);
					numNewContacts++;
				}
			}
...
MoverPtr SensorSystem::getMoverPointer (void)
{
...
MoverPtr SensorSystem::scanNewContact (void) 
{
	//NOW returns size of largest contact!
	long currentLargest = -1;
...
/*bool SensorSystem::scanNewContact2 ()//long* myMover) 
{
	//NOW returns size of largest contact!
	long currentLargest = -1;
	...
	for (long i = 0; i < numMovers; i++) 
	{
		...
		if (mover->getExists() && (mover->getTeamId() != owner->getTeamId())) 
		{
			...
			if (isContact(mover)) 
			{
				...
				{
					removeContact(mover);
					for (int i = 0; i < master->numAllContacts; i++)
					{
						if (master->allContacts[i] == mover->getHandle())
						{
								master->allContacts[i] = master->allContacts[master->numAllContacts - 1];
								master->numAllContacts--;
						}
					}
				}
				else
			...
			else 
			{
				if (contactStatus != CONTACT_NONE) 
				{
					addContact(mover, contactStatus == CONTACT_VISUAL ? true : false);
					getLargest(currentLargest,mover,contactStatus);
					if ((contactStatus == CONTACT_SENSOR_QUALITY_1) || (contactStatus == CONTACT_SENSOR_QUALITY_2)
						|| (contactStatus == CONTACT_SENSOR_QUALITY_3) || (contactStatus == CONTACT_SENSOR_QUALITY_4))
					{
						for (int i = 0; i < master->numAllContacts; i++)
						{
							if (master->allContacts[i] == mover->getHandle())
					...
					//myMover[numNewContacts] = mover->getHandle();
					master->allContacts[master->numAllContacts] = mover->getHandle();
					master->numAllContacts++;
					numNewContacts++;
					isNewContact = true;
					//return(mover);
...
long SensorSystem::scanNewContact3 (long* myMover) 
{
	//NOW returns size of largest contact!
	long currentLargest = -1;
	...
	for (long i = 0; i < numMovers; i++) 
	{
		MoverPtr mover = (MoverPtr)ObjectManager->getMover(i);
		//if (mover->getExists() && (mover->getTeamId() != owner->getTeamId())) 
		if (mover->getExists() && (mover->getTeamId() != owner->getTeamId()) && ((mover->getObjectClass() == BATTLEMECH) || (mover->getObjectClass() == GROUNDVEHICLE))) 
		{
			long contactStatus = calcContactStatus(mover);
			if (isContact(mover)) 
			{
			...
			else 
			{
				if (contactStatus != CONTACT_NONE) 
				{
					myMover[numNewContacts] = mover->getHandle();//Magic 55
					//addContact(mover, contactStatus == CONTACT_VISUAL ? true : false);
					addContact(mover, contactStatus == CONTACT_VISUAL ? true : false, owner->hasActiveProbe()); //magic 13022012
					//addContact(mover, contactStatus == CONTACT_VISUAL ? true : false, hasProbe); //magic 13022012
					getLargest(currentLargest,mover,contactStatus);
...
long SensorSystemManager::init (bool debug) {
	...
	//sensorPool = new SensorSystemPtr[MAX_SENSORS * sizeof(SensorSystemPtr)]; //magic 30092011
	sensorPool = new SensorSystemPtr[MAX_SENSORS]; //magic 30092011
