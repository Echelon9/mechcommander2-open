...
InfoWindow::InfoWindow()
{
	...
	skillInfos = new StaticInfo[9];
	memset( skillInfos, 0, sizeof( StaticInfo ) *  9);
...
void InfoWindow::init( FitIniFile& file )
{
	...
	for ( int i = 0; i < 9; i++ ) //Magic 66 was 7
	{
	...
	memset( skillInfos, 0, sizeof( StaticInfo ) *  9); //magic 29112011
	memset( buttonData, 0, sizeof( buttonData ) );
...
void InfoWindow::drawScrollingStuff()
{
	...
	memset( names, 0, sizeof( long ) * 60 ); //magic 10022012
...
//		curY += height;
		...
		for ( i = 0; i < curComponentCount; i++ )
		{
			...
			if ( ranges[i] == j )
			{
				if ( curY > NAMETOP )
				{
					cLoadString( IDS_COMP_ABBR0 + names[i], tmpName, 255 ); //read component name from mc2res dll
...
//				color = 0xff7f7f7f;
	...
	if ( pUnit->getTeam() && !Team::home->isEnemy(pUnit->getTeam()) && pUnit->isMech() && !pUnit->isDisabled() && !pUnit->isDestroyed())
	{
		...
		int skills[4] = { MWS_GUNNERY, MWS_PILOTING, MWS_SENSORS, MWS_JUMPING }; //Magic 66 added sens & jump
		...
		for ( j = 0; j < 5; j ++ ) //Magic 66 was 3 ne ovo je za rank
		{
			gos_VERTEX v[4];
		...
		for ( i = 0; i < NUM_SPECIALTY_SKILLS; i++ )
		{
			if ( pWarrior->specialtySkills[i] )
			{
				if ( curY > NAMETOP )
				{
					//cLoadString( IDS_SPECIALTY + i, buffer, 256 );//Magic 81 disabled
					//componentFont.render( buffer, NAMELEFT, curY, NAMERIGHT - NAMELEFT, height, 0xff005392, 0, 0 ); //Magic 81 disabled
					componentFont.render( pWarrior->getSpecSkillText(i), NAMELEFT, curY, NAMERIGHT - NAMELEFT, height, 0xff005392, 0, 0 );
				}
				curY += height;
			}
		}
	//Magic 66 begin
	char tmpName[32];
	sprintf( tmpName,"mission kills: %ld",(long)pWarrior->numKilled);
	componentFont.render( tmpName, NAMELEFT, curY, NAMERIGHT - NAMELEFT, height, 0xff005392, 0, 0 );
	curY += height;
	long mechsKilled = (pWarrior->numMechKills[1][1]) + (pWarrior->numMechKills[2][1]) + (pWarrior->numMechKills[3][1]) + (pWarrior->numMechKills[4][1]);
	sprintf( tmpName,"mech kills: %ld",mechsKilled);
	componentFont.render( tmpName, NAMELEFT, curY, NAMERIGHT - NAMELEFT, height, 0xffc29b00, 0, 0 );
	curY += height;
	//Magic 66 end
	}
	else
