...
long		MechIcon::damageColors[5][3] = 
		{ 194 << 16 | 229 << 8 | 255 | 0xff << 24,	92 << 16 | 150 << 8 | 194 | 0xff << 24, 0 << 16 | 83 << 8 | 146 | 0xff << 24,
		  248 << 16 | 241 << 8 | 193 | 0xff << 24,	248 << 16 | 206 << 8 | 31 | 0xff << 24, 139 << 16 | 114 << 8 | 0 | 0xff << 24,
...
ForceGroupIcon::ForceGroupIcon(  )
{
	...
	if ( s_textureHandle[0] == -1 )
	{
		...
		//s_textureMemory = (TGAFileHeader*)(char *)gos_Malloc(size); //magic 26092011
...
void ForceGroupIcon::init( FitIniFile& file, int which )
{
	...
	*/ //magic 03032012 disabled
	//magic 04032012 begin
	textArea[which].bottom  = Environment.screenHeight - 57;
	textArea[which].top     = textArea[which].bottom - 13;
	//magic 04032012 end
	file.readIdLong( "PilotTextTop", 	pilotTextTop[which] );
	//magic 04032012 begin
	pilotTextTop[which] = Environment.screenHeight -  71;
	//magic 04022012 end
	
	//pilotTextTop[which] += ControlGui::hiResOffsetY;   //magic 03032012 disabled
	...
	*/ //magic 03032012 disabled
	//magic 04032012 begin
	bottom  = Environment.screenHeight -  3;
	top = bottom - 47;
	//magic 04032012 end
	for ( int i = 0; i < 4; i++ )
	{
	...
	*/  //magic 03032012 disabled
	//magic 04032012 begin
	bottom  = Environment.screenHeight -  4;
	top = bottom -45;
	//magic 04032012 end
	...
	*/  //magic 03032012 disabled
	//magic 04032012 begin
	pilotRect[which].bottom     = Environment.screenHeight -  3;
	pilotRect[which].top = pilotRect[which].bottom - 47;
	//magic 04032012 end
	...
	*/ //magic 03032012 disabled
	//magic 04032012 begin
	selectionRect[which].bottom = Environment.screenHeight -  2;
	selectionRect[which].top = selectionRect[which].bottom - 69;
	//magic 04032012 end
	...
	*/  //magic 03032012 disabled
	//magic 04032012 begin
	healthBar[which].bottom     = Environment.screenHeight -  52;
	healthBar[which].top = healthBar[which].bottom - 3;
	//magic 04032012 end
	if ( !jumpJetIcon )
...
void ForceGroupIcon::reInit( int which )
	//magic 04032012 new function - if screenWidth is les then 1280 force group must go in 2 rows
{
	if (which > 7)
	{
		textArea[which].left	-= 640;
		textArea[which].right	-= 640; 
	}
	else
	{
		textArea[which].top     -= 70; 
		textArea[which].bottom  -= 70; 
	}
	
	if (which < 8)
		pilotTextTop[which] -= 70;   //magic 03032012 disabled
	...
	if (which > 7)
	{
		pilotLocation[which][0] -= 640;
		pilotLocation[which][2] -= 640;
	}
	else
	{
		pilotLocation[which][1] -= 70;
		pilotLocation[which][3] -= 70;
	}

	if (which > 7)
	{
		pilotRect[which].left		-= 640;
		pilotRect[which].right      -= 640;
	}
	else
	{
		pilotRect[which].top        -= 70;
		pilotRect[which].bottom     -= 70;
	}

	if (which > 7)
	{
		selectionRect[which].left   -= 640;
		selectionRect[which].right  -= 640;
	}
	else
	{
		selectionRect[which].top    -= 70;
		selectionRect[which].bottom -= 70;
	}

	if (which > 7)
	{
		healthBar[which].left       -= 640;
		healthBar[which].right      -= 640;
	}
	else
	{
		healthBar[which].top        -= 70;
		healthBar[which].bottom     -= 70;
	}
/*
	if ( !jumpJetIcon )
		jumpJetIcon = new StaticInfo;
...
bool MechIcon::initTextures()
{
	if ( !s_MechTextures )
	{
		...
		//s_MechTextures = (TGAFileHeader*)(char *)gos_Malloc(size); //magic 26092011
...
void ForceGroupIcon::render()
{
	...
	//magic 09092012 begin
	long numKills = unit->getPilot()->numKilled;
	if (numKills)
	{
		sprintf(buffer,"%d",numKills);
		//strcpy( buffer, (char*)unit->getPilot()->numKilled );
		//gosFontHandle->render( buffer,  textArea[locationIndex].left + 50, pilotTextTop[locationIndex]+30, 0, 0, 0xffffffff, 0, 0 );
		if (numKills < 10)
			gosFontHandle->render( buffer,  textArea[locationIndex].left + 68, pilotTextTop[locationIndex], 0, 0, SD_RED, 0, 0 );
		else
			gosFontHandle->render( buffer,  textArea[locationIndex].left + 60, pilotTextTop[locationIndex], 0, 0, SD_RED, 0, 0 );
...
bool VehicleIcon::init( Mover* pMover )
{
	if ( !s_VehicleTextures )
	{
		...
		//s_VehicleTextures = (TGAFileHeader*)(char *)gos_Malloc(size); //magic 26092011
...
int ForceGroupIcon::sort( const void* p1, const void* p2 )
{
	...
	if ( bv2 && !bv1 )
		return -1;
	//else if ( bv1 && !bv2 ) Magic 79 disabled
	//else //if ( bv1 && !bv2 ) //Magic 79 disabled
	else if ( bv1 && !bv2 ) //Magic 09022011 disabled
		return 1;
//magic 090221011 begin
	int w1 = m1->unit->getTeamRosterIndex();
	int w2 = m2->unit->getTeamRosterIndex();
	if ( w1 < w2  )
		return -1;
	else //if ( w2 < w1)
		return 1;
...
PilotIcon::PilotIcon()
{
	...
	if ( !s_pilotTextureHandle )
	{
		...
		{
			strcat( path, "pilots_l2.tga" );
		}
		else if ( Environment.screenWidth == 800 )
		{
			strcat( path, "pilots_med.tga" );
		}
		else
		{
			//strcat( path, "mcui_high2.tga" );
			strcat( path, "pilots_high.tga" ); //magic 30012011
		}
