...
void LogisticsSaveDialog::beginCampaign()
{
	...
	{
		selectedName = "campaign"; //magic 25082012 disabled
		//bCampaign= false; //magic 25082012
		bDone = true;
		return;
	}
	else
...
int	LogisticsSaveDialog::handleMessage( unsigned long what, unsigned long who )
{
	...
	if ( NO == who )
	{
		selectedName = "";
		status = NO;
		//status = MAINMENU; //magic 25082012 no good 
		end();
	}
...
void LogisticsVariantDialog::setEditText(const char* str)
{
	edits[0].allowIME( false );
	edits[0].setFocus(true);
	edits[0].setEntry( str );
}
//magic 09052012 end
//-------------------------------------------------------------------------------------------------
