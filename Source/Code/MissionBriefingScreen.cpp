...
#define MAP_INDEX 0 //32 magic 19092011
#define BUTTON_TEXT 2 //34 magic 19092011
#define VIDEO_SCREEN 1 //33 magic 19092011
#define RP_INDEX 1
	...
	if ( selItem != -1 && oldSel != selItem )
	{
		...
		if ( ID != -1  )
		{
			if ( objectiveModels[ID].Length() )
			{
				...
				statics[3].showGUIWindow( 0 ); //magic 19092011
			}
			else
			{
				camera.setObject( NULL, -1 );
				//statics[35].showGUIWindow( true );
				statics[3].showGUIWindow( true );
			}
		...
		if ( selItem != -1 )
		{
			...
			if ( ID != -1 )
			{
				...
				if ( objectiveModels[ID].Length() )
					...
					statics[3].showGUIWindow( 1 ); //magic 19092011
			}
			else
			{
				camera.setObject( NULL, -1 );
...
void MissionBriefingScreen::begin()
{
	...
	//M16 begin load mechs from purchase file
	//LogisticsData::instance->initPurVariants();
	//M16 end - definitivno ne ide ovde vec u mission selection
		//Magic63 begin load mechs and components from purchase file
		if (LogisticsData::instance->isLoadSolo())
		{
			LogisticsData::instance->initPurVariants();
			//LogisticsData::instance->setLoadSolo(false);
			LogisticsData::instance->loadSoloMission = false;
		}
		//Magic63 end
	runTime = 0;
	bClicked = 0;
