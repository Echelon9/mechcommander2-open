...
SystemTrackerPtr Team::addJammer (GameObjectPtr owner, long masterId) {
	...
	SystemTrackerPtr newJammer = new SystemTracker; //magic 02102011
	if (!newJammer)
...
void Team::removeJammer (SystemTrackerPtr jammerTracker) {
	...
	delete jammerTracker; //magic 02102011
}
...
SystemTrackerPtr Team::addECM (GameObjectPtr owner, long masterId) {
	...
	SystemTrackerPtr newECM = new SystemTracker; //magic 02102011
	if (!newECM)
		Fatal(0, " Cannot allocate SystemTracker ");
...
void Team::destroy (void) {
	...
	if (ecms) {
		...
		while (curTracker) {
			...
			delete curTracker; //magic 02102011
...
void Team::reBuildRoster (long* rosterOrder, int rosterCount)
{

	rosterSize = rosterCount;
	for (int i = 0; i < rosterCount; i++)
			roster[i] = rosterOrder[i];
