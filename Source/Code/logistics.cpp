...
//int MissionBegin::logisticResolution;
extern bool aborted;
...
//	userInput->mouseOn();
	...
	switch (startMode)
	{
		case log_RESULTS: // pull out results later...
			...
			int LR = MissionBegin::logisticResolution;
			switch (LR)
			{
				case 0:
		...
		/*	if ( Environment.screenWidth > 800 ) //magic 16092011
			{
				...
				if (prefs.renderer == 3)
					gos_SetScreenMode(1280,1024,16,0,0,0,true,localFullScreen,0,localWindow,0,localRenderer);//magic 16092011 
				else
					gos_SetScreenMode(1280,1024,16,prefs.renderer,0,0,0,localFullScreen ,0,localWindow,0,localRenderer); //magic 16092011
			}
			else
			//magic 16092011 end
				if (prefs.renderer == 3)
	...
	if ( bMovie )
	{
		...
		if (result)
		{
			...
			if (strnicmp(bMovie->getMovieName(),"cinema5",7) == 0)
			{
				...
				FullPathFileName path;
				path.init( moviePath, "credits", ".bik" ); //magic 26042011 to be changed
...
int _stdcall Logistics::beginMission(void*, int, void*[])
{
		...
		if (missionLoadType == MISSION_LOAD_MP_LOGISTICS) {
			...
			for (EList< LogisticsMech*, LogisticsMech* >::EIterator iter = list.Begin(); !iter.IsDone(); iter++) {
				...
				if (mechData.numComponents)	{
					...
					long* componentList =new long[mechData.numComponents]; //magic 02102011
					...
					delete [] componentList; //magic 02102011
				}
	...
	else if (missionLoadType == MISSION_LOAD_SP_LOGISTICS) //load mechs from SP logistics
	{ 
		EList< LogisticsMech*, LogisticsMech* > list;
		LogisticsData::instance->getForceGroup( list );
		...
		for ( EList< LogisticsMech*, LogisticsMech* >::EIterator iter = list.Begin(); !iter.IsDone(); iter++) {
			...
			//Magic 56 begin
			data.sensorSkill = (*iter)->getPilot()->getSensorSkill();
			data.jumpingSkill = (*iter)->getPilot()->getJumping();
			//Magic 56 end
			...
			{		
				data.variant = 0;
				mission->addMover( &data, (*iter)); //add custom mech variant from logistics
			}
			else
			{
				data.variant = (*iter)->getVariant()->getFileID();
				//mission->addMover( &data );
				mission->addMover( &data, (*iter)); //magic 10052012
			}
...
void Logistics::playFullScreenVideo( const char* fileName )
{
	...
	FullPathFileName path;
	path.init( moviePath, fileName, ".bik" ); //magic 26042011 to be changed
