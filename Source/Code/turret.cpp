...
long TurretType::init (FilePtr objFile, unsigned long fileSize, const char* builName) {
...
void Turret::init (bool create) {
	...
	minRange = 0.0;
	//maxRange = 0.0; //magic 29112011 double
...
long Turret::setTeamId (long _teamId, bool setup) {
	...
	else
	{
		...
		//else if (_teamId > 2)						//magic 28072011 disabled
		//	teamId = 1;			//Not ally.  ENEMY!!//magic 28072011 disabled
		else
...
float Turret::calcAttackChance (GameObjectPtr target, long* range, long weaponId) 
{
	...
	float weaponMinRange = (float)MasterComponent::masterList[weaponMasterId].getMinWeaponRange();
	float weaponMaxRange = (float)MasterComponent::masterList[weaponMasterId].getMaxWeaponRange();
	//magic 11022012 end
...
Stuff::Vector3D Turret::getPositionFromHS (long nodeId) 
{
	...
	if ((nodePos == position) || (nodeId != -1))
		...
		nodePos = appearance->getWeaponNodePosition(nodeId);//magic 04022011
...
void Turret::fireWeapon (GameObjectPtr target, long weaponId) {
	...
	currentWeaponNode = weaponId;//magic 04022011
	/*//Magic Debug file begin
	char tmp2[256];
	FitIniFile MtestFile;
	MtestFile.open("data\\turrettest.fit", CREATE);
	sprintf( tmp2, "current weapon node %ld", currentWeaponNode );
...
void Turret::render (void) 
{
	if (appearance->canBeSeen())
	{
		...
		if (((TurretTypePtr)getObjectType())->turretTypeName != 0 )
		{
			char tmpString[255];
			cLoadString(((TurretTypePtr)getObjectType())->turretTypeName, tmpString, 254);
			appearance->setObjectName(tmpString);
		}
		else
			appearance->setObjectName(((TurretTypePtr)getObjectType())->buildingName);
		//magic 09092011 end
		windowsVisible = turn;
...
long Turret::calcFireRanges (void) {
	...
	for (long curWeapon = 0; curWeapon < MAX_TURRET_WEAPONS; curWeapon++) {
		...
		if (weaponMasterId != -1) {
			...
			//magic 11022012 begin
			float minWeaponRange = (float)MasterComponent::masterList[weaponMasterId].getMinWeaponRange();
			float maxWeaponRange = (float)MasterComponent::masterList[weaponMasterId].getMaxWeaponRange();
			//magic 11022012 end
...
void Turret::Save (PacketFilePtr file, long packetNum)
{
	...
	file->writePacket(packetNum,(MemoryPtr)&data,sizeof(TurretData),STORAGE_TYPE_RAW); //magic 11032012
}
...
void Turret::setName (const char *objName) //magic 09092011 new function
{
		strcpy_s(((TurretTypePtr)getObjectType())->buildingName, objName);//magic 09092011
		((TurretTypePtr)getObjectType())->turretTypeName = 0;
}
//***************************************************************************
