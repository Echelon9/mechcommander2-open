...
UserHeapPtr				ObjectTypeManager::objectTypeCache = NULL; //magic 25032012 disabled
UserHeapPtr				ObjectTypeManager::objectCache = NULL; //magic 25032012
...
void ObjectType::destroy (void) {
	...
	//if (appearName)
	//	gos_Free(appearName); //magic 25032012
	appearName = NULL;
}
...
long ObjectType::init (FilePtr objFile, unsigned long fileSize, const char* vehicleName) {
...
long ObjectType::init (FitIniFilePtr objFile) {
	...
	if (result == NO_ERR)
	{
		...
		//appearName = (char *)gos_Malloc(strlen(apprName)+1); //magic 24032012
...
//---------------------------------------------------------------------------
long ObjectType::init (const char* mechName) {
	...
	//if (result == NO_ERR)
	//{
		appearName = (char *)ObjectTypeManager::objectTypeCache->Malloc(strlen(mechName)+1);
		//appearName = (char *)gos_Malloc(strlen(mechName)+1); //magic 24032012
		strcpy(appearName,mechName);
	//}
...
//***************************************************************************************
long ObjectType::init (const char* objName, int myObjectType) {

	long result = 0;
	
	numUsers = 0;
	objectTypeClass = myObjectType;

	appearName = (char *)ObjectTypeManager::objectTypeCache->Malloc(strlen(objName)+1);
	//appearName = (char *)gos_Malloc(strlen(objName)+1); //magic 24032012
	strcpy(appearName,objName);

		//char tmp1[16];
		//sprintf( tmp1, "%ld", objName );

		FullPathFileName mFullFileName;
		mFullFileName.init(objectPath, objName, ".fit");

		FitIniFile* objTypeFile = new FitIniFile;
		gosASSERT(objTypeFile != NULL);
		
		/*sprintf( tmp2, "ful path file name: %s", mFullFileName );
		MtestFile.writeBlock( tmp2 );*/

		result = objTypeFile->open(mFullFileName);
		/*sprintf( tmp2, "result open file name: %ld", result );
		MtestFile.writeBlock( tmp2 );*/
		if (result == NO_ERR)
		{
				result = objTypeFile->readIdLong("ExplosionObject",explosionObject);
				if (result != NO_ERR)
					explosionObject = 0;

				result = objTypeFile->readIdLong("DestroyedObject",destroyedObject);
				if (result != NO_ERR)
					destroyedObject = 0;
								
				result = objTypeFile->readIdFloat("ExtentRadius",extentRadius);
				if (result != NO_ERR)
					extentRadius = 0.0f;
			...
			if (result != NO_ERR)
				Fatal(objTypeNum, " ObjectTypeManager.load5: can't create object ");
			...
			if (result != NO_ERR)
				Fatal(objTypeNum, " ObjectTypeManager.load7: can't create object ");*/

		/*sprintf( tmp2, "object type num: %ld", objectTypeNum );
		MtestFile.writeBlock( tmp2 );
...
long ObjectTypeManager::init (char* objectFileName, long objectTypeCacheSize, long objectCacheSize, long maxObjectTypes) {
	...
	//result = objectTypeCache->init(objectTypeCacheSize,"ObjectTypeHeap",true); //magic 25032012 - No mechs wont show in roster on first load
	if (result != NO_ERR)
		return(result);
	 //magic 25032012 disabled
	...
	//result = objectCache->init(objectCacheSize,"ObjectHeap",true);  //magic 25032012 - No mechs wont show in roster on first load
	if (result != NO_ERR)
		return(result);
	 //magic 25032012 disabled
	numObjectTypes = maxObjectTypes;
	table = (ObjectTypePtr*)objectTypeCache->Malloc(sizeof(ObjectTypePtr) * numObjectTypes);
	//table = new ObjectTypePtr[ numObjectTypes]; //magic 25032012
	if (!table)
...
ObjectTypePtr ObjectTypeManager::load (ObjectTypeNumber objTypeNum, bool noCacheOut, bool forceLoad) {
	...
	char tmp1[64];
	sprintf( tmp1, "%ld", objTypeNum );
	//char tmp2[256];
	//sprintf( tmp2, "data\\%ld", objTypeNum );
	//FitIniFile MtestFile;
	//MtestFile.open("data\\objtype_load.fit", CREATE);
	...
	else if (objectFile->seekPacket(objTypeNum) == NO_ERR)
	{
		...
		if (result != NO_ERR)
			Fatal(objTypeNum, " ObjectTypeManager.load1: can't create object ");

		result = objTypeFile.seekBlock("ObjectClass");
		if (result != NO_ERR)
			Fatal(objTypeNum, " ObjectTypeManager.load2: can't create object ");
				
		result = objTypeFile.readIdLong("ObjectTypeNum",objectTypeNum);
		if (result != NO_ERR)
			Fatal(objTypeNum, " ObjectTypeManager.load3: can't create object ");
	...
	{
		//Fatal(objTypeNum, " ObjectTypeManager.load4: can't create object ");

		/*sprintf( tmp2, "object type num else grana: %ld", objectTypeNum );
		MtestFile.writeBlock( tmp2 );*/

		//magic 28112010 ovde treba naci naziv csv fajla u moverobj.fit
		//char tmp1[16];
		FullPathFileName mFullFileName;
		...
		if (result == NO_ERR)
		{
			...
			result = objTypeFile->readIdString("CSVname",tmp1,63);
			if (result != NO_ERR)
				Fatal(objTypeNum, " ObjectTypeManager.load7: can't create object ");
		...
		mechFullFileName.init(objectPath, tmp2, ".csv");
		CSVFile* mechFile = new CSVFile;
		gosASSERT(mechFile != NULL);
		
		result = mechFile->open(mechFullFileName);
		if (result != NO_ERR)
			Fatal(objTypeNum, " ObjectTypeManager.load: can't open CSV file ");*/
	//-----------------------------------------------------
	...
	switch (objectTypeNum) {
		case CRAPPY_OBJECT:
			...
			Fatal(CANT_LOAD_INVALID_OBJECT, " ObjectTypeManager.load8: can't create object ");
			break;
	...
	if (noCacheOut)	{
		...
		if (objType->getExplosionObject() > 0)
			load(objType->getExplosionObject(),0);
	}
