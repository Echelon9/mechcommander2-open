...
//#include <dshow.h> //magic 18032012
...
const char* ControlGui::vehicleNames[5] = {
	...
	"minelayer",
	"pv20600",
	"mobileartillery" //magic 31032011 "guardTower"// needs to be recovery

};

long ControlGui::vehicleIDs[5] = 
{
	2067, //182, //magic 31032011 try 2067 (182) 
	1283, //393,//magic 31032011 try 1283 (393) 
	2033, //120,//magic 31032011 try 2033 (120) 
	147,
	2034 //415 //magic 31032011 was 2006 (415) 
};
...
ControlGui::ControlGui()
{
	...
	//Magic 87 begin message box by rollin
	/* by mgto */
	i_dispMsg = false;
	messageMode = 0;
	//Magic 87 end
...
void ControlGui::render( bool bPaused )
{
	if (drawGUIOn)
	{
		...
		//magic 03032012 begin

			//drawRect( rectInfos[0].rect, rectInfos[0].color );
			//drawRect( rectInfos[2].rect, rectInfos[2].color );
		...
		{ 
			renderVehicleTab();
			tacMap.render(); //magic 27022012
		}
		else
		{
			renderInfoTab();
			tacMap.render(); //magic 27022012
		}*/ //magic 28022012 disabled
		//magic 28022012 begin
		if ( getButton( TACMAP_TAB )->state & ControlButton::PRESSED )
		{
			drawRect( rectInfos[0].rect, rectInfos[0].color ); //magic 03032012
			tacMap.render();
		}
		if ( getButton( VEHICLE_TAB )->state & ControlButton::PRESSED )
		{ 
			drawRect( rectInfos[2].rect, rectInfos[2].color ); //magic 03032012
			renderVehicleTab();
			//tacMap.render(); //magic 27022012
		}
		if ( getButton( INFO_TAB )->state & ControlButton::PRESSED )
		{
			drawRect( rectInfos[1].rect, rectInfos[1].color ); //magic 03032012
			renderInfoTab();
			//tacMap.render(); //magic 27022012
		}
		//magic 28022012 end
	...
	if (drawGUIOn)
	{
		...
		if ( LogisticsData::instance->rpJustAdded )
		{
			...
			else if ( tabFlashTime >  .25 )
			{
				...
				//magic 29022012 begin rect to flash support button
				//GUI_RECT rect = {buttons[14].location[0].x + 5, buttons[14].location[0].y + 5, buttons[14].location[2].x - 25, buttons[14].location[1].y - 10};
				GUI_RECT rect = {buttons[14].location[0].x + 5, buttons[14].location[0].y + 5, buttons[14].location[2].x - 5, buttons[14].location[1].y - 5};
				//magic 29022012 end
				drawRect( rect, 0xff000000 );
			}
		}
	}
	//Magic 87 begin rolin message box
	if (drawGUIOn)
		{
...
void ControlGui::renderResults()
{
	if ( renderStatusInfo )
	{
		//magic 25072011 begin
		long unspentRP = LogisticsData::instance->getResourcePoints();
		if (unspentRP > 0)
		{
			char total[128];
			sprintf( total, "UNSPENT RESOURCE POINTS %ld ...%ld CBills", unspentRP, unspentRP );
			//LogisticsData::instance->addCBills(unspentRP);
			setMessageText( 0, total, 10);
		}
		//magic 25072011 end
...
void ControlGui::RenderObjectives()
{
	...
	if ( renderObjectives || renderStatusInfo )
	{
		...
		//magic 04032012 begin
		
		if ( getButton( INFO_TAB )->state & ControlButton::PRESSED )
		{
			OBJECTIVEHEADERLEFT = 210;
			OBJECTIVESLEFT = 230;
			objectiveInfos[objectiveInfoCount-3].location[0].x = objectiveInfos[objectiveInfoCount-3].location[1].x = 210;
			objectiveInfos[objectiveInfoCount-3].location[2].x = objectiveInfos[objectiveInfoCount-3].location[3].x = 224;

			objectiveInfos[objectiveInfoCount-2].location[0].x = objectiveInfos[objectiveInfoCount-2].location[1].x = 211;
			objectiveInfos[objectiveInfoCount-2].location[2].x = objectiveInfos[objectiveInfoCount-2].location[3].x = 223;

			objectiveInfos[objectiveInfoCount-1].location[0].x = objectiveInfos[objectiveInfoCount-1].location[1].x = 211;
			objectiveInfos[objectiveInfoCount-1].location[2].x = objectiveInfos[objectiveInfoCount-1].location[3].x = 223;
		}
		else
		{
			OBJECTIVEHEADERLEFT = 30;
			OBJECTIVESLEFT = 50; //45
			objectiveInfos[objectiveInfoCount-3].location[0].x = objectiveInfos[objectiveInfoCount-3].location[1].x = 30;
			objectiveInfos[objectiveInfoCount-3].location[2].x = objectiveInfos[objectiveInfoCount-3].location[3].x = 44;

			objectiveInfos[objectiveInfoCount-2].location[0].x = objectiveInfos[objectiveInfoCount-2].location[1].x = 31;
			objectiveInfos[objectiveInfoCount-2].location[2].x = objectiveInfos[objectiveInfoCount-2].location[3].x = 43;

			objectiveInfos[objectiveInfoCount-1].location[0].x = objectiveInfos[objectiveInfoCount-1].location[1].x = 31;
			objectiveInfos[objectiveInfoCount-1].location[2].x = objectiveInfos[objectiveInfoCount-1].location[3].x = 43;
...
void ControlGui::update( bool bPaused, bool bLOS )
{
	...
	bool bSalvage = 1;
	bool bRepair = 1;
	bool bRecharge = 1; //magic 08092012
		...
		//	if ( !isSelectingebject() )
			{
				...
				bRepair &=  ( pMover->isRefit() ? 1 : 0 ); //magic 06072011
				bRecharge &=  ( pMover->isRefit() && (pMover->getRefitPoints() < pMover->getTotalRefitPoints()) ? 1 : 0 ); //magic 08092012
	...
	if ( bMover && bMineLayer )
	{
		...
		getButton( RECHARGE )->hide( 1 ); //magic 08092012
	}	
	...
	else if ( bMover && bRepair )
	{
		...
		if (bRecharge && (LogisticsData::instance->getResourcePoints() > 20))
		{
			getButton( RECHARGE )->disable( 0 );
			//getButton( RECHARGE )->press( 0 );
		}
		else
		{
			getButton( RECHARGE )->disable( 1 );
			//getButton( RECHARGE )->press( 0 );
		}
		//magic 08092012 end 
	}
	/*//magic 06072011 begin
	else if ( bMover && bRepairNoRP )
	{
		if ( !bMouseInButton )
			setRolloverHelpText( IDS_STRING30962 ); //Truck Empty
		getButton( REPAIR )->disable( 0 );
		getButton( REPAIR )->press( 1 );
		getButton( LAYMINES )->hide( 1 );
		getButton( SALVAGE )->hide( 1 );
	...
	if ( rectInfos[0].rect.left <= userInput->getMouseX()
		...
		&&  ( getButton( TACMAP_TAB )->state & ControlButton::PRESSED ) //magic 17032012
		)
	{
	...
	//magic 27022012 begin
	bMouseInsideTacArea = 0;

	if ( infoWnd->INFOLEFT <= userInput->getMouseX()
		&& (infoWnd->INFOLEFT + infoWnd->INFOWIDTH) >= userInput->getMouseX()
		&& infoWnd->INFOTOP <= userInput->getMouseY()
		&& (infoWnd->INFOTOP + infoWnd->INFOHEIGHT) >= userInput->getMouseY() 
		&& ( getButton( INFO_TAB )->state & ControlButton::PRESSED )	//magic 17032012	
		)
	{
		bMouseInsideTacArea = true; 
	}
	//magic 27022012 end
	if ( getButton( INFO_TAB )->state & ControlButton::PRESSED ) //magic 27022012
	{
		infoWnd->update();
		if ( bMouseInsideTacArea )
		{
	...
	//else if ( bMouseInsideTacArea ) //magic 27022012
	//magic 28022012 begin
	bMouseInsideTacArea = 0;

	if ( rectInfos[2].rect.left <= userInput->getMouseX()
		&& rectInfos[2].rect.right >= userInput->getMouseX()
		&& rectInfos[2].rect.top <= userInput->getMouseY()
		&& rectInfos[2].rect.bottom >= userInput->getMouseY()
		&& ( getButton( VEHICLE_TAB )->state & ControlButton::PRESSED ) //magic 17032012
		)
	{
		bMouseInsideTacArea = true; 
	}
	...
	//magic 28022012 end
	if ( getButton( VEHICLE_TAB )->state & ControlButton::PRESSED ) //magic 28022012
	{
		updateVehicleTab( mouseX, mouseY, bLOS ); //magic 17032012
		if ( bMouseInsideTacArea ) //magic 28022012
		{
	...
	//updateVehicleTab( mouseX, mouseY, bLOS ); //magic 17032012 moved above
	//magic 08092012 begin

	if ( getButton( RECHARGE )->state & ControlButton::PRESSED ) //magic 27022012
	{
		float difRP = pSelectedMover->getTotalRefitPoints() - pSelectedMover->getRefitPoints();
		float logisticsRP = LogisticsData::instance->getResourcePoints()/20.0f;
		if (difRP > logisticsRP)
			difRP = logisticsRP;

		pSelectedMover->addRefitPoints(difRP);
		difRP *= 20.0f;
		LogisticsData::instance->addResourcePoints(-difRP);
...
bool ControlGui::inRegion( int mouseX, int mouseY, bool bPaused )
{
	...
	//magic 17032012 begin

	if ( (rectInfos[0].rect.left)  <= mouseX && 
			rectInfos[0].rect.right >= mouseX && 
			rectInfos[0].rect.top <= mouseY &&
			rectInfos[0].rect.bottom >= mouseY 
			&&  ( getButton( TACMAP_TAB )->state & ControlButton::PRESSED ) //magic 17032012			 
			)
			return true;
	if ( (rectInfos[1].rect.left)  <= mouseX && 
			rectInfos[1].rect.right >= mouseX && 
			rectInfos[1].rect.top <= mouseY &&
			rectInfos[1].rect.bottom >= mouseY 
			&&  ( getButton( INFO_TAB )->state & ControlButton::PRESSED ) //magic 17032012			 
			)
			return true;
	if ( (rectInfos[2].rect.left)  <= mouseX && 
			rectInfos[2].rect.right >= mouseX && 
			rectInfos[2].rect.top <= mouseY &&
			rectInfos[2].rect.bottom >= mouseY 
			&&  ( getButton( VEHICLE_TAB )->state & ControlButton::PRESSED ) //magic 17032012			 
			)
			return true;
	//magic 17032012 end
...
void ControlGui::addMover (MoverPtr mover)
{
	...
	forceGroupBar.sortForceGroup(); //magic 29012011
	}
}
...
void ControlGui::initMechs()
{
	...
	//Magic 81 begin
	//forceGroupBar.setIndexLocation();
	forceGroupBar.sortForceGroup();
	//Magic 81 end
}
...
void ControlGui::handleClick( int ID )
{
	...
	case VEHICLE_TAB:
		...
		break;*/ //magic 28022012 disabled
	//magic 28022012 begin
	case TACMAP_TAB:
		//getButton( ID )->press( !getButton( ID )->state );
		getButton( ID )->toggle();
		break;
	case INFO_TAB:
		//getButton( ID )->press( !getButton( ID )->state );
...
void ControlGui::renderVehicleTab()
{
	...
	for ( int i = 0; i < LAST_VEHICLE; i++ )
	{
		...
		if ( vehicleCosts[i] )
		{
			...
			vehicleFont.render( buffer, left, top, right - left, bottom - top, SX_CYAN, 0, 2 ); //magic 29022012
		}
	}

	/*long left = RPLEFT;
	long top = RPTOP;
	*/ //magic 29022012 disabled
	//magic 29022012 begin
	long top = 0;
	long left = Environment.screenWidth - 901; //691;
	if (left < 0)
	{
		left = Environment.screenWidth - 291;
		top = 70;
	}
	//magic 29022012 end
	// draw in RP
	...
	if (rpNumFlashes)
	{
		...
		else if ( rpFlashTime > .25f )
		{
			...
			vehicleFont.render( buffer, left, top, 0, 0, SX_CYAN, 0, 0 ); //magic 29022012 color change
		}
	}
	else
	{
...
void ControlGui::handleVehicleClick( int ID )
{
	...
	switch ( ID )
	{
		...
		case SENSOR_PROBE:
			...
			getButton( INFO_COMMAND )->press( 0 );
			//idToUnPress = ID; //magic 28022012 NO good
			break;
...
const char* ControlGui::getSupVehicleNameFromID (long ID) //magic 31032011
{
	for (long i = 0; i < 5; i++)
		if (vehicleIDs[i] == ID)
...
void ControlButton::initButtons( FitIniFile& buttonFile, long buttonCount, ControlButton* Buttons, ButtonData* Data, const char* str, aFont* font )
{
	...
	/*//magic 28022012 begin
		if (i == 12)
		{
			Buttons[i].location[0].y = Buttons[i].location[3].y = height; //49 size of tacmap button
			Buttons[i].location[2].x = Buttons[i].location[3].x = Environment.screenWidth - 209; //209 size of tacmap
			Buttons[i].location[0].x = Buttons[i].location[1].x = Buttons[i].location[2].x - width; //72 button width
			Buttons[i].location[1].y = Buttons[i].location[2].y = Buttons[i].location[0].y + height;
		}
		if (i == 13)
		{
			Buttons[i].location[0].y = Buttons[i].location[3].y = height * 2; //49 size of tacmap button
			Buttons[i].location[2].x = Buttons[i].location[3].x = Environment.screenWidth - 209; //209 size of tacmap
			Buttons[i].location[0].x = Buttons[i].location[1].x = Buttons[i].location[2].x - width; //72 button width
			Buttons[i].location[1].y = Buttons[i].location[2].y = Buttons[i].location[0].y + height;
		}
		if (i == 14)
		{
			Buttons[i].location[0].y = Buttons[i].location[3].y = 0; //49 size of tacmap button
			Buttons[i].location[2].x = Buttons[i].location[3].x = Environment.screenWidth - 209; //209 size of tacmap
			Buttons[i].location[0].x = Buttons[i].location[1].x = Buttons[i].location[2].x - width; //72 button width
			Buttons[i].location[1].y = Buttons[i].location[2].y = Buttons[i].location[0].y + height;
		}
	//magic 28022012 end*/ //transfered to swapResolutions its not for here
	}
...
void ControlGui::initStatics( FitIniFile& file )
{
	...
	for ( int i = 0; i < staticCount; i++ )
	{
		...
		staticInfos[i].init( file, buffer , 0, 0);
	}

	/*if ( staticCount > 20 )
		staticInfos[20].showGUIWindow( 0 );*/ //magic 29022012 disabled
	//magic 29022012 begin
	if ( staticCount > 6 )
		staticInfos[6].showGUIWindow( 0 );
	//magic 29022012 end
	...
	*/ //magic 03032012
	...
	if (Environment.screenWidth < 1280)
		HELPAREA_BOTTOM		= Environment.screenHeight - 160;
	else
		HELPAREA_BOTTOM		= Environment.screenHeight - 80;
	//magic 04032012 end
	...
	/*//magic 29022012 begin
	RPTOP = 0;
	RPLEFT = Environment.screenWidth - 891; //691;
	if (RPLEFT < 0)
	{
		RPLEFT = Environment.screenWidth - 291;;
		RPTOP = 50;
	}
	//magic 29022012 end*/
	...
	*/ //magic 05032012 disabled
	...
	videoTextRect.right += hiResOffsetX;*/ //magic 05032012 disabled
	//magic 05032012 begin
	videoTextRect.left = 333;
	videoTextRect.right = 460;
	//magic 05032012 end
...
/*
	...
	//magic 05032012 begin
	timerInfoCount = 0;
	timerRect.rect.left = Environment.screenWidth - 20;
	timerRect.rect.top = Environment.screenHeight - 100;
	//magic 05032012 end
...
void ControlGui::initRects( FitIniFile& file )
{
	...
	if ( rectCount )
	{
		...
		for ( int i = 0; i < rectCount; i++ )
		{
			...
			*/ //magic 27022012 disabled - NO rect0 only (tacmap)
		}
		//magic 27022012 begin tacmap rect
		long sizeRect = rectInfos[0].rect.right - rectInfos[0].rect.left; 
		//rectInfos[0].rect.right = Environment.screenWidth;
		//magic 03032012 begin
		rectInfos[0].rect.right = Environment.screenWidth -(buttons[12].location[2].x - buttons[12].location[0].x);
		//magic 03032012 end
		rectInfos[0].rect.left = rectInfos[0].rect.right - sizeRect;
		//magic 28022012 begin rect under vehicle buttons
		sizeRect = rectInfos[2].rect.right - rectInfos[2].rect.left; 
		//rectInfos[2].rect.right = rectInfos[0].rect.left - 75; //used for support palette background
		//rectInfos[2].rect.left = rectInfos[0].rect.left - sizeRect - 75; //75 size of support button 
		//magic 03032012 begin
		rectInfos[2].rect.right = rectInfos[0].rect.left; //used for support palette background
		rectInfos[2].rect.left = rectInfos[0].rect.left - sizeRect; //75 size of support button 
		//magic 03032012 end
		//magic 28022012 end
		//magic 27022012 end
...
void ControlGui::swapResolutions( int resolution )
{
	...
	//Magic 67 end*/
	//Magic 16022011 begin
	else if ( resolution == 1920 )
	{
		if ( Environment.screenHeight >= 1200 )
			strcpy( fileName, "buttonlayout1921.fit" );
		else
			strcpy( fileName, "buttonlayout1920.fit" );
	}
	else if ( resolution == 1680 )
			strcpy( fileName, "buttonlayout1680.fit" );
	else if ( resolution == 1440 )
			strcpy( fileName, "buttonlayout1440.fit" );
	//Magic 16022011 end
	//magic 08022012 begin
	else if ( resolution == 1366 )					
		strcpy( fileName, "buttonlayout1366.fit" );	
	//magic 08022012 end
	else 
		strcpy( fileName, "buttonlayout1024.fit" );
	...
	//infoWnd->init( buttonFile ); //magic 29022012 moved above
	...
	Assert( buttonCount == 21, buttonCount, "wrong number of control buttons" ); //magic 08092012
	...
	//magic 28022012 begin allign support, tacmap and info button to the left of tacmap 
	long buttonWidth = buttons[12].location[2].x - buttons[12].location[0].x;
	long buttonHeight = buttons[12].location[1].y - buttons[12].location[0].y;
		//if (i == 12)
		{
			buttons[12].location[0].y = buttons[12].location[3].y = buttonHeight; //47; //47 size of tacmap button
			//buttons[12].location[2].x = buttons[12].location[3].x = Environment.screenWidth - 209; //209 size of tacmap
			//magic 03032012 begin
			buttons[12].location[2].x = buttons[12].location[3].x = Environment.screenWidth; //209 size of tacmap
			//magic 03032012 end
			buttons[12].location[0].x = buttons[12].location[1].x = buttons[12].location[2].x - buttonWidth; //72; //72 button width
			buttons[12].location[1].y = buttons[12].location[2].y = buttons[12].location[0].y + buttonHeight; //47;
		}
		//if (i == 13)
		{
			buttons[13].location[0].y = buttons[13].location[3].y = buttonHeight * 2; //47 * 2; //49 size of tacmap button
			//buttons[13].location[2].x = buttons[13].location[3].x = Environment.screenWidth - 209; //209 size of tacmap
			//magic 03032012 begin
			buttons[13].location[2].x = buttons[13].location[3].x = Environment.screenWidth;
			//magic 03032012 end
			buttons[13].location[0].x = buttons[13].location[1].x = buttons[13].location[2].x - buttonWidth; //72; //72 button width
			buttons[13].location[1].y = buttons[13].location[2].y = buttons[13].location[0].y + buttonHeight; //47;
		}
		//if (i == 14)
		{
			buttons[14].location[0].y = buttons[14].location[3].y = 0; //49 size of tacmap button
			//buttons[14].location[2].x = buttons[14].location[3].x = Environment.screenWidth - 209; //209 size of tacmap
			//magic 03032012 begin
			buttons[14].location[2].x = buttons[14].location[3].x = Environment.screenWidth;
			//magic 03032012 end
			buttons[14].location[0].x = buttons[14].location[1].x = buttons[14].location[2].x - buttonWidth; //72; //72 button width
			buttons[14].location[1].y = buttons[14].location[2].y = buttons[14].location[0].y + buttonHeight; //47;
		}
	//magic 28022012 end
	//magic 29022012 begin
	//int maxRight = Environment.screenWidth - 209 - 82;
	/*for (int k = 4; k < 12; k++)
	{
			buttonWidth = buttons[k].location[2].x - buttons[k].location[0].x;
			buttonHeight = buttons[k].location[1].y - buttons[k].location[0].y;
			buttons[k].location[0].y = buttons[k].location[3].y = 210 + (k-4)*buttonHeight; //below the tacmap
			buttons[k].location[2].x = buttons[k].location[3].x = Environment.screenWidth; //align to the right screen border
			buttons[k].location[0].x = buttons[k].location[1].x = buttons[k].location[2].x - buttonWidth; //72; //72 button width
			buttons[k].location[1].y = buttons[k].location[2].y = buttons[k].location[0].y + buttonHeight; //47;
	}*/
	//magic 03032012 begin
	for (int k = 4; k < 9; k++)
	{
			buttonWidth = buttons[k].location[2].x - buttons[k].location[0].x;
			buttonHeight = buttons[k].location[1].y - buttons[k].location[0].y;
			buttons[k].location[0].y = buttons[k].location[3].y = 3*buttonHeight + (k-4)*buttonHeight; //below the tacmap
			buttons[k].location[2].x = buttons[k].location[3].x = Environment.screenWidth; //align to the right screen border
			buttons[k].location[0].x = buttons[k].location[1].x = buttons[k].location[2].x - buttonWidth; //72; //72 button width
			buttons[k].location[1].y = buttons[k].location[2].y = buttons[k].location[0].y + buttonHeight; //47;
	}
			buttons[19].location[0].y = buttons[19].location[3].y = 3*buttonHeight + 5*buttonHeight; //below the tacmap
			buttons[19].location[2].x = buttons[19].location[3].x = Environment.screenWidth; //align to the right screen border
			buttons[19].location[0].x = buttons[19].location[1].x = buttons[19].location[2].x - buttonWidth; //72; //72 button width
			buttons[19].location[1].y = buttons[19].location[2].y = buttons[19].location[0].y + buttonHeight; //47;
	for (int k = 9; k < 12; k++)
	{
			buttonWidth = buttons[k].location[2].x - buttons[k].location[0].x;
			buttonHeight = buttons[k].location[1].y - buttons[k].location[0].y;
			buttons[k].location[0].y = buttons[k].location[3].y = 3*buttonHeight + (6+(k-9))*buttonHeight; //below the tacmap
			buttons[k].location[2].x = buttons[k].location[3].x = Environment.screenWidth; //align to the right screen border
			buttons[k].location[0].x = buttons[k].location[1].x = buttons[k].location[2].x - buttonWidth; //72; //72 button width
			buttons[k].location[1].y = buttons[k].location[2].y = buttons[k].location[0].y + buttonHeight; //47;
	}
	for (int k = 15; k < 19; k++)
	{
			buttonWidth = buttons[k].location[2].x - buttons[k].location[0].x;
			buttonHeight = buttons[k].location[1].y - buttons[k].location[0].y;
			buttons[k].location[0].y = buttons[k].location[3].y = 3*buttonHeight + (9+(k - 15))*buttonHeight; //below the tacmap (8*buttonHeight)
			buttons[k].location[2].x = buttons[k].location[3].x = Environment.screenWidth; //align to the right screen border
			buttons[k].location[0].x = buttons[k].location[1].x = buttons[k].location[2].x - buttonWidth; //72; //72 button width
			buttons[k].location[1].y = buttons[k].location[2].y = buttons[k].location[0].y + buttonHeight; //47;
	}
	//magic 03032012 end
	//magic 08092012 begin
			buttons[20].location[0].y = buttons[20].location[3].y = 3*buttonHeight + 11*buttonHeight; //below the tacmap
			buttons[20].location[2].x = buttons[20].location[3].x = Environment.screenWidth; //align to the right screen border
			buttons[20].location[0].x = buttons[20].location[1].x = buttons[20].location[2].x - buttonWidth; //72; //72 button width
			buttons[20].location[1].y = buttons[20].location[2].y = buttons[20].location[0].y + buttonHeight; //47;
	//magic 08092012 end
	/*for (int k = 15; k < 20; k++)
	{
			buttonWidth = buttons[k].location[2].x - buttons[k].location[0].x;
			buttonHeight = buttons[k].location[1].y - buttons[k].location[0].y;
			buttons[k].location[0].y = buttons[k].location[3].y = 210 + (k - 15)*buttonHeight; //below the tacmap (8*buttonHeight)
			buttons[k].location[2].x = buttons[k].location[3].x = Environment.screenWidth - buttonWidth; //align to the right screen border
			buttons[k].location[0].x = buttons[k].location[1].x = buttons[k].location[2].x - buttonWidth; //72; //72 button width
			buttons[k].location[1].y = buttons[k].location[2].y = buttons[k].location[0].y + buttonHeight; //47;
	}*/
	//magic 29022012 end THEY ARE NOT IN ORDER MUST GO ONE BY ONE
	...
	//magic 28022012 begin
	int maxRight = Environment.screenWidth - 209 - 62; //82;
	buttonWidth = vehicleButtons[0].location[2].x - vehicleButtons[0].location[0].x;
	buttonHeight = vehicleButtons[0].location[1].y - vehicleButtons[0].location[0].y;
	for (int k = 7; k > -1; k--)
	{
		 vehicleButtons[k].location[0].y =  vehicleButtons[k].location[3].y = 0; //48 size of veh button
		 vehicleButtons[k].location[2].x =  vehicleButtons[k].location[3].x = maxRight - (buttonHeight * (7 - k));//(40 * (7 - k)); //209 size of tacmap
		 vehicleButtons[k].location[0].x =  vehicleButtons[k].location[1].x =  vehicleButtons[k].location[2].x - buttonHeight;//40; //40 button width
		 vehicleButtons[k].location[1].y =  vehicleButtons[k].location[2].y =  vehicleButtons[k].location[0].y + buttonWidth;//48; //48;
	}
	//magic 28022012 end

	getButton( TACMAP_TAB )->press( true );
	/*//magic 28022012 begin
	getButton( INFO_TAB )->press( true );
	getButton( VEHICLE_TAB )->press( true );
	//magic 28022012 end*/

	initStatics( buttonFile );
	//magic 04032012 begin align force group bar statics to bottom of the screen
		for (int j=0; j<4; j++)
		{
				staticInfos[j].location[1].y = staticInfos[j].location[2].y = Environment.screenHeight;
				staticInfos[j].location[0].y = staticInfos[j].location[3].y = staticInfos[j].location[1].y - 75;
		}
		staticInfos[6].location[1].y = staticInfos[6].location[2].y = Environment.screenHeight - 2;
		staticInfos[6].location[0].y = staticInfos[6].location[3].y = staticInfos[6].location[1].y - 68;
	//magic 04032012 end
	//magic 04032012 begin
	// ako je screenwidth manje od 1280 roster mora u dva reda
	//static0 i static1 umanjuje se y za 70, static 2 i 3 umanjuje se x za 640
	if (Environment.screenWidth < 1280)
	{
		for (int j=0; j<2; j++)
			for (int i=0; i<4; i++)
				staticInfos[j].location[i].y -= 70;

		for (int j=2; j<4; j++)
			for (int i=0; i<4; i++)
				staticInfos[j].location[i].x -= 640;
	}
	//magic 04032012 end
	initRects( buttonFile );

	//forceGroupBar.init( buttonFile, &staticInfos[20] );
	forceGroupBar.init( buttonFile, &staticInfos[6] ); //magic 29022012
	//magic 04032012 begin
	//ovde treba promeniti mechicon
	//sve top i bottom umanjiti za 70 za mechicon0 do mechicon7.
	//sve left i right smanjiti za 640 za mechicon 8 do mechicon15
	if (Environment.screenWidth < 1280)
		forceGroupBar.reInit();
	//magic 04032012 end
...
void ControlGui::playMovie( const char* fileName )
{
	...
	movieName.init(moviePath,realName,".avi"); //magic 26042011
	bMovie = new MC2Movie;
...
/*void ControlGui::playMovie2( const char* fileName )
{
	if (moviePlaying)
		return;
	...
	//movieName.init(moviePath,realName,".bik");
	//movieName.init(moviePath,realName,".avi"); //magic 26042011
	movieName.init(moviePath,realName,".asf");
	bMovie = new MC2Movie;
	bMovie->init((char *)movieName,vRect,true);

	//magic 14032012 begin
	...
	hr = CoCreateInstance(FG1, NULL, CLSCTX_INPROC_SERVER,IG1, (void **)&pGraph);
    if (FAILED(hr))
    {
	...
	pVidWin->put_Owner((OAHWND)instance);
	pVidWin->put_WindowStyle(WS_CHILD | WS_CLIPSIBLINGS);

	pVidWin->SetWindowPosition(vRect.left,vRect.top,vRect.right,vRect.bottom);

    // Build the graph. IMPORTANT: Change this string to a file on your system.
    //hr = pGraph->RenderFile(L"C:\\Example.avi", NULL);
	LPCWSTR mName;
	//LPCTSTR mName;
	strcpy((char*)mName,movieName);
	hr = pGraph->RenderFile(mName, NULL);
    if (SUCCEEDED(hr))
    {
        // Run the graph.
        hr = pControl->Run();
...
// set the message text - mesage box by rollin
/* by mgto */ 
void ControlGui::setMessageText( int mode, const char* getString, int timeToDisplay)
{
	// CHANGE TEXT
	//gosASSERT( strlen( getString ) < 256 );
	//strcpy( i_text, getString);
	messageMode = mode;
	switch (mode)
	{
		case 0:
			instantStartTime = scenarioTime;
			instantEndTime = instantStartTime + timeToDisplay;
			renderInstantMessageText (getString, instantEndTime);
		break;
		case 1:
			// forward text to render function
			renderMessageText (getString);
			// pause the game
			if ( !MissionInterfaceManager::instance()->isPausedWithoutMenu() )
				MissionInterfaceManager::instance()->togglePauseWithoutMenu(); // ... topple pause
	}	
	// set message state
	i_dispMsg = true;

	//MessageBox(NULL,"der buffer","der titel",MB_OK); // use this for debugging
}


// render the message text - mesage box by rollin
/* by mgto */ 
void ControlGui::renderMessageText (const char* getString)
{
	// change text if getString is not empty
	//if (getString != "" )
	if (strcmp(getString,"")>0) //magic 29112011
		i_text = getString;
	...
	if (i_text != "")
	{
		...
		// check if game is paused and topple pause if not
		if ( !MissionInterfaceManager::instance()->isPausedWithoutMenu() )
			MissionInterfaceManager::instance()->togglePauseWithoutMenu(); // ... topple pause
		...
		itw_right =20; //10; //magic 28092012
		itw_top = 10;
		//magic 16022011 begin
		//itw_width = 400;
		//itw_height = 400;
		itw_width = Environment.screenWidth / 4;
		itw_height = Environment.screenHeight / 3;
		//magic 16022011 end
	...
	// wait for user to press "continue" key (we use ENTER)
	bool bEnterPressed = gos_GetKeyStatus(KEY_RETURN) == KEY_PRESSED;

	if (bEnterPressed && i_dispMsg)
	{
		//change i_text to nothing
		i_text = "";

		// un-pause the game
		// check if game is paused and topple pause if it is
		if ( MissionInterfaceManager::instance()->isPausedWithoutMenu() )
			MissionInterfaceManager::instance()->togglePauseWithoutMenu(); // ... topple pause
...
/* by mgto */ 
void ControlGui::setInstantMessageText( const char* getString, int timeToDisplay)
{
...
void ControlGui::renderInstantMessageText (const char* getString, int endTime)
{
	...
	if (i_text != "")
	{
		...
		//itw_height = 400;
		itw_width = Environment.screenWidth / 4;
		itw_height = Environment.screenHeight / 4;
		//magic 16022011 end
		itw_left = Environment.screenWidth - itw_right - itw_width;
