...
/*	path.init( moviePath, "msft", ".bik" ); //magic 26042011 to be changed
...
int	MainMenu::handleMessage( unsigned long what, unsigned long who )
{
	switch ( who )
	{
		case MM_MSG_NEW_CAMPAIGN:
			...
			{
				endAnim.begin(); //magic 20022012 disabled
				beginAnim.end(); //magic 20022012 disabled

				bLoadCampaign = true;
				LogisticsSaveDialog::instance()->beginCampaign();
				if ( LogisticsSaveDialog::instance()->isDone() )
				//if ( LogisticsSaveDialog::instance()->getStatus() == LogisticsScreen::YES) //magic 26082012
				//if (status != NO)
				{
						LogisticsData::instance->startNewCampaign( LogisticsSaveDialog::instance()->getFileName());
						status = RESTART;
				} //magic 25082012 disabled
				/*//magic 25082012 begin
				if ( LogisticsSaveDialog::instance()->isDone() )
					if ( LogisticsSaveDialog::instance()->oKorCancel())
					{
						LogisticsData::instance->startNewCampaign( LogisticsSaveDialog::instance()->getFileName());
						status = RESTART;
					}
				...
				*/
				/*switch ( LogisticsSaveDialog::instance()->getStatus() )
				{
					case NO: //PREVIOUS
						//LogisticsData::instance->startNewCampaign( LogisticsSaveDialog::instance()->getFileName());
						bLoadCampaign = false;
						status = MAINMENU;
						LogisticsSaveDialog::instance()->setStatus(MAINMENU);
					break;
					case YES: //NEXT:
						LogisticsData::instance->startNewCampaign( LogisticsSaveDialog::instance()->getFileName());
						status = RESTART;
				...
				*///magic 25082012 end //no good return to first mission of campaign RESTART
			}
			break;
		case MM_MSG_SAVE:
			if ( MPlayer )
		...
		case MM_MSG_SINGLE_MISSION:
			...
			{
				bLoadSingle = true;
				//LogisticsData::instance->setLoadSolo(true);//Magic 63
				LogisticsData::instance->loadSoloMission = true;
...
void MainMenu::update()
{
	...
	//getButton( MM_MSG_MULTIPLAYER )->disable( true ); //magic 15022012 ENABLED MULTIPLAYER BUTTON

	if ( introMovie )
	{
		//userInput->mouseOff(); //Magic25
...
//				beginFadeOut( 1.0f );
			...
			else
			{
				...
				bSave = bLoad = 0;
				status = RESTART;
				//status = MAINMENU; //magic 26082012
				file.close();
	...
	else if ( promptToDisconnect )
	{
		...
		if ( LogisticsOKDialog::instance()->isDone() )
		{
			if ( YES == LogisticsOKDialog::instance()->getStatus() )
			{
				...
				setDrawBackground( false ); //Magic was true
			}
			else
