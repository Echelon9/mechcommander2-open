...
void GameSoundSystem::purgeSoundSystem (void)
{
	...
	//messagesInQueue = 0; //magic 29112011 double
	//wholeMsgDone = true; //magic 29112011 double
...
long i;
	...
	if (queue[msgNumber])
	{
		...
		//UserHeapPtr msgHeap = queue[msgNumber]->msgHeap; //magic 02102011 disabled
		for (i=0;i<MAX_FRAGMENTS;i++)
		{
			//msgHeap->Free(queue[msgNumber]->data[i]);
			gos_Free(queue[msgNumber]->data[i]); //magic 02102011
			queue[msgNumber]->data[i] = NULL;
			//msgHeap->Free(queue[msgNumber]->noise[i]);
			gos_Free(queue[msgNumber]->noise[i]);  //magic 02102011
			queue[msgNumber]->noise[i] = NULL;
		}
		
		//msgHeap->Free(queue[msgNumber]);
		delete queue[msgNumber]; //magic 02102011
...
void GameSoundSystem::removeCurrentMessage(void)
{
	if (currentMessage)
	{
		...
		//UserHeapPtr msgHeap = currentMessage->msgHeap; //magic 02102011 disabled
		for (long j=0;j<MAX_FRAGMENTS;j++)
		{
			//msgHeap->Free(currentMessage->data[j]);
			gos_Free(currentMessage->data[j]); //magic 02102011
			currentMessage->data[j] = NULL;
			//msgHeap->Free(currentMessage->noise[j]);
			gos_Free(currentMessage->noise[j]); //magic 02102011
			currentMessage->noise[j] = NULL;
		}
		
		//msgHeap->Free(currentMessage);
		delete currentMessage; //magic 02102011
		currentMessage = NULL;
	}
