...
void WeaponBoltType::destroy (void)
{
	if (textureName)
		...
		//gos_Free(textureName); //magic 25032012
	textureName = NULL;
...
long WeaponBoltType::init (FilePtr objFile, unsigned long fileSize)
{
	...
	if (result != NO_ERR)
	{
		...
		//textureName = (char *)gos_Malloc(strlen(txmName)+1); //magic 25032012
...
void WeaponBolt::init (bool create, ObjectTypePtr _type)
{
	...
	if (((WeaponBoltTypePtr)_type)->textureName && stricmp(((WeaponBoltTypePtr)_type)->textureName,"NONE") != 0)
	{
		...
		//sprintf(tPath,"%s256\\",tglPath); //magic 31012012
		sprintf(tPath,"%s512\\",tglPath); //magic 15022012
...
void WeaponBolt::Save (PacketFilePtr file, long packetNum)
{
	...
	file->writePacket(packetNum,(MemoryPtr)&data,sizeof(WeaponBoltData),STORAGE_TYPE_RAW); //magic 11032012
}
