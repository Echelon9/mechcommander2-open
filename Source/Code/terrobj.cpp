...
void TerrainObject::destroy (void) 
{
	if (cellsCovered) 
	{
		...
		delete [] cellsCovered; //magic 02102011
		cellsCovered = NULL;
	}
	...
	if (subAreas0)
	{
		...
		//delete [] subAreas0; //magic 25032012
		subAreas0 = NULL;
	}
	...
	if (subAreas1)
	{
		...
		//delete [] subAreas1; //magic 25032012
		subAreas1 = NULL;
	}
...
void TerrainObject::calcCellFootprint (Stuff::Vector3D& pos) {
	...
	long cellList[MAX_CELL_COORDS]; //magic 07032012
...
void TerrainObject::calcSubAreas (long numCells, long cells[MAX_GAME_OBJECT_CELLS][2])
{

	numCellsCovered = numCells;
	if (numCellsCovered) {
		//cellsCovered = (short*)systemHeap->Malloc(4 * numCellsCovered); //bug - should be (sizeof(short) * 4 * numCellsCovered)
		//cellsCovered = new short[4 * numCellsCovered]; //magic 02102011
		cellsCovered = new long[4 * numCellsCovered]; //magic 07032012
		if (cellsCovered) {
			//short* curCoord = cellsCovered;
			long* curCoord = cellsCovered; //magic 07032012
		...
		for (long i = 0; i < numCellsCovered; i++) 
		{
			...
			if (addIt) 
			{
				if (!subAreas0)
				{
					...
					//magic 07032012 begin
					subAreas0 = (long *)ObjectTypeManager::objectCache->Malloc(sizeof(long) * MAX_SPECIAL_SUB_AREAS);
					//subAreas0 = new long[ MAX_SPECIAL_SUB_AREAS]; //magic 25032012
					memset(subAreas0,0,sizeof(long) * MAX_SPECIAL_SUB_AREAS);
					//magic 07032012 end
				}
		...
		for (i = 0; i < numCellsCovered; i++) 
		{
			...
			if (addIt) 
			{
				if (!subAreas1)
				{
					...
					//magic 07032012 begin
					subAreas1 = (long *)ObjectTypeManager::objectCache->Malloc(sizeof(long) * MAX_SPECIAL_SUB_AREAS);
					//subAreas1 = new long[ MAX_SPECIAL_SUB_AREAS]; //magic 25032012
					memset(subAreas1,0,sizeof(long) * MAX_SPECIAL_SUB_AREAS);
					//magic 07032012 end
				}
...
void TerrainObject::Save (PacketFilePtr file, long packetNum)
{
	...
	file->writePacket(packetNum,(MemoryPtr)&data,sizeof(TerrainObjectData),STORAGE_TYPE_RAW); //magic 11032012
}
...
void TerrainObject::CopyTo (TerrainObjectData *data)
{
	...
	memcpy(data->cellFootprint,cellFootprint,sizeof(long) * 4); //magic 07032012
	...
	if (subAreas0)
		...
		memcpy(data->subAreas0,subAreas0,sizeof(long) * MAX_SPECIAL_SUB_AREAS);//magic 07032012
	else
		//memset(data->subAreas0,0,sizeof(short) * MAX_SPECIAL_SUB_AREAS);
		memset(data->subAreas0,0,sizeof(long) * MAX_SPECIAL_SUB_AREAS);//magic 07032012

	if (subAreas1)
		//memcpy(data->subAreas1,subAreas1,sizeof(short) * MAX_SPECIAL_SUB_AREAS);
		memcpy(data->subAreas1,subAreas1,sizeof(long) * MAX_SPECIAL_SUB_AREAS);//magic 07032012
	else
		//memset(data->subAreas1,0,sizeof(short) * MAX_SPECIAL_SUB_AREAS);
		memset(data->subAreas1,0,sizeof(long) * MAX_SPECIAL_SUB_AREAS);//magic 07032012
	...
	memcpy(data->cellsCovered,cellsCovered, sizeof(long) * numCellsCovered * 2); //magic 07032012

	GameObject::CopyTo(dynamic_cast<GameObjectData *>(data));
}
//***************************************************************************
/*void TerrainObject::OldToNew (TerrainObjectData *data, TerrainObjectDataOld *dataOld) //magic 09032012 new function
{
	data->damage = dataOld->damage;
	data->vertexNumber = dataOld->vertexNumber;
	data->blockNumber = dataOld->blockNumber;
	data->pitchAngle = dataOld->pitchAngle;
	data->fallRate = dataOld->fallRate;
	data->powerSupply = dataOld->powerSupply;
	//memcpy(data->cellFootprint,cellFootprint,sizeof(short) * 4);
	//memcpy(data->cellFootprint,dataOld->cellFootprint,sizeof(long) * 4); //magic 07032012
	//long tempCellFootprint[4];
	for (int i=0;i<4;i++)
		data->cellFootprint[i] = (long)dataOld->cellFootprint[i];
		//tempCellFootprint[i] = dataOld->cellFootprint[i];
	//memcpy(data->cellFootprint,tempCellFootprint,sizeof(long) * 4); //magic 07032012
	memcpy(data->vectorFootprint,dataOld->vectorFootprint,sizeof(Stuff::Vector3D) * 4);
	data->numSubAreas0 = dataOld->numSubAreas0;
	data->numSubAreas1 = dataOld->numSubAreas1;

	if (dataOld->subAreas0)
		//memcpy(data->subAreas0,subAreas0,sizeof(short) * MAX_SPECIAL_SUB_AREAS);
		for (int i=0;i<MAX_SPECIAL_SUB_AREAS;i++)
			data->subAreas0[i] = (long)dataOld->subAreas0[i];
		//memcpy(data->subAreas0,subAreas0,sizeof(long) * MAX_SPECIAL_SUB_AREAS);//magic 07032012
	else
		//memset(data->subAreas0,0,sizeof(short) * MAX_SPECIAL_SUB_AREAS);
		memset(data->subAreas0,0,sizeof(long) * MAX_SPECIAL_SUB_AREAS);//magic 07032012

	if (dataOld->subAreas1)
		//memcpy(data->subAreas1,subAreas1,sizeof(short) * MAX_SPECIAL_SUB_AREAS);
		for (int i=0;i<MAX_SPECIAL_SUB_AREAS;i++)
			data->subAreas1[i] = (long)dataOld->subAreas1[i];
		//memcpy(data->subAreas1,subAreas1,sizeof(long) * MAX_SPECIAL_SUB_AREAS);//magic 07032012
	else
		//memset(data->subAreas1,0,sizeof(short) * MAX_SPECIAL_SUB_AREAS);
		memset(data->subAreas1,0,sizeof(long) * MAX_SPECIAL_SUB_AREAS);//magic 07032012

	data->listID = dataOld->listID;

	data->numCellsCovered = dataOld->numCellsCovered;
	if (numCellsCovered >= 162)
		STOP(("Object %d covers too many cells in Save/Load!!",getObjectType()->getObjTypeNum()));

	//memcpy(data->cellsCovered,cellsCovered, sizeof(short) * numCellsCovered * 2);
	for (int i=0;i<data->numCellsCovered;i++)
		data->cellsCovered[i] = (long)dataOld->cellsCovered[i];
	//memcpy(data->cellsCovered,cellsCovered, sizeof(long) * numCellsCovered * 2); //magic 07032012
...
void TerrainObject::Load (TerrainObjectData *data)
{
	...
	memcpy(cellFootprint,data->cellFootprint,sizeof(long) * 4); //magic 07032012
	...
	if (numSubAreas0)
	{
		...
		subAreas0 = (long *)ObjectTypeManager::objectCache->Malloc(sizeof(long) * MAX_SPECIAL_SUB_AREAS);
		//subAreas0 = new long[ MAX_SPECIAL_SUB_AREAS]; //magic 25032012
		memcpy(subAreas0,data->subAreas0,sizeof(long) * MAX_SPECIAL_SUB_AREAS);
		//magic 07032012 end
	}
	...
	if (numSubAreas1)
	{
		...
		subAreas1 = (long *)ObjectTypeManager::objectCache->Malloc(sizeof(long) * MAX_SPECIAL_SUB_AREAS);
		//subAreas1 = new long[ MAX_SPECIAL_SUB_AREAS]; //magic 25032012
		memcpy(subAreas1,data->subAreas1,sizeof(long) * MAX_SPECIAL_SUB_AREAS);
		//magic 07032012 end
	}
	...
	if (numCellsCovered)
	{
		...
		//cellsCovered = new short[numCellsCovered * 2]; //magic 02102011
		//memcpy(cellsCovered, data->cellsCovered,sizeof(short) * numCellsCovered * 2);
		//magic 07032012 begin
		cellsCovered = new long[numCellsCovered * 2]; //magic 02102011
		memcpy(cellsCovered, data->cellsCovered,sizeof(long) * numCellsCovered * 2);
		//magic 07032012 end
	}
}
