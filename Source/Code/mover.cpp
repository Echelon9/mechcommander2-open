...
void* MoveChunk::operator new (size_t ourSize) {
	...
	void* result = gos_Malloc(ourSize); //magic 24032012
	return(result);
}
...
void* StatusChunk::operator new (size_t ourSize) {
	...
	result = gos_Malloc(ourSize); //magic 24032012
	return(result);
}
...
void Mover::init (bool create) {
	...
	//numArmorLocations = 0; //magic 29112011 double
	numOther = 0;
	numWeapons = 0;
	numAmmos = 0;
	numAmmoTypes = 0;
...
void Mover::updateDebugWindow (GameDebugWindow* debugWindow) {
	...
	if (debugPage == 0) {
		...
		sprintf(s, "cont:");
//Magic ******************************** pogledati ovo za contact
		if (sensorSystem) {
			// display up to 4...
			long numConts = 4;
...
long Mover::setTeamId (long _teamId, bool setup) {
	...
	if (setup) {
		if (teamId > -1) {
			...
			//{ //magic 29012011
				pilot->setTeam(team);
			//team->addToRoster(this); //magic 29012011 no good here doubles the roster - need to know if this is triggered from mission
										//this is virtual func, must change headers for all virtual func setTeamId
										//in order to add new argument. Not for now.
			//team->addToRoster(pilot->getVehicle()); //magic 29012011
			//} //magic 29012011
...
float Mover::getEcmRange (void) {
	...
	if ((ecm != 255))
		...
		return MasterComponent::masterList[inventory[ecm].masterID].getEcmRange(); //magic 16022012
...
void Mover::loadPilot (char* pilotFileName, char* brainFileName, LogisticsPilot *lPilot) {
	...
	if (lPilot)
	{
		...
		pilot->skills[MWS_SENSORS] = pilot->skillRank[MWS_SENSORS] = lPilot->getSensorSkill();//Magic 55
		pilot->skills[MWS_JUMPING] = pilot->skillRank[MWS_JUMPING] = lPilot->getJumping();//Magic 55
					...
					//if (GlobalMoveMap[moveLevel]->areas[area].type == AREA_TYPE_NORMAL) {
						if (GlobalMoveMap[moveLevel]->getPathExists(curArea, area) == GLOBALPATH_EXISTS_UNKNOWN) {
							if (CalcValidAreaTable) {
								...
								GlobalPathStep globalPath[MAX_GLOBAL_PATH]; //magic 07032012
...
long Mover::calcMovePath (MovePathPtr path,
	...
	if (pathType == MOVEPATH_SIMPLE) {
		...
		long clearCost = 0;
		if (maxMoveSpeed != 0.0)
			clearCost = (float2short)(cellLength / maxMoveSpeed * 50.0); //magic 07032012 ? staviti (long)?
		if (clearCost > 0) {
			long jumpCost = 0;
			long numOffsets = 8;
...
bool Mover::weaponInRange (long weaponIndex, float metersToTarget, float buffer) {
	...
	float minRange = (float)weapon->getMinWeaponRange() - buffer;
	float maxRange = (float)weapon->getMaxWeaponRange() + buffer;
	//magic 11022012 end
	...
	if (numWeapons > 0) 
	{
		...
		for (i = numOther + numWeapons; i < (numOther + numWeapons + numAmmos); i++) 
		{
			...
			if (!foundAmmoWeapon)
				...
				PAUSE(("Mech %s has ammo for a weapon it is not carrying!  Ammo Index is %d",name,ammoMasterId)); //magic 10022012
		}
...
long Mover::calcFireRanges (void) {
	...
	for (long curWeapon = numOther; curWeapon < (numOther + numWeapons); curWeapon++) {
		if (!inventory[curWeapon].disabled && (getWeaponShots(curWeapon) > 0)) {
			...
			//magic 11022012 begin
			MasterComponentPtr weapon = &MasterComponent::masterList[inventory[curWeapon].masterID];
			float minWeaponRange = (float)weapon->getMinWeaponRange();
			float maxWeaponRange = (float)weapon->getMaxWeaponRange();
			//magic 11022012 end
	...
	*/ //magic 11022012 disabled
	//magic 11022012 begin
	/*//Magic Debug file begin
	char tmp2[256];
	FitIniFile MtestFile;
	MtestFile.open("data\\calcfireranges.fit", CREATE);
	//debug file--------------*/

	float lastOptimalRange = optimalRange;
	float maxMinRange = 0; //max Min Range for all active weapons
	float minMaxRange = 1000000.0; //min max Range for all active weapons
	float sumOfMaxRanges = 0;
	float sumOfMinRanges = 0;
	int wCounter = 0;
	for (curWeapon = numOther; curWeapon < (numOther + numWeapons); curWeapon++)
	{
		MasterComponent* weapon = &MasterComponent::masterList[inventory[curWeapon].masterID];
		if (!inventory[curWeapon].disabled && (getWeaponShots(curWeapon) > 0))
		{
			...
			if (maxWeaponRange < minMaxRange)
				minMaxRange = maxWeaponRange;
			if (minWeaponRange > maxMinRange)
				maxMinRange = minWeaponRange;

			sumOfMaxRanges += maxWeaponRange;
			sumOfMinRanges += minWeaponRange;
	...
	if (minMaxRange > maxMinRange)
		optimalRange = (minMaxRange + maxMinRange) / 2.0; //callc is OK but no good
		//optimalRange = minMaxRange * 1.5;
	else
		optimalRange = (sumOfMinRanges + sumOfMaxRanges) / (float)wCounter;

		/*//debug file---------------
		sprintf( tmp2, "minMaxRange %f", minMaxRange );
		MtestFile.writeBlock( tmp2 );
		sprintf( tmp2, "maxMinRange %f", maxMinRange );
		MtestFile.writeBlock( tmp2 );
		sprintf( tmp2, "sumOfMinRanges %lf", sumOfMinRanges );
		MtestFile.writeBlock( tmp2 );
		sprintf( tmp2, "sumOfMaxRanges %lf", sumOfMaxRanges );
		MtestFile.writeBlock( tmp2 );
		sprintf( tmp2, "optimalRange %lf", optimalRange );
		MtestFile.writeBlock( tmp2 );
	MtestFile.close();
...
float Mover::calcAttackChance (GameObjectPtr target, long aimLocation, float targetTime, long weaponIndex, float modifiers, long* range, Stuff::Vector3D* targetPoint) 
{
	...
	//magic 11022012 begin
	float weaponMinRange = (float)MasterComponent::masterList[inventory[weaponIndex].masterID].getMinWeaponRange();
	float weaponLongRange = (float)MasterComponent::masterList[inventory[weaponIndex].masterID].getMaxWeaponRange();
	//magic 11022012 end
...
bool Mover::refit (float pointsAvailable, float& pointsUsed, bool ammoOnly) {
	...
	if (getResourcePoints() > 0)
	{
		GameObjectPtr refitBuddy = ObjectManager->getByWatchID(refitBuddyWID);
		float curRP, maxRP;
		curRP = refitBuddy->getRefitPoints();
		maxRP = refitBuddy->getTotalRefitPoints();
		float logisticsRP = LogisticsData::instance->getResourcePoints()/20.0f; //magic 06072011
		if (curRP < maxRP)
		{
			float difRP = maxRP - curRP;
			float numResourcePoints = getResourcePoints()/20.0f;
			//magic 06072011 begin
			if (difRP > logisticsRP)
				difRP = logisticsRP;
			//magic 06072011 end
			if (difRP < numResourcePoints)
			{
				refitBuddy->addRefitPoints(difRP);
				difRP *= 20.0f;
				addResourcePoints(-difRP);
				LogisticsData::instance->addResourcePoints(-difRP);
			}
			else
			{
				refitBuddy->addRefitPoints(numResourcePoints);
				numResourcePoints *= 20.0f;
				addResourcePoints(-numResourcePoints);
				LogisticsData::instance->addResourcePoints(-numResourcePoints);
			}
			//magic 06072011 begin
			//if (LogisticsData::instance->getResourcePoints() < 0)
			//	LogisticsData::instance->setResourcePoints(0);
			//magic 06072011 end
			result = true;
		}
	}
	...
	//if (getResourcePoints() > 0)
	if (getResPoints() > 0) //magic 29072011
	//if (getResourcePoints() > 20.0f) //magic 08092012
	{
		GameObjectPtr refitBuddy = ObjectManager->getByWatchID(refitBuddyWID);
		float curRP, maxRP;
		curRP = refitBuddy->getRefitPoints();
		...
		if (curRP < maxRP)
		{
			float difRP = maxRP - curRP;
			//float numResourcePoints = getResourcePoints()/20.0f; 
			float numResourcePoints = getResPoints(); ///20.0f;
			//float numResourcePoints = getResourcePoints()/20.0f; //magic 08092012
			...
			if (difRP < numResourcePoints)
			{
				...
				//difRP *= 20.0f;
				//addResourcePoints(-difRP);
				//addResPoints(-difRP); //magic 29072011
				//magic 01082011 begin
				armor[GROUNDVEHICLE_LOCATION_TURRET].curArmor -= difRP;
				if ( armor[GROUNDVEHICLE_LOCATION_TURRET].curArmor <= 0.0f )
					armor[GROUNDVEHICLE_LOCATION_TURRET].curArmor = 0.f;
				difRP *= 20.0f;
				//magic 01082011 end //magic 08092012 disabled
				//addResourcePoints(-difRP); //magic 08092012
				LogisticsData::instance->addResourcePoints(-difRP);
			}
			else
			{
				...
				//numResourcePoints *= 20.0f;
				//addResourcePoints(-numResourcePoints);
				//addResPoints(-numResourcePoints); //magic 29072011
				
				//magic 01082011 begin
				armor[GROUNDVEHICLE_LOCATION_TURRET].curArmor -= numResourcePoints;
				if ( armor[GROUNDVEHICLE_LOCATION_TURRET].curArmor <= 0.0f )
					armor[GROUNDVEHICLE_LOCATION_TURRET].curArmor = 0.f;
				numResourcePoints *= 20.0f;
				//magic 01082011 end //magic 08092012 disabled
				//addResourcePoints(-numResourcePoints); //magic 08092012
				LogisticsData::instance->addResourcePoints(-numResourcePoints);
			}
			//char tmp2[64];
			//sprintf( tmp2, "DifRP, numResourcePoints, curArmor: %f,%f, %f ",difRP, numResourcePoints, armor[GROUNDVEHICLE_LOCATION_TURRET].curArmor );
			//MessageBox(NULL,tmp2,"MOVER",MB_OK);
...
//		destroyBodyLocation(MECH_BODY_LOCATION_HEAD);
	...
	//ControlGui::instance->forceGroupBar.sortForceGroup(); //magic 28092012 not here
...
void Mover::Save (PacketFilePtr file, long packetNum)
{
	...
	file->writePacket(packetNum,(MemoryPtr)&data,sizeof(MoverData),STORAGE_TYPE_RAW); //magic 11032012
}
...
bool Mover::isJumping(MoverPtr myVehicle)
{
return (((BattleMechPtr)myVehicle)->inJump);
}
