...
void LogisticsMissionInfo::readMissionInfo( FitIniFile& file, LogisticsMissionInfo::MissionInfo* pInfo )
{
	...
	topDropWeight = pInfo->dropWeight;//M17
}
	...
	if ( NO_ERR == file.seekBlock( "AdditionalPurchaseFiles" ) )
	{
		...
		while( true )
		{
			...
			if ( NO_ERR == file.readIdString( header, tmp, 255 ) )
			{
				...
				//char* pfName = (char *)gos_Malloc(strlen( tmp )+1 ); //magic 26092011
				strcpy( pfName, tmp );
				additionalPurchaseFiles.Append( pfName );
				if (pfName) //magic 28022012
					delete [] pfName; //magic 28092011
				i++;
			}
			else
				break;
...
void LogisticsMissionInfo::setSingleMission( const char* missionFileName )
{
	...
	pInfo->fileName = missionName;
	/*//Magic 63 begin
	EString path1 = missionPath;
	path1 += "purchase.fit";
	pInfo->purchaseFileName = path;
	//Magic 63 end*/
	pInfo->completed = 0;
	...
	topDropWeight = pInfo->dropWeight; //M
...
void				LogisticsMissionInfo::addBonusPurchaseFile( const char* fileName )
{
	...
	char* pNewFile = new char[strlen( fileName )+1]; //create new entry for the list
	//char* pNewFile = (char *)gos_Malloc(strlen( fileName )+1); //magic 26092011
	strcpy( pNewFile, fileName ); //magic 02092012 disabled

 	additionalPurchaseFiles.Append( pNewFile ); //cause wron filename to be stored

	/*if (pNewFile) //magic 28022012
		delete [] pNewFile; //magic 28092011 */ //magic 02092012 disabled - Entry released elswere
}
...
int LogisticsMissionInfo::incMaxDropWeight(int amount)
{
		//pInfo->dropWeight += amount;//M17
	MissionGroup* pGroup = &groups[currentStage];

	if ( currentMission != -1 )
	{
		pGroup->infos[currentMission]->dropWeight += amount;
		return pGroup->infos[currentMission]->dropWeight;
	}
	else
		return 10000;
...
int LogisticsMissionInfo::decMaxDropWeight(int amount)
{
		//pInfo->dropWeight += amount;//M17
	MissionGroup* pGroup = &groups[currentStage];
...
bool LogisticsMissionInfo::toggleRepairTruck() //new magic 14072011
{
	MissionGroup* pGroup = &groups[currentStage]; 
	pGroup->infos[currentMission]->enableRepairTruck = !pGroup->infos[currentMission]->enableRepairTruck;
	return pGroup->infos[currentMission]->enableRepairTruck;
}

bool LogisticsMissionInfo::toggleSalvageCraft() //new magic 29082011
{
	MissionGroup* pGroup = &groups[currentStage]; 
	pGroup->infos[currentMission]->enableSalavageCraft = !pGroup->infos[currentMission]->enableSalavageCraft;
	return pGroup->infos[currentMission]->enableSalavageCraft;
}

//magic 25052012 begin
bool LogisticsMissionInfo::toggleSensorStrike() //new magic 14072011
{
	MissionGroup* pGroup = &groups[currentStage]; 
	pGroup->infos[currentMission]->enableSensorStrike = !pGroup->infos[currentMission]->enableSensorStrike;
	return pGroup->infos[currentMission]->enableSensorStrike;
}

bool LogisticsMissionInfo::toggleArtilleryPiece() //new magic 29082011
{
	MissionGroup* pGroup = &groups[currentStage]; 
	pGroup->infos[currentMission]->enableArtilleryPiece = !pGroup->infos[currentMission]->enableArtilleryPiece;
	return pGroup->infos[currentMission]->enableArtilleryPiece;
}

bool LogisticsMissionInfo::toggleAirStrike() //new magic 29082011
{
	MissionGroup* pGroup = &groups[currentStage]; 
	pGroup->infos[currentMission]->enableAirStrike = !pGroup->infos[currentMission]->enableAirStrike;
	return pGroup->infos[currentMission]->enableAirStrike;
}
//magic 25052012 end
//magic 02062012 begin
int LogisticsMissionInfo::addRP(int amount)
{
		//pInfo->dropWeight += amount;//M17
	MissionGroup* pGroup = &groups[currentStage];
	...
	if ( currentMission != -1 )
	{
		pGroup->infos[currentMission]->resourcePoints += amount;
		return pGroup->infos[currentMission]->resourcePoints;
	}
	else
		return 10000;
...
int LogisticsMissionInfo::setRP(int amount)
{
		//pInfo->dropWeight += amount;//M17
	MissionGroup* pGroup = &groups[currentStage];
...
int LogisticsMissionInfo::setMaxDropWeight(int amount)
{
		//pInfo->dropWeight += amount;//M17
	MissionGroup* pGroup = &groups[currentStage];
