...
//extern UserHeapPtr missionHeap; //magic 01102011
...
void GoalObject::addLink (GoalObjectPtr gobject, GoalLinkType type) {
	...
	GoalLinkPtr newLink = new GoalLink; //[sizeof(GoalLink)]; //magic 30092011
...
void GoalManager::destroy (void) {
	...
	if (goalObjectPool) {
		...
		delete [] goalObjectPool; //magic 30092011
		goalObjectPool = NULL;
	}
	numGoalObjects = 0;
	goalObjectPoolSize = 0;
...
void GoalManager::setup (long poolSize) {
	...
	//goalObjectPool = new GoalObject[sizeof(GoalObject) * goalObjectPoolSize]; //magic 30092011
	goalObjectPool = new GoalObject[goalObjectPoolSize]; //magic 30092011
	clear();
}
