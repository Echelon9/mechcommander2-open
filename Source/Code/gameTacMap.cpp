	...
	for (i=0;i<(ObjectManager->numMovers);i++)
	{
		...
		if (mover && mover->getExists() && !(mover->isDestroyed() || mover->isDisabled()))
		{
			...
			if (mover->getTeamId() == Team::home->id)
			{
				...
				//&&  (mover->getStatus() != OBJECT_STATUS_SHUTDOWN) //magic 13022012 disabled will show if someone has active probe in range
				...
				&&	(mover->getEcmRange() <= 0.0f) ) )	//Do not draw ECM mechs!!)
			{
				...
				if  (mover->getStatus() != OBJECT_STATUS_SHUTDOWN) //if enemy is not shutdown show red color ring
				//magic 13022012 end
					colorRing = 0xffff0000;
				//magic 13022012 begin
				else
					colorRing = 0; //dont show sensor ring for shutdown mechs
				//magic 13022012 end
