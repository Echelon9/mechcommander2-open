...
class GroundVehicleType : public ObjectType {
	...
	public:
		...
		long				maxRefitPoints; //magic 08092012
...
class GroundVehicle : public Mover {
	...
	public:
		...
		virtual long init (const char* vehicleName);
		//magic 12122010 end
	...
	//magic 05072011 begin
		float getResourcePoints(void) {
				return(((GroundVehicleTypePtr)getObjectType())->resourcePoints);
		}
		bool addResourcePoints(long numAdd) {
				((GroundVehicleTypePtr)getObjectType())->resourcePoints += numAdd;
				if (((GroundVehicleTypePtr)getObjectType())->resourcePoints < 0) 
					((GroundVehicleTypePtr)getObjectType())->resourcePoints = 0; //magic 08092012
				return (true);
		}
		bool addRefitPoints(float pointsToAdd);
	//magic 05072011 end 
...
/*//magic 08092012 begin
		float getTotalRefitPoints(void) {
			if (canRefit)
				return(((GroundVehicleTypePtr)getObjectType())->maxRefitPoints);
			else
				return(0.0);
		}
		...
		float getRefitPoints(void) {
			if (canRefit)
				return(((GroundVehicleTypePtr)getObjectType())->refitPoints);
			else
				return(0.0);
		}
//magic 08092012 end */
