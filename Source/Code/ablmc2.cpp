...
UserHeapPtr				AblStackHeap = NULL; //magic 21042012
//UserHeapPtr				AblCodeHeap = NULL; //magic 13032012
//UserHeapPtr				AblSymbolHeap = NULL; //magic 21042012
...
void execObjectChangeSides (void) {
	...
	if ((objectId >= MIN_UNIT_PART_ID) && (objectId <= MAX_UNIT_PART_ID)) {
		...
		if (object)
			...
			//if (object->isMover()) //magic 29012011
			//{
				object->setTeamId(newObjectSide,1); //magic 29012011
			//}
			//else //magic 29012011
			//{
			//	object->setTeamId(newObjectSide,0); //magic 28012011
			//}
	}
}

//*****************************************************************************
void execAddMoverToPlayer (void) //magic 29012011 new function
{
	...
	long objectId = ABLi_popInteger();
	long newObjectSide = ABLi_popInteger();
	long newObjectCommander = ABLi_popInteger();
	...
	if ((objectId >= MIN_UNIT_PART_ID) && (objectId <= MAX_UNIT_PART_ID)) {
		...
		// We have a group.  Act accordingly.
		// Run through all objects fitting the group.
		Fatal(0, " Cannot ABL:AddMoverToPlayer for Mover Groups ");
		...
		if (object)
			if (object->isMover())
			{
				//magic 27042012 begin override existing brain with pbrain when adding existing unit
				long numErrors, numLinesProcessed;
				FullPathFileName brainFullFileName;
				object->getPilot()->setBrainName("pbrain"); 
				...
				object->getPilot()->setBrain(moduleHandle);
				//27042012 end
				mission->tradeMover2(object->getPilot()->getVehicle(), newObjectSide, newObjectCommander);
			}
	}
}
//*****************************************************************************
void execRemoveMoverFromPlayer (void) //magic 21052012 new function
{

	//Remove mover from players Force Group.
	//
	//		PARAMS:	integer
	//
	//		Returns: nothing!
	...
	if ((objectId >= MIN_UNIT_PART_ID) && (objectId <= MAX_UNIT_PART_ID)) {
		...
		// We have a group.  Act accordingly.
		// Run through all objects fitting the group.
		Fatal(0, " Cannot ABL:RemoveMoverFromPlayer for Mover Groups ");
		...
		if (object)
			if (object->isMover())
			{
				...
				FullPathFileName brainFullFileName;
				object->getPilot()->setBrainName("pbrain"); 
				brainFullFileName.init(warriorPath, "dredattack01", ".abl");
				long moduleHandle = ABLi_preProcess(brainFullFileName, &numErrors, &numLinesProcessed);
				gosASSERT(moduleHandle >= 0);
				object->getPilot()->setBrain(moduleHandle);
				//27042012 end
				mission->removeMoverFromFG(object->getPilot()->getVehicle());
			}
	}
}
//*****************************************************************************
void execSetBrain (void) //magic 27042011 new function
{
	...
	if ((objectId >= MIN_UNIT_PART_ID) && (objectId <= MAX_UNIT_PART_ID)) {
		...
		// We have a group.  Act accordingly.
		// Run through all objects fitting the group.
		Fatal(0, " Cannot ABL:SetBrain for Mover Groups ");
		...
		if (object)
			if (object->isMover())
			{
				//override existing brain with new brain
				long numErrors, numLinesProcessed;
				FullPathFileName brainFullFileName;
				object->getPilot()->setBrainName(brainName); 
				brainFullFileName.init(warriorPath, brainName, ".abl");
				long moduleHandle = ABLi_preProcess(brainFullFileName, &numErrors, &numLinesProcessed);
				gosASSERT(moduleHandle >= 0);
				object->getPilot()->setBrain(moduleHandle);
...
void execAddResourcePoints (void) //magic 29082011 new function
{
	...
	long pointsToAdd = ABLi_popInteger();
	LogisticsData::instance->addResourcePoints(pointsToAdd);

}
//*****************************************************************************

void execAddMoney (void) //magic 29082011 new function
{
	...
	long pointsToAdd = ABLi_popInteger();
	LogisticsData::instance->addCBills(pointsToAdd);

}
//*****************************************************************************

void execToggleRepairTruck (void) //magic 29082011 new function
{
...
void execToggleSensorStrike (void) //new function
{
	//		PARAMS:	none
	//		Returns: nothing!
	LogisticsData::instance->toggleSensorStrike();
}
//*****************************************************************************
void execToggleArtilleryPiece (void) //new function
{
	//		PARAMS:	none
	//		Returns: nothing!
	LogisticsData::instance->toggleArtilleryPiece();
}
//*****************************************************************************
void execToggleAirStrike (void) //new function
{
	//		PARAMS:	none
	//		Returns: nothing!
	LogisticsData::instance->toggleAirStrike();
}
//magic 25052012 end
//*****************************************************************************

void execToggleSalvageCraft (void) //magic 29082011 new function
{
	...
	//long pointsToAdd = ABLi_popInteger();
	LogisticsData::instance->toggleSalvageCraft();
...
void execTeleportToPoint (void) {
	...
	if (object) 
	{
		if (object->isMover())
		{
			...
			mover->setTeleportPosition(location);
		}
	}
...
void execSetObjectiveStatus (void) {
	...
	ABLi_pushInteger(mission->setObjectiveStatus2(objectiveNum,status));
}
...
//*****************************************************************************
void execSetTxtBuildingName (void) //magic 09092011 new function
{
	...
	//		PARAMS:	integer			building ID
	//
	//		char pointer		pointer to string with new name
	//
	//-----------------------------------------------------

	long objectId = ABLi_popInteger();
	char* newName = ABLi_popCharPtr();
...
//*****************************************************************************
void execGetTargetRelativePosition (void) //magic 11022012 new function
{

		//-----------------------------------------------------
	//
	//	GET TARGET RELATIVE POSITION function:
	//
	//		Retrieves the range and angle to the current target.
	//
	//		PARAMS:
	//				integer				target id

	//				@real				is set to the range (in meters) to contact
	//
	//				@real				is set to the angle (in degrees) to contact
	//
	//		RETURN: integer				error code
	...
	*angle = 0.0;
	...
	GameObjectPtr object = getObject(objectId);
	if (object && CurObject) {
		*range = CurObject->distanceFrom(object->getPosition());
		*angle = CurObject->relFacingTo(object->getPosition());
		result = NO_ERR;
	}
...
//*****************************************************************************
void execGetRelativePositionToTarget (void)  //magic 11022012 new function
{
	//----------------------------------------------------------------------
	//
	//	Get Relative Position To Target function:
	//
	//		Calcs a position relative to the given object
	//
	//		PARAMS:	integer				object id
...
/*void execMagicAttack (void) {
	...
	long result = 0;
	if (CurWarrior)
		result = CurWarrior->magicAttack(target, params);
...
{
	//Takes an integer for msg ID 0 or 1.
	//
	// char pointer to text message
	// integer time duration in seconds
	// returns nothing.
   	//
	
	long msgId = ABLi_popInteger();
	char* msg = ABLi_popCharPtr();
	long timeToDisplay = ABLi_popInteger();

	//char messageText[512];
	mission->missionInterface->setTextMsg(msgId,msg,timeToDisplay);
		//setTutorialText(tutorialMessages[currentMessage]);
...
void* ablSystemMallocCallback (unsigned long memSize) {
	...
	//return(gos_Malloc(memSize)); //magic 02102011
	//magic 21042012 begin
	if (memSize > 0) 
		return(gos_Malloc(memSize));
	else
		return NULL;
	//magic 21042012 end
}
...
void* ablStackMallocCallback (unsigned long memSize) {
	...
	//if (!AblSymbolHeap) //magic - error in orig code
	if (!AblStackHeap)
...
void* ablCodeMallocCallback (unsigned long memSize) {
	...
	if (memSize > 0) //magic 21042012
		return(gos_Malloc(memSize));  //magic 13032012
	else
		return NULL; //magic 21022012
}
...
void ablCodeFreeCallback (void* memBlock) {
	...
	if (memBlock)
		gos_Free(memBlock); //magic 13022012
}
...
void initABL (void) {
	...
	//long heapErr = AblSymbolHeap->init(1023999,"0",true); //magic 27032011
	if (heapErr != NO_ERR)
		ABL_Fatal(0, "ABLi_init: unable to create ABL symbol table heap");*/ //magic 21042012 disabled

	AblStackHeap = new UserHeap;
	//long heapErr = AblStackHeap->init(511999);
	//long heapErr = AblStackHeap->init(1024000); //magic 10052012
	long heapErr = AblStackHeap->init(4096000); //magic 10052012
	//heapErr =  AblStackHeap->init(767999,"0",true); //magic 27032011
	if (heapErr != NO_ERR)
		ABL_Fatal(0, "ABLi_init: unable to create ABL stack heap"); //magic 13022012 disabled

	/*AblCodeHeap = new UserHeap;
	heapErr = AblCodeHeap->init(307199);
	//heapErr = AblCodeHeap->init(511999); //magic 27032011
	if (heapErr != NO_ERR)
		ABL_Fatal(0, "ABLi_init: unable to create ABL code heap");*/ //magic 13022012 disabled

	ABLi_init(40958, //20479, //AblRunTimeStackSize, //magic 07032012
			  204800, //102400, //AblMaxCodeBlockSize, //magic 07032012
			  400, //200, //AblMaxRegisteredModules, //magic 07032012
			  200, //100, //AblMaxStaticVariables, //magic 07032012
...
//	ABLi_addFunction("checkobjectivetimer", false, "i", "r", execCheckObjectiveTimer);
	...
	ABLi_addFunction("gettargetrelativeposition", false, "irr", "i", execGetTargetRelativePosition);
...
//	ABLi_addFunction("addstrikes", false, "iii", NULL, execAddStrikes);
	...
	/*// magic 25112010 begin
	ABLi_addFunction("magicattack", true, "ii", "i", execMagicAttack);
	//magic 25112010 end*/
	//magic 27122010 begin
	ABLi_addFunction("settextmsg", false, "iCi", NULL, execSetTextMsg);
	ABLi_addFunction("addmovertoplayer", false, "iii", NULL, execAddMoverToPlayer);
	//magic 27122010 end
	//magic 29082010 begin
	...
	//magic 09092011 end
	//magic 02022012 begin
	ABLi_addFunction("teleporttopoint", false, "iR", NULL, execTeleportToPoint);
	//magic 02022012 end
	//magic 27042012 begin
	ABLi_addFunction("setbrain", false, "iC", NULL, execSetBrain);
	//magic 27042012 end
	//magic 21052012 begin
	ABLi_addFunction("removemoverfromplayer", false, "i", NULL, execRemoveMoverFromPlayer);
	//magic 21052012 end
