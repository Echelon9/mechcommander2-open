...
int MechLabScreen::init( FitIniFile& file )
{
	...
	if ((Environment.screenWidth > 800) && (Environment.screenHeight > 600))
	{
		float resXFactor = Environment.screenWidth / 800.0;
		float resYFactor = Environment.screenHeight / 600.0;
		LogisticsScreen::init2( file, resXFactor, resYFactor, "Static", "Text", "Rect", "Button" );
	}
	else
	{
		//float resXFactor = 1.0;
		//float resYFactor = 1.0;
		LogisticsScreen::init( file, "Static", "Text", "Rect", "Button" );
	}
	//magic 18022012 end*/ //factor is not working - leave it for now
	...
	//M14
	componentListBox.initComponents();
	//M14
...
void MechLabScreen::begin()
{
	...
	if (!cMech)
		cMech = LogisticsData::instance->getMechToModify();

	LogisticsVariant* pVariant = cMech->getVariant(); 
	//magic 12052012 end
	if ( !pVariant ) // if we are coming directly from the main menu
	{
		...
		//Magic 50 begin
		LogisticsComponent* p2Comp[64];
		int p2count;
		long p2pom;
		pVariant->getComponents( p2pom, p2Comp );
		p2count = p2pom;
		LogisticsComponent* p1Comp[300];
		int p1count;
		LogisticsData::instance->getAllInventoryComponents( p1Comp, p1count );
		for (int i = 0; i < p2count; i++)
		{
			p1Comp[p1count] = p2Comp[i];
			p1count++;
		}
		//Magic 50 end
		...
		if ( maxCount )
		{
			...
			{
				//if ( pVar[i]->allComponentsAvailable() ) //Magic 50 removed
				if ( pVar[i]->allComponentsInInventory(p1Comp, p1count))// || pVar[i]->allComponentsInInventory(p2Comp, p2count)) // OR SELF Magic 50
				{
	...
	if ( pVariant )
	{
		//textObjects[3].setText( pVariant->getChassisName() ); //magic 31102011 disabled
		//magic 31102011 begin
		textObjects[3].setText(pVariant->getRealChassisName());
		//magic 31102011 end

		originalCost = pVariant->getCost();
		
		//magic 14122010 begin
		//EString varName = pVariant->getName();
		textObjects[17].setText( pVariant->getName() );
		//textObjects[17].setText( varName); //Not work
		//magic 14122010 end
		char path[256];
		strcpy( path, artPath );
		strcat( path, "MCL_MC_" );
	...
	/*	strcat( path, mechName );
		...
		//for ( int i = 51; i < 54; i++ )
		for ( int i = 11; i < 14; i++ ) //magic 18022012
		...
		statics[10].setTexture( path ); //magic 18022012
		...
		componentListBox.setType2( COMPONENT_FORM_WEAPON_BALLISTIC, -1, -1, -1, -1, -1 ); //magic 08022012
		...
		{
			statics[11].showGUIWindow( true ); //magic 18022012
		}
		else
			statics[11].showGUIWindow( false ); //magic 18022012
			//magic 07052012 end
	}
	...
	/*if ( pVariant->getChassis()->jumpJetsAllowed() )
	{
		...
		//statics[14].showGUIWindow( true ); //magic 18022012
		statics[11].showGUIWindow( true ); //magic 18022012
	}
	else
		//statics[54].showGUIWindow( false );
		//statics[14].showGUIWindow( false ); //magic 18022012
		statics[11].showGUIWindow( false ); //magic 18022012*/ //magic 07022012 moved above
}
...
void MechLabScreen::update()
{
	...
	LogisticsVariant* pVariant = cMech->getVariant(); //magic 12052012
	...
	//variantList.update(); //magic 14122010
	...
	//	float curAmount = previousAmount - (costOfChange/countDownTime * previousAmount); 
	...
	//magic 07052012 activated 4 lines below
	char text[64];
	sprintf( text, "%.0lf ", costOfChange );
	...
	//magic 07052012 begin
	bool disableIntRepair = cMech->getInternalDamage() <= 0; //magic 12052012
	bool disableArmRepair = cMech->getArmorDamage() <= 0;  //magic 12052012

	if (disableIntRepair)
		getButton( MB_MSG_REPAIR__INT )->disable( 1 );
	else
		getButton( MB_MSG_REPAIR__INT )->disable( 0 );

	if (disableArmRepair)
		getButton( MB_MSG_REPAIR__ARM )->disable( 1 );
	else
		getButton( MB_MSG_REPAIR__ARM )->disable( 0 );

	if (disableArmRepair && disableIntRepair)
		getButton( MB_MSG_REPAIR__ALL )->disable( 1 );
	else
		getButton( MB_MSG_REPAIR__ALL )->disable( 0 );
	//magic 07052012 end
			...
			if ( i != -1 && j != -1 )
			{
				...
				if ( pComp && pComp == pSelectedComponent )
				{
					if ( canRemoveComponent( pComp ) )
					{
						...
						dragY = j;
						bDragFromList = 0;
						removeComponentToDrag( i, j ); //napraviti removeToDrag novu

						setComponent( pComp );
						// Magic begin add component to list box (NOT HERE - END DRAG)
						
						//ComponentListItem* pNItem = new ComponentListItem( pComp );
						//componentListBox.AddItem( pNItem );
						//componentListBox.AddSortedItem( pNItem );
						//Magic End 
						updateDiagram();
					}
					else
...
//		else if ( userInput->isLeftDoubleClick() )
		else if ( userInput->isRightClick() )
		{
			long tmpI, tmpJ;
			...
			if ( tmpI != -1 && tmpJ != -1 )
			{
				...
				if ( pComp )
				{
					if ( canRemoveComponent( pComp ) )
					{
						...
						// Magic begin add component to list box
						//ComponentListItem* pNItem = new ComponentListItem( pComp );
						//componentListBox.AddItem( pNItem );
						//Dodati funkciju koja ubacuje u icomponents
						//componentListBox.addIComponent( pSelectedComponent );
						//Magic End 
						componentListBox.SelectItem( -1 );
						if ( -1 == selectFirstDiagramComponent() )
							selectFirstLBComponent();				
...
void MechLabScreen::swapVariant()
{
	...
	{
		//Magic begin m10
		pVariant->getComponents( compCount, compList );
		for ( int i = 0; i < compCount; ++i )
		{
			LogisticsComponent* pComponent = LogisticsData::instance->getComponent( compList[i] );
			componentListBox.addIComponent( pComponent );
			
		}
		pNewVariant->getComponents( compCount, compList );
		for ( int i = 0; i < compCount; ++i )
		{
			LogisticsComponent* pComponent = LogisticsData::instance->getComponent( compList[i] );
			componentListBox.removeIComponent( pComponent );
		}
	//Magic end
...
int	MechLabScreen::handleMessage( unsigned long msg, unsigned long who)
{
	...
	switch( who )
	{
		case MB_MSG_NEXT: // actuall accept
			{
				...
				//if ( costOfChange > LogisticsData::instance->getCBills() )
				//{
				...
				//{				
					...
					if ( pCurVar && *pCurVar == *pVariant )
					{
						...
						componentListBox.saveInv( true );//M9
						status = UP;
					}
					else
					{	
						...
						pCurDialog->setEditText(pVariant->getName()); //no good crush
						//magic 09052012 end
						componentListBox.saveInv( true );//M9
					}
				//}
					costOfChange = 0; //magic 07052012
			}
			break;
		case MB_MSG_PREV: // actually cancel
			LogisticsData::instance->cancelMechModfications();
			componentListBox.saveInv( false );//M9
			pVariant = NULL;
			costOfChange = 0; //magic 07052012
			status = UP;
			break;
		...
		case	COMPONENT_FORM_WEAPON_BALLISTIC:
		case	COMPONENT_FORM_WEAPON_MISSILE:
		//case	COMPONENT_FORM_AMMO: //magic 08022012
			for ( i = COMPONENT_FORM_WEAPON_ENERGY; i < COMPONENT_FORM_JUMPJET + 1; i++ )
			{
				aButton* pButton = getButton( i );
				if ( pButton )
			...
			componentListBox.setType2( who, -1, -1, -1, -1, -1); //magic 08022012
		...
		case	COMPONENT_FORM_JUMPJET:
			...
			componentListBox.setType2( COMPONENT_FORM_JUMPJET, COMPONENT_FORM_BULK, COMPONENT_FORM_HEATSINK, COMPONENT_FORM_AMMO, COMPONENT_FORM_ECM, COMPONENT_FORM_PROBE ); //magic 08022012
			break;
		...
		case MB_MSG_ADD:
			...
			long y = -1;
			//Mbegin
			//componentListBox.removeIComponent( componentListBox.getComponent() );
			//Mend
			//magic 19122010 begin
			//costOfChange += (componentListBox.getCostOfSelected() / 10); //works ok
			//magic 19122010 end
			addComponent( componentListBox.getComponent(), x, y );
			//Magic Begin remove component from list box
					//bool bDel = true;
					//ComponentListItem* pNItem = new ComponentListItem( componentListBox.getComponent() );
					//ComponentListItem* pNItem = new ComponentListItem( pSelectedComponent );
					//componentListBox.removeComponent( pSelectedComponent );// , bDel );
					//componentListBox.update(); Mozda ukljuciti
			// Magic End
			}
			break;
		...
		case MB_MSG_REMOVE:
			if ( pSelectedComponent && (selI != -1 && selJ != -1) )
			{
				...
				//Magic End
				//magic19122010 begin
				//costOfChange += (pSelectedComponent->getCost() / 10); //works ok
				//magic19122010 end
				pSelectedComponent = 0;
				if ( -1 == selectFirstDiagramComponent() )
					selectFirstLBComponent();
		...
		//magic 07052012 begin
		case MB_MSG_REPAIR__ALL:
			{
				costOfChange += cMech->repairInternalPoints() * 200;//magic 12052012
				costOfChange += cMech->repairArmorPoints() * 50; //magic 12052012
			}
			break;
		case MB_MSG_REPAIR__INT:
			{
				costOfChange += cMech->repairInternalPoints() * 200; //magic 12052012
			}
			break;
		case MB_MSG_REPAIR__ARM:
			{
				costOfChange += cMech->repairArmorPoints() * 50; //magic 12052012
			}
			break;
...
void	MechLabScreen::setComponent( LogisticsComponent* pComponent, bool bMessageFromLB )
{
	...
	if ( pCurComponent )
	{
		...
		if ( pCurComponent->getType() == COMPONENT_FORM_JUMPJET )
		{
			...
			//statics[56].showGUIWindow( 0 ); //Magic 10022012 Ammo icon
			//statics[16].showGUIWindow( 0 ); //Magic 18022012 Ammo icon
			statics[13].showGUIWindow( 0 ); //Magic 18022012 Ammo icon
			// set up jump range
		...
		else if ( pCurComponent->getType() == COMPONENT_FORM_HEATSINK )
		{
			...
			statics[7].showGUIWindow( 0 ); //magic 18022012

			textObjects[15].setText( "" );

			// jump range
			//statics[48].showGUIWindow( 0 );
			statics[8].showGUIWindow( 0 ); //magic 18022012
			// armor
			//statics[49].showGUIWindow( 0 );
			statics[9].showGUIWindow( 0 ); //magic 18022012
			// damage
			//statics[30].showGUIWindow( 0 );
			statics[2].showGUIWindow( 0 ); //magic 18022012
			// heat
			//statics[55].showGUIWindow( 1 );	
			//statics[15].showGUIWindow( 1 );	 //magic 18022012
			statics[12].showGUIWindow( 1 );	 //magic 18022012
		...
		{	
			...
			statics[9].showGUIWindow( 1 ); //magic 18022012
			// damage
			//statics[30].showGUIWindow( 0 );
			statics[2].showGUIWindow( 0 ); //magic 18022012
		...
		{	
			...
			//statics[56].showGUIWindow( 1 ); //Magic 10022012 Ammo icon
			//statics[16].showGUIWindow( 1 ); //Magic 18022012 Ammo icon
			statics[13].showGUIWindow( 1 ); //Magic 18022012 Ammo icon

			sprintf( text, "%.1lf", pCurComponent->getRecycleTime() );
			textObjects[7].setText( text );
			textObjects[7].setColor( 0xff005392 );
			textObjects[8].setText( "" );
			textObjects[9].setText( "" );
			...
			rects[16].setHelpID( IDS_HELP_MC_AMMO );
			rects[17].setHelpID( 0 );
			rects[18].setHelpID( 0 );
			rects[19].setHelpID( 0 );
			rects[20].setHelpID( 0 );
		...
		{	
			...
			//statics[56].showGUIWindow( 0 ); //Magic 10022012 Ammo icon
			//statics[29].showGUIWindow( 1 ); //Magic 10022012 range icon
			//statics[47].showGUIWindow( 0 ); //Magic 10022012 range icon
			//magic 18022012 begin
			//statics[16].showGUIWindow( 0 ); //Magic 10022012 Ammo icon
			statics[13].showGUIWindow( 0 ); //Magic 10022012 Ammo icon
			statics[1].showGUIWindow( 1 ); //Magic 10022012 range icon
			statics[7].showGUIWindow( 0 ); //Magic 10022012 range icon
			//magic 18022012 end
...
void MechLabScreen::showJumpJetItems( bool bShow )
{
		...
		//statics[49].showGUIWindow( 0 );
		//magic 18022012 begin
		statics[7].showGUIWindow( bShow );
		statics[8].showGUIWindow( bShow );
		statics[9].showGUIWindow( 0 );
		//magic 18022012 end
		//statics[16].showGUIWindow( bShow ); //Magic 10022012 Ammo icon
		statics[13].showGUIWindow( bShow ); //Magic 10022012 Ammo icon
		attributeMeters[4].showGUIWindow( bShow );
		attributeMeters[0].showGUIWindow( 1 );
		//statics[30].showGUIWindow( !bShow ); // damage
		statics[2].showGUIWindow( !bShow ); // magic 18022012 damage
		...
		//statics[15].showGUIWindow( 0 ); //magic 18022012
		statics[12].showGUIWindow( 0 ); //magic 18022012
		
		// turn off weapon stuff
		//for ( int i = 31; i < 35; i++ )
		for ( int i = 3; i < 7; i++ ) //magic 18022012
		{
...
int MechLabScreen::addComponent( LogisticsComponent* pComponent, long& x, long& y )
{
	...
	if ( pComponent )
	{
		retVal = pVariant->canAddComponent( pComponent, x, y ); //M8
		//retVal = pVariant->addComponent( pComponent, x, y );
		//Mbegin
		if (retVal == 0)//M8
		{//M8
			pVariant->addComponent( pComponent, x, y );//M8
...
void MechLabScreen::endDrag( )
{
	...
	if ( pDragComponent && rects[13].pointInside( mouseX, mouseY ) )
	{
		...
		if ( x == -2 && pDragComponent->getType() == COMPONENT_FORM_JUMPJET )
		{
			...
			y = -2;
			//Mbegin
			//componentListBox.removeIComponent( pDragComponent );
			//Mend
			if ( bDragFromMech )
				addComponentFromDrag( pDragComponent, x, y );
			else
				addComponent( pDragComponent, x, y );
			//componentListBox.update(); //M5 no good
			// MAGIC begin
			//bool bDel = true;
			//ComponentListItem* pNItem = new ComponentListItem( pDragComponent );
			//componentListBox.RemoveItem( pNItem, bDel );
			//componentListBox.update();
			//Magic End
		}
		else
		{	
			if ( bDragFromMech )
			{
				if ( NO_ERR == addComponentFromDrag( pDragComponent, x, y) )
				{
					componentListBox.SelectItem( -1 );
					selI = x;
					selJ = y;
			...
			{
				if ( NO_ERR == addComponentFromDrag( pDragComponent, selI, selJ) )
				{
					componentListBox.SelectItem( -1 );
					pSelectedComponent = pDragComponent;
				}
			...
			/*
			if (bDragFromList) // if drag from componentlistbox put it back to list
			{
				if ( NO_ERR == addComponentListFromDrag( pDragComponent ))
				{
					componentListBox.SelectItem( -1 );
					pSelectedComponent = pDragComponent;
				}
	...
	{
		//Mbegin
		//if ( (pDragComponent) && (bDragFromList) )
		if ( bDragFromMech )
		{
			//getMouseDiagramCoords( selRect->left(), selRect->top(), x, y );
			if ( NO_ERR == addComponentListFromDrag( pDragComponent ))
			{
				//addComponentListFromDrag( pDragComponent );
				componentListBox.SelectItem( -1 );
				pSelectedComponent = pDragComponent;
				//componentListBox.reType();
			}
	...
	bDragFromList = 0;
	bDragFromMech = 0;
	//componentListBox.reType(); //M not here
}
...
// Magic begin put component in component list box
	//if ((!bDragLeft) && (!bDragFromList))
		//LogisticsData::instance->addComponentToInventory( pComp ); //M9 OK
		componentListBox.addIComponent( pComp );
		componentListBox.reType(); //M7 
...
void MechLabScreen::setDragFromList( bool dfl )
{
	bDragFromList = dfl;
	bDragFromMech = !dfl;
...
void MechLabScreen::removeComponentToDrag( long i, long j )
{
	LogisticsVariant* pVariant = cMech->getVariant(); //magic 12052012
	long tmpX, tmpY;
	LogisticsComponent* pComp = pVariant->getCompAtLocation(i, j, tmpX, tmpY );
...
// Magic begin put component in component list box
	//pDragComponent = pComp;
	//if ((!bDragLeft) && (!bDragFromList))
	//LogisticsData::instance->addComponentToInventory( pComp );
...
int MechLabScreen::addComponentFromDrag( LogisticsComponent* pComponent, long& x, long& y )
{
	if ( !pComponent )
		return -1;
	...
	if ( pComponent )
	{
		retVal = pVariant->addComponent( pComponent, x, y );
		//Mbegin
		//if ( (!bDragLeft) && (!bDragFromList)) //m3
			//LogisticsData::instance->removeComponentFromInventory( pComponent );
			//componentListBox.Update();
		//Mend
	}
	//componentListBox.reType();
...
void MechLabScreen::removeComponentListToDrag(LogisticsComponent* pComponent )
{
	//LogisticsData::instance->removeComponentFromInventory( pComponent ); //M9 OK
	componentListBox.removeIComponent( pComponent );
	componentListBox.reType();
...
int MechLabScreen::addComponentListFromDrag( LogisticsComponent* pComponent )
{
	if ( !pComponent )
		return -1;
...
//void MechLabScreen::getCinventory( LogisticsComponent** pComp, int& maxCount )
//{
//	pComp = Cinventory;
//	maxCount = MaxCinventory;
//
//}

/*void MechLabScreen::buttonAddPressed( bool p1 )
{
