...
//OLD One... In case someone changes their mind.  I know that never happens!! activated by MAGIC
...
float tonnageTurnRate[110] = 
{
	...
	120.0f,120.0f,120.0f,120.0f,120.0f,120.0f,120.0f,120.0f,120.0f,120.0f, //80 through 89 tons
	 ...
	 90.0f, 90.0f, 90.0f, 90.0f, 90.0f, 90.0f, 90.0f, 90.0f, 30.0f, 30.0f //100 through 109 tons
};


// New one desabled by MAGIC
/*
float tonnageTurnRate[110] = 
{
...
#define ARMOR_HEAD_PERCENT						0.04f //0.02f //magic 14092011
#define ARMOR_CTORSO_PERCENT					0.17f
#define ARMOR_RTORSO_PERCENT					0.12f //0.125f magic 14092011
#define ARMOR_LTORSO_PERCENT					0.12f //0.125f magic 14092011
#define ARMOR_LARM_PERCENT						0.1f //0.12f
#define ARMOR_RARM_PERCENT						0.1f //0.12f
#define ARMOR_LLEG_PERCENT						0.12f //0.13f
#define ARMOR_RLEG_PERCENT						0.12f //0.13f
#define ARMOR_RCTORSO_PERCENT					0.05f //0.03f
#define ARMOR_RLTORSO_PERCENT					0.03f //0.02f
#define ARMOR_RRTORSO_PERCENT					0.03f //0.02f
//---------------------------------------------------------------------------
...
long BattleMechType::init (FilePtr objFile, unsigned long fileSize) 
{
	...
	if (result != NO_ERR)
		return(result);
//Magic - ovde cita iz object2.pak
	...
	//mechCSVName.init(objectPath,fName,".csv");
...
long BattleMechType::init (FilePtr objFile, unsigned long fileSize, const char* vehicleName) 
{
	long result = 0;
	//float tonnage = 25;
	...
	//MtestFile.open("data\\mech_load.fit", CREATE);
		...
		//return(result);
	{
		...
		mechCSVName.init(objectPath,fileType,".csv");
		appearName = (char *)ObjectTypeManager::objectTypeCache->Malloc(strlen(fileType)+1);
		//appearName = (char *)gos_Malloc(strlen(fileType)+1); //magic 25032012
		strcpy(appearName,fileType);
	}	
	else
	{
		moveType = MOVETYPE_GROUND;
		...
		//explDmg = 20.0f;
		//explRad = 80.0f;
		mechCSVName.init(objectPath,vehicleName,".csv");
		appearName = (char *)ObjectTypeManager::objectTypeCache->Malloc(strlen(vehicleName)+1);
		//appearName = (char *)gos_Malloc(strlen(vehicleName)+1); //magic 25032012
		strcpy(appearName,vehicleName);
	}
	//--------------------------------------------------------------
	// New from here on down.  Now gets data from Mech CSV file.
	...
	//magic read tonnage and set destruct dam and expl. dam
	mechCSVFile.readFloat(3,5,destructDamage);
	explDmg = destructDamage / 2.0f;
	explRad = 80.0f;
	mechCSVFile.readLong(1,3,moveType);
	if ((moveType != MOVETYPE_GROUND) && (moveType != MOVETYPE_AIR))
		moveType = MOVETYPE_GROUND;
	//------------------------------------------------------------------
	// Now, read in the max internal structure for each body location...
	// Reader reads rows from 1 just like spreadsheet.
...
long BattleMech::init (DWORD variantNum)
{
	...
	name[MAXLEN_MOVER_NAME-1] = NULL; //magic 29112011
	//magic 12092011 begin
	appearance->setMechName(thisMechName); //magic 12092011
	strcpy_s(name, thisMechName);//magic 12092011 jedno od ova 2
	//magic 12092011 end
	...
	sprintf( longName, "%s %.0ft 'Mech", thisMechName, tonnage );
...
*/
//magic 01082011 begin
	mechFile->readFloat(18,2,armor[MECH_ARMOR_LOCATION_HEAD].maxArmor);
	mechFile->readFloat(18,7,armor[MECH_ARMOR_LOCATION_CTORSO].maxArmor);
	mechFile->readFloat(18,5,armor[MECH_ARMOR_LOCATION_LTORSO].maxArmor);
	mechFile->readFloat(18,6,armor[MECH_ARMOR_LOCATION_RTORSO].maxArmor);
	mechFile->readFloat(18,3,armor[MECH_ARMOR_LOCATION_LARM].maxArmor);
	mechFile->readFloat(18,4,armor[MECH_ARMOR_LOCATION_RARM].maxArmor);
	mechFile->readFloat(18,8,armor[MECH_ARMOR_LOCATION_LLEG].maxArmor);
	mechFile->readFloat(18,9,armor[MECH_ARMOR_LOCATION_RLEG].maxArmor);
	mechFile->readFloat(18,12,armor[MECH_ARMOR_LOCATION_RCTORSO].maxArmor);
	mechFile->readFloat(18,10,armor[MECH_ARMOR_LOCATION_RLTORSO].maxArmor);
	mechFile->readFloat(18,11,armor[MECH_ARMOR_LOCATION_RRTORSO].maxArmor);
//magic 01082011 end
/*
...
*/
//magic 14092011 begin
	mechFile->readFloat(115 + (97 * variantNum),2,armor[MECH_ARMOR_LOCATION_HEAD].curArmor);
	mechFile->readFloat(115 + (97 * variantNum),7,armor[MECH_ARMOR_LOCATION_CTORSO].curArmor);
	mechFile->readFloat(115 + (97 * variantNum),5,armor[MECH_ARMOR_LOCATION_LTORSO].curArmor);
	mechFile->readFloat(115 + (97 * variantNum),6,armor[MECH_ARMOR_LOCATION_RTORSO].curArmor);
	mechFile->readFloat(115 + (97 * variantNum),3,armor[MECH_ARMOR_LOCATION_LARM].curArmor);
	mechFile->readFloat(115 + (97 * variantNum),4,armor[MECH_ARMOR_LOCATION_RARM].curArmor);
	mechFile->readFloat(115 + (97 * variantNum),8,armor[MECH_ARMOR_LOCATION_LLEG].curArmor);
	mechFile->readFloat(115 + (97 * variantNum),9,armor[MECH_ARMOR_LOCATION_RLEG].curArmor);
	mechFile->readFloat(115 + (97 * variantNum),12,armor[MECH_ARMOR_LOCATION_RCTORSO].curArmor);
	mechFile->readFloat(115 + (97 * variantNum),10,armor[MECH_ARMOR_LOCATION_RLTORSO].curArmor);
	mechFile->readFloat(115 + (97 * variantNum),11,armor[MECH_ARMOR_LOCATION_RRTORSO].curArmor);
//magic 14092011 end
	//---------------------------------------------------------------------------
...
//Magic 77 begin moved from below
...
//Magic 77 end
		...
		if (inventory[realItemNum].masterID && (inventory[realItemNum].masterID != 255))
		{
			...
			switch (MasterComponent::masterList[inventory[realItemNum].masterID].getForm()) 
			{
				case COMPONENT_FORM_JUMPJET:
					//numJumpJets = 5; 
					numJumpJets = (5 - chassisClass) + 2; //Magic 77
				...
				case COMPONENT_FORM_BULK:		//Special case now.
					...
					//numArmorComponents++;
					numArmorComponents += (long)MasterComponent::masterList[inventory[realItemNum].masterID].getArmorPoints(); //magic 14092011
				break;
		...
		if (inventory[realItemNum].masterID && (inventory[realItemNum].masterID != 255))
		{
			...
			switch (MasterComponent::masterList[inventory[realItemNum].masterID].getForm()) 
			{
				...
				case COMPONENT_FORM_WEAPON_MISSILE:
					...
					inventory[realItemNum].effectiveness *= (short)MasterComponent::masterList[inventory[realItemNum].masterID].getMaxWeaponRange() / 24; //magic 11022012
					ItemLocationToInvLocation[curItem] = realItemNum;
					realItemNum++;
					//-------------------------------------
	...
	for (long curLocation = 0; curLocation < NUM_MECH_BODY_LOCATIONS; curLocation++) 
	{
		...
		//magic 14092011 begin
		mechFile->readFloat(114 + (97 * variantNum), stupidNewSageColumns[curLocation], body[curLocation].curInternalStructure);
		//magic 14092011 end
		...
		for (long curSpace = 0; curSpace < numSpaces; curSpace++)
		{
			...
			//body[curLocation].criticalSpaces[curSpace].hit = true;				//Everything is NOT always repaired now.			
			...
			if (spaceData < 255)
			{
				...
				switch (MasterComponent::masterList[inventory[spaceData].masterID].getForm())
				{
					...
					case COMPONENT_FORM_PROBE:
						{
						probe = spaceData;
						//magic 09022012 begin
						if (sensorSystem)
						{
							//sensorSystem->setRange(MasterComponent::masterList[inventory[sensor].masterID].getSensorRange());
							float sensRange = MasterComponent::masterList[inventory[sensor].masterID].getSensorRange();
							//float probeFactor = MasterComponent::masterList[inventory[sensor].masterID].getEcmEffect();
							float probeFactor = MasterComponent::masterList[inventory[spaceData].masterID].getProbeEffect();
							sensorSystem->setRange(sensRange * probeFactor );
							//sensorSystem->setHasProbe(true ); //magic 13022012
						}
						//magic 09022012 end
						}
						break;
	...
	if (numArmorComponents)
	{
		...
		float totalArmorPoints = numArmorComponents; //magic 14092011
		...
		armor[MECH_ARMOR_LOCATION_HEAD].curArmor += armorPercent; //magic 14092011
		...
		armor[MECH_ARMOR_LOCATION_CTORSO].curArmor += armorPercent; //magic 14092011
		...
		armor[MECH_ARMOR_LOCATION_LTORSO].curArmor += armorPercent; //magic 14092011
		...
		armor[MECH_ARMOR_LOCATION_RTORSO].curArmor += armorPercent; //magic 14092011
		...
		armor[MECH_ARMOR_LOCATION_LARM].curArmor += armorPercent; //magic 14092011
		...
		armor[MECH_ARMOR_LOCATION_RARM].curArmor += armorPercent; //magic 14092011
		...
		armor[MECH_ARMOR_LOCATION_LLEG].curArmor += armorPercent; //magic 14092011
		...
		armor[MECH_ARMOR_LOCATION_RLEG].curArmor += armorPercent; //magic 14092011
		...
		armor[MECH_ARMOR_LOCATION_RCTORSO].curArmor += armorPercent; //magic 14092011
		...
		armor[MECH_ARMOR_LOCATION_RLTORSO].curArmor += armorPercent; //magic 14092011
		...
		armor[MECH_ARMOR_LOCATION_RRTORSO].curArmor += armorPercent; //magic 14092011
	}
	...
	/*//-----------------------------
	// Calc mech's chassis class...
	chassisClass = getMechClass();*/ //Magic 77 moved above
...
//----------------------------------------------------------------------------------
void BattleMech::resetCurInternalPoints (float *internalArray)
{
		for (int i=0; i<8; i++)
		body[i].curInternalStructure = internalArray[i];
		//? allow mech with damaged internal structure into mission - NO check for internal damage before adding to force group
}

//----------------------------------------------------------------------------------
void BattleMech::resetCurArmorPoints (float *armorArray)
{

	/*for (int i=0; i<11; i++)
		armor[i].curArmor = armorArray[i];
	//--------------------------------------------------------------------------
	//If we added Armor to the mech, reset current armor points
	if (numArmorComponents)
	{
		//float totalArmorPoints = ARMOR_POINTS_PER_NEWARMOR * numArmorComponents;
		float totalArmorPoints = numArmorComponents; //magic 14092011
		...
		armor[MECH_ARMOR_LOCATION_HEAD].curArmor -= armorArray[MECH_ARMOR_LOCATION_HEAD];
		armor[MECH_ARMOR_LOCATION_CTORSO].curArmor -= armorArray[MECH_ARMOR_LOCATION_CTORSO];
		armor[MECH_ARMOR_LOCATION_LTORSO].curArmor -= armorArray[MECH_ARMOR_LOCATION_LTORSO];
		armor[MECH_ARMOR_LOCATION_RTORSO].curArmor -= armorArray[MECH_ARMOR_LOCATION_RTORSO];
		armor[MECH_ARMOR_LOCATION_LARM].curArmor -= armorArray[MECH_ARMOR_LOCATION_LARM];
		armor[MECH_ARMOR_LOCATION_RARM].curArmor -= armorArray[MECH_ARMOR_LOCATION_RARM];
		armor[MECH_ARMOR_LOCATION_LLEG].curArmor -= armorArray[MECH_ARMOR_LOCATION_LLEG];
		armor[MECH_ARMOR_LOCATION_RLEG].curArmor -= armorArray[MECH_ARMOR_LOCATION_RLEG];
		armor[MECH_ARMOR_LOCATION_RCTORSO].curArmor -= armorArray[MECH_ARMOR_LOCATION_RCTORSO];
		armor[MECH_ARMOR_LOCATION_RLTORSO].curArmor -= armorArray[MECH_ARMOR_LOCATION_RLTORSO];
		armor[MECH_ARMOR_LOCATION_RRTORSO].curArmor -= armorArray[MECH_ARMOR_LOCATION_RRTORSO];
}
//magic 05052012 end
//----------------------------------------------------------------------------------
void BattleMech::resetComponents (long totalComponents, long *componentList)
{
	//magic 19022012 ovde napraviti da se cita iz logistics kad napravim lokacije na prave lokacije
	/*MECH_BODY_LOCATION_LTORSO, MECH_BODY_LOCATION_CTORSO, MECH_BODY_LOCATION_RTORSO, MECH_BODY_LOCATION_RARM, MECH_BODY_LOCATION_LARM*/
	//------------------------------------------------------------
	...
	for (long newItems = 0;newItems < totalComponents;newItems++)
	{
		switch (MasterComponent::masterList[componentList[newItems]].getForm())
		{
			...
			case COMPONENT_FORM_BULK:
				//numNewArmors++;
				numNewArmors += (long)MasterComponent::masterList[componentList[newItems]].getArmorPoints(); //magic 14092011
			break;			
	...
	if (numNewArmors)
	{
		...
		float totalArmorPoints = numNewArmors; //magic 14092011
...
*/
//magic 01082011 begin
	result = mechFile->readIdFloat("Head", armor[MECH_ARMOR_LOCATION_HEAD].maxArmor);
	if (result != NO_ERR)
		return(result);

	result = mechFile->readIdFloat("CenterTorso", armor[MECH_ARMOR_LOCATION_CTORSO].maxArmor);
	if (result != NO_ERR)
		return(result);

	result = mechFile->readIdFloat("LeftTorso", armor[MECH_ARMOR_LOCATION_LTORSO].maxArmor);
	if (result != NO_ERR)
		return(result);

	result = mechFile->readIdFloat("RightTorso", armor[MECH_ARMOR_LOCATION_RTORSO].maxArmor);
	if (result != NO_ERR)
		return(result);

	result = mechFile->readIdFloat("LeftArm", armor[MECH_ARMOR_LOCATION_LARM].maxArmor);
	if (result != NO_ERR)
		return(result);

	result = mechFile->readIdFloat("RightArm", armor[MECH_ARMOR_LOCATION_RARM].maxArmor);
	if (result != NO_ERR)
		return(result);

	result = mechFile->readIdFloat("LeftLeg", armor[MECH_ARMOR_LOCATION_LLEG].maxArmor);
	if (result != NO_ERR)
		return(result);

	result = mechFile->readIdFloat("RightLeg", armor[MECH_ARMOR_LOCATION_RLEG].maxArmor);
	if (result != NO_ERR)
		return(result);

	result = mechFile->readIdFloat("RearCenterTorso", armor[MECH_ARMOR_LOCATION_RCTORSO].maxArmor);
	if (result != NO_ERR)
		return(result);

	result = mechFile->readIdFloat("RearLeftTorso", armor[MECH_ARMOR_LOCATION_RLTORSO].maxArmor);
	if (result != NO_ERR)
		return(result);

	result = mechFile->readIdFloat("RearRightTorso", armor[MECH_ARMOR_LOCATION_RRTORSO].maxArmor);
	if (result != NO_ERR)
		return(result);
//magic 01082011 end
	...
	while (curItem < (numOther + numWeapons)) {
		...
		//inventory[curItem].effectiveness *= (short)MasterComponent::masterList[inventory[curItem].masterID].getMaxWeaponRange() / 24; //magic 11022012
...
void BattleMech::pilotingCheck (unsigned long situation, float modifier) {
	...
	float pilotRoll = RandomNumber(80); //magic was 100 changed to 80 22112010

	//--------------------
	// Mech's situation...
	if (situation & PILOTCHECK_SITUATION_JUMPING)//Magic 55
		pilotRoll += 20.0;//Magic 55
...
//#if 0
	...
	{
		failedPilotingCheck = (pilotRoll >= pilot->getSkill(MWS_JUMPING));
		//SkillTry[MWS_JUMPING]++;//M56
		pilot->skillPoints[MWS_JUMPING] += 0.15f;//SkillTry[MWS_JUMPING];
		if (!failedPilotingCheck)
		{
			//SkillSuccess[MWS_JUMPING]++;
			pilot->skillPoints[MWS_JUMPING] += 0.3f;//SkillSuccess[MWS_JUMPING];
		}
	}
	else
//#endif
	{
		...
		//SkillTry[MWS_PILOTING]++;
		pilot->skillPoints[MWS_PILOTING] += 0.1f;//SkillTry[MWS_PILOTING];
		if (!failedPilotingCheck)
		{
			//SkillSuccess[MWS_PILOTING]++;
			pilot->skillPoints[MWS_PILOTING] += 0.2f;//SkillSuccess[MWS_PILOTING];
		}
	}
}
...
//Magic 51
		//---------------------------------------------
	if (distanceFrom(jumpGoal) > 8.0) //52
		{ //52
		float turnRate = 0.0;
		float relFacing = relFacingTo(jumpGoal, MECH_BODY_LOCATION_LLEG);
		if ((relFacing < -2.0) || (relFacing > 2.0)) 
		{
			//-----------------------------------------------
			// We can and will shift facing to destination...
			turnRate = -relFacing;//magic 51 * frameLength;
			float maxRate;
			if ((tonnage) < 40)//magic 52
				maxRate = tonnageTurnRate[long(tonnage)] * frameLength;
			else
				maxRate = tonnageTurnRate[39] * frameLength;
			//float maxRate = 360.0f * frameLength; //Magic 52 no good
			if (turnRate < 0.0f)
				maxRate = -maxRate;
			...
			control.settings.mech.rotate = maxRate;			
			//control.settings.mech.facingRotate = turnRate; //Magic 51 no
		}
		}//52
//Magic 51
	...
	else if (!appearance->isInJump()) 
	{
		...
		//pilot->incNumSkillUses(COMBAT_STAT_MISSION, MWS_JUMPING); //M56
		//Magic 56 begin
		float pilotRoll = RandomNumber(70); //magic was 80 changed to 70 18112010
		//pilotRoll += 20.0;
		if ((body[MECH_BODY_LOCATION_RLEG].curInternalStructure == 0) || (body[MECH_BODY_LOCATION_LLEG].curInternalStructure == 0))
			pilotRoll += 100.0;
		failedPilotingCheck = (pilotRoll >= pilot->getSkill(MWS_JUMPING));
		failedLastJump = failedPilotingCheck;
		pilot->skillPoints[MWS_JUMPING] += 0.1f;
		if (!failedPilotingCheck)
		{
			pilot->skillPoints[MWS_JUMPING] += 0.2f;
		}
		//else //else grana radi samo nece da padne
			//pilot->radioMessage(RADIO_AIRSTRIKE,true); 
			//control.settings.mech.gestureGoal = 7; //M57 ne radi
			//appearance->setGestureGoal(7); //m57 ne radi

		//Magic 56 end
		//failedPilotingCheck = true; //M57 nema uticaja
		//pilotingCheck(PILOTCHECK_SITUATION_JUMPING); //M57 disabled
...
bool BattleMech::pivotTo (void) {
			...
			if ((path->numStepsWhenNotPaused > 0) && (path->curStep < path->numStepsWhenNotPaused)) 
			{
				...
				if ((relFacingToWayPt < -5.0) || (relFacingToWayPt > 5.0)) {
					...
					if (getMoveType() == MOVETYPE_AIR)
						maxRate = tonnageTurnRate[10] * frameLength;
					else
						maxRate = tonnageTurnRate[long(tonnage)] * frameLength;
					//Magic 52 end
					//float maxRate = tonnageTurnRate[long(tonnage)] * frameLength; //Magic 52 disabled
					if (turnRate < 0.0f)
						maxRate = -maxRate;
					...
					/*if (getMoveType() == MOVETYPE_AIR) // Magic 68
						control.settings.mech.pivot = false;//Magic 68
					else //Magic 68 */ //magic 17122011 disabled
	...
	else if (moveState == MOVESTATE_PIVOT_REVERSE) //Magic 68 disabled
	//else if ((moveState == MOVESTATE_PIVOT_REVERSE)  && (getMoveType() != MOVETYPE_AIR)) //Magic 68 //magic 17122011 disabled
	{
		if ((moveStateGoal == MOVESTATE_PIVOT_REVERSE) || (moveStateGoal == MOVESTATE_REVERSE)) 
		{
			...
			if ((path->numStepsWhenNotPaused > 0) && (path->curStep < path->numStepsWhenNotPaused)) 
			{
				...
				if ((relFacingToWayPt > -175.0) && (relFacingToWayPt < 175.0)) {
					...
					float maxRate;
					if (getMoveType() == MOVETYPE_AIR)
						maxRate = tonnageTurnRate[10] * frameLength; //Magic 70 bilo 109
					else
						maxRate = tonnageTurnRate[long(tonnage)] * frameLength;
					//Magic 52 end
					//float maxRate = tonnageTurnRate[long(tonnage)] * frameLength;//Magic 52 disabled
					...
					if (moveStateGoal == MOVESTATE_REVERSE) //Magic 68 disabled
					//if ((moveStateGoal == MOVESTATE_REVERSE) && (getMoveType() != MOVETYPE_AIR)) //Magic 68 //magic 17122011 disabled
			...
			if (path->curStep == path->numSteps)
				...
				if (!goalReached) 
				{
					...
					if (moveState == MOVESTATE_FORWARD) 
					{
						...
						else if (moveStateGoal == MOVESTATE_REVERSE) //Magic 68 disabled
						//else if ((moveStateGoal == MOVESTATE_REVERSE) && (getMoveType() != MOVETYPE_AIR)) //Magic 68 //magic 17122011 disabled
						{
							//---------------------------------------------------
						...
						else if (moveStateGoal == MOVESTATE_PIVOT_REVERSE) //Magic 68 disabled
						//else if ((moveStateGoal == MOVESTATE_PIVOT_REVERSE) && (getMoveType() != MOVETYPE_AIR)) //Magic 68 //magic 17122011 disabled
						{
							//---------------------------------------------------
							// Stop, and pivot fully reverse to next path step...
							//((MechActor*)appearance)->forceStop();
					...
					else if (moveState == MOVESTATE_REVERSE) //Magic 68 disabled
					//else if ((moveState == MOVESTATE_REVERSE)  && (getMoveType() != MOVETYPE_AIR)) //Magic 68 //magic 17122011 disabled
					{
						if (moveStateGoal == MOVESTATE_FORWARD) 
						{
							//---------------------------------------------------
					...
					else 
					{
						...
						// Magic 68 da li i ovde postaviti
						{
							//((MechActor*)appearance)->forceStop();
							pilot->pausePath(); //pilot->getMovePath()->numSteps = 0;
							newMoveState = MOVESTATE_PIVOT_REVERSE;
...
void BattleMech::updateTorso (float newRotatePerSec) 
{
	...
	float fireArc = getFireArc();
	//if ((torsoRelFacing < -fireArc) || (torsoRelFacing > fireArc)) //magic 100 disabled
...
{
	//Magic 55 begin
	long myTeamContactList[255];
	long numMyTeamContacts;
	long myNewContactList[64];
	long numMyNewContacts;
	numMyTeamContacts = sensorSystem->getTeamContacts(myTeamContactList, CONTACT_CRITERIA_SENSOR, CONTACT_SORT_NONE);
	//Magic 55 end
...
// TEMP BLOCK OUT FOR MC2 port...
	...
	//---------------------------------------------------------------------------------
	//Magic 55 begin
	if ((pilot->onHomeTeam()) && (sensorSystem) && (!sensorSystem->broken))//magic 57 added last
	{
		numMyNewContacts = sensorSystem->scanNewContact3(myNewContactList);
		if (numMyNewContacts > 0)
		{
			for (int i = 0; i < numMyNewContacts; i++)
			{
				for (int j = 0; j < numMyTeamContacts; j++)
				{
					if (myNewContactList[i] == myTeamContactList[j])
						return(1);
				}
			}
	//Magic 55 end
	//Magic 53 begin
	//M55 if ((pilot->onHomeTeam()) && (sensorSystem))
	//M55 {
		//M55 MoverPtr myNewContact = sensorSystem->scanNewContact();

		//M55 if (myNewContact)
			//pilot->radioMessage(RADIO_SENSOR_CONTACT,true);//Magic 57 disabled
		//magic 55 begin
			float pilotRoll = RandomNumber(70); //magic was 80 changed to 70 18112010

			pilot->incNumSkillUses(COMBAT_STAT_MISSION, MWS_SENSORS);//M56
			bool failedSensorCheck = (pilotRoll >= pilot->getSkill(MWS_SENSORS));
			//SkillTry[MWS_SENSORS]++; //M56
			//pilot->skillPoints[MWS_SENSORS] += SkillTry[MWS_SENSORS];
			if (!failedSensorCheck)
			{
				pilot->incNumSkillSuccesses(COMBAT_STAT_MISSION, MWS_SENSORS);//M56
				pilot->radioMessage(RADIO_SENSOR_CONTACT,true);//Magic 57
				//SkillSuccess[MWS_SENSORS]++;
				//pilot->skillPoints[MWS_SENSORS] += SkillSuccess[MWS_SENSORS];
			}
	...
	if ((failedLastJump))
	{
		//failedPilotingCheck = true; //ne valja ostaje fallen
		//appearance->setGestureGoal(7); //radi ali nije dobro
		//failedLastJump = false;
		if (getMoveType() != MOVETYPE_AIR)
		{
			//long fallGesture = RandomNumber(2) ? MECH_STATE_FALLEN_FORWARD : MECH_STATE_FALLEN_BACKWARD;
			//appearance->setGestureGoal(fallGesture);
			failedPilotingCheck = true; //M59 ovo radi sklonjeno radi probe 2 varijante
			long curStateGoal = appearance->getCurrentGestureGoal();
			if ((curStateGoal == MECH_STATE_FALLEN_FORWARD) || (curStateGoal == MECH_STATE_FALLEN_BACKWARD))
			{
				failedLastJump = false;
				failedPilotingCheck = false;
				//control.settings.mech.gestureGoal = 1;
				...
				//appearance->setGestureGoal(-1);ne radi
			}
			//control.settings.mech.throttle = 100;
			//pilot->clearTacOrderQueue(); //ne radi
			//pilot->rethinkPath(0);//ne radi
		}
	}
	//else
		//if (pilot->getTacOrderQueueLocked()) ne radi
		//	pilot->unlockTacOrderQueue(); ne radi
	...
	if (Terrain::IsGameSelectTerrainPosition(position))
	{
		if (getTeamId() != Team::home->getId())
		{
			...
			if ((cStat == CONTACT_VISUAL) || isDestroyed() || isDisabled() || ShowMovers || (MPlayer && MPlayer->allUnitsDestroyed[MPlayer->commanderID]))
			{
				...
				appearance->setObjectName(name); //magic 12092011
				appearance->setVisibility(true,true);
				appearance->render();
			}
...
float BattleMech::getJumpRange (long* numOffsets, long* jumpCost) {
	...
	//if (getPilot()->isLongJump())
		//numJets++; //Magic 77 disabled obsolete
	if (numOffsets) {
		if (numJets > 5)
...
bool BattleMech::hitInventoryItem (long itemIndex, bool setupOnly) {
	...
	switch (MasterComponent::masterList[masterID].getForm()) {
		...
		case COMPONENT_FORM_ACTUATOR:
			...
			if ((bodyLocation == MECH_BODY_LOCATION_LLEG) || (bodyLocation == MECH_BODY_LOCATION_RLEG))//magic 29112011
	...
	if (getInventoryDamage(itemIndex) >= MasterComponent::masterList[masterID].getDisableLevel()) {
		...
		switch (MasterComponent::masterList[masterID].getForm()) {
			...
			// If you want to destroy the sensor mech, blow it up!
				if (sensorSystem)//Magic 53 activated
					sensorSystem->broken = true;//Magic 53 activated
					//sensorSystem->setRange(MasterComponent::masterList[inventory[sensor].masterID].getSensorRange());
					//SensorManager->removeTeamSensor(teamId, sensorSystem); //ne moze pravi runtime error
					sensorSystem->setShutdown(true);
				break;
			case COMPONENT_FORM_ECM:
#if 0
...
//#pragma optimize("",off) //magic 19022011
...
long BattleMech::handleWeaponHit (WeaponShotInfoPtr shotInfo, bool addMultiplayChunk) 
{
	...
	//magic 06092011 begin	
	// If this was a heavy hit, play the hit gesture.
	if (!MPlayer && (totalHitType == HIT_TYPE_HEAVY) && (getCommanderId() != Commander::home->getId()))
	{
		float pilotRoll = RandomNumber(120); 
		
		if ((chassisClass == MECH_CLASS_LIGHT) || (chassisClass == MECH_CLASS_MEDIUM))
			failedPilotingCheck = true;
		else
			failedPilotingCheck = (pilotRoll >= pilot->getSkill(MWS_PILOTING));
...
void BattleMech::Save (PacketFilePtr file, long packetNum)
{
	...
	file->writePacket(packetNum,(MemoryPtr)&data,sizeof(MechData),STORAGE_TYPE_RAW); //magic 11032012
}
...
//***************************************************************************
float BattleMech::getArmorDamageFactor (void)
{
	float sumCurArmor = 0;
	float sumMaxArmor = 0;
	for (long i=0; i<11; i++)
	{
		sumCurArmor += armor[i].curArmor;
		sumMaxArmor += armor[i].maxArmor;
	}

	return (sumCurArmor/sumMaxArmor);
}

//------------------------------------------------------------
// Now, read in the component layout for each body location...

float BattleMech::getInternalDamageFactor (void)
{
	float sumCurIS = 0;
	float sumMaxIS = 0;
	for (long curLocation = 0; curLocation < NUM_MECH_BODY_LOCATIONS; curLocation++) 
	{
		sumCurIS += body[curLocation].curInternalStructure;
		sumMaxIS += body[curLocation].maxInternalStructure;
	}

	return (sumCurIS/sumMaxIS);
}

//Napraviti getDestroyedWeaponsCost kad se odradi weapon damage transfer.
//magic 25092012 end
//***************************************************************************
