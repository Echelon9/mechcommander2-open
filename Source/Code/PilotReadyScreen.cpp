...
void PilotReadyScreen::init(FitIniFile* file)
{
	...
	attributeMeters[3].init( file, "AttributeMeter3" );
...
void PilotReadyScreen::render(int xOffset, int yOffset )
{
	...
	if ( mechSelected )
	{
		...
		//statics[27].render( xOffset, yOffset ); //magic 19092011 disabled
		//statics[28].render( xOffset, yOffset ); //magic 19092011 disabled
	}
...
void PilotReadyScreen::setPilot( LogisticsPilot* pPilot )
{
	...
	if ( pPilot != pCurPilot )
	{
		...
		statics[28].setTexture( str );
		statics[28].setColor( 0xffffffff );
		statics[28].setUVs( 0, 0, 92, 128 );
		//magic 19092011 end
	}
...
//Magic 55 begin
		char str1[256];
		sprintf( str1, "%ld", (long)pCurPilot->getSensorSkill() );
		textObjects[14].setText( str1 );
		attributeMeters[2].setValue( ((float)pCurPilot->getSensorSkill())/80.f );

		char number1[64];
		sprintf( number1, "%ld", (long)pCurPilot->getJumping() );
		textObjects[15].setText( number1 );
		attributeMeters[3].setValue( pCurPilot->getJumping()/80.f );

		sprintf( number1, "%ld", (long)pCurPilot->getVehicleKills() );
		textObjects[16].setText( number1 );
...
//Magic 55 end
		...
		for ( int i = 0; i < 4; i++ )
		{
			if ( i < count )
			{
				...
				//Magic 81 begin
				int tmpI = skillIDs[i] + IDS_SPECIALTY_HELP1;
				if ((tmpI > 5) && (tmpI < 12))
					tmpI++;
				if ((tmpI > 11) && (tmpI < 22))
					tmpI += 2;
				...
				//Magic 81 end
				//textObjects[9+i].setHelpID( skillIDs[i] + IDS_SPECIALTY_HELP1 ); //Magic 81 disabled
				textObjects[9+i].setHelpID( tmpI );
				...
				//skillIcons[i].setHelpID( skillIDs[i] + IDS_SPECIALTY_HELP1 ); //Magic 81 disabled
				skillIcons[i].setHelpID( tmpI );
