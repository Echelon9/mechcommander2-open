...
void FireType::destroy (void) 
{
	if (fireOffsetX) 
	{
		...
		gos_Free(fireOffsetX); //magic 02102011
		fireOffsetX = NULL;
	}
	...
	if (fireOffsetY) 
	{
		...
		gos_Free(fireOffsetY); //magic 02102011
		fireOffsetY = NULL;
	}
	...
	if (fireDelay) 
	{
		...
		gos_Free(fireDelay); //magic 02102011
		fireDelay = NULL;
	}
	...
	if (fireRandomOffsetX) 
	{
		...
		gos_Free(fireRandomOffsetX); //magic 02102011
	...
	if (fireRandomOffsetY) 
	{
		...
		gos_Free(fireRandomOffsetY); //magic 02102011
	...
	if (fireRandomDelay) 
	{
		...
		gos_Free(fireRandomDelay); //magic 02102011
	...
	else if (carnageType == CARNAGE_EXPLOSION) 
	{
		if (getFlag(OBJECT_FLAG_JUSTCREATED)) 
		{
			...
			if (((ExplosionTypePtr)getObjectType())->duration > 0.0f)
			{
				...
				pointLight = new TG_Light; //magic 02102011
				...
				if (lightId == -1)
				{
					...
					//systemHeap->Free(pointLight);
					delete pointLight; //magic 02102011
					pointLight = NULL;
					duration = 0.0f;
				}
			}
	...
	else if (carnageType == CARNAGE_EXPLOSION) 
	{
		...
		if (pointLight)
		{
			if (eye)
				eye->removeWorldLight(lightId,pointLight);
			//systemHeap->Free(pointLight);
			delete pointLight; //magic 0210201
			pointLight = NULL;
		}
	}
}
...
void Carnage::Save (PacketFilePtr file, long packetNum)
{
	...
	file->writePacket(packetNum,(MemoryPtr)&data,sizeof(CarnageData),STORAGE_TYPE_RAW); //magic 11032012
}
