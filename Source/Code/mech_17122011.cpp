...
#define ARMOR_HEAD_PERCENT						0.02f //magic 14092011
...
bool BattleMech::pivotTo (void) {
	...
	//else if ((moveState == MOVESTATE_PIVOT_REVERSE)  && (getMoveType() != MOVETYPE_AIR)) //Magic 68
	else if (moveState == MOVESTATE_PIVOT_REVERSE) //Magic 08112011
	{
		//magic 08112011 begin
		if (getMoveType() == MOVETYPE_AIR)
			...
			if ((path->numStepsWhenNotPaused > 0) && (path->curStep < path->numStepsWhenNotPaused)) 
			{
				...
				if ((relFacingToWayPt > -175.0) && (relFacingToWayPt < 175.0)) {
					...
					if (fabs(turnRate) > maxRate) {
 					...
 					control.settings.mech.rotate = turnRate;
					if (getMoveType() == MOVETYPE_AIR) // Magic 68
						control.settings.mech.pivot = false;//Magic 68 //ne treba false
					else //Magic 68 
						control.settings.mech.pivot = true;
					...
					if ((moveStateGoal == MOVESTATE_REVERSE) && (getMoveType() != MOVETYPE_AIR)) //Magic 68
		...
		if (moveStateGoal == MOVESTATE_PIVOT_TARGET) {
			...
			//magic 08112011 begin
			if (getMoveType() == MOVETYPE_AIR)
				appearance->setGestureGoal(MECH_STATE_RUNNING);
			else	
			//magic 08112011 end
			appearance->setGestureGoal(MECH_STATE_STANDING);
			control.settings.mech.throttle = 100;
			...
			if ((relFacingToTarget < -fireArc) || (relFacingToTarget > fireArc)) {
				...
				control.settings.mech.rotate = turnRate;
				if (getMoveType() == MOVETYPE_AIR) // Magic 68
					control.settings.mech.pivot = false;//Magic 68 //ne ovako false
				else //Magic 68 
					control.settings.mech.pivot = true;
			...
			if (path->curStep == path->numSteps)
				...
				if (!goalReached) 
				{
					...
					if (moveState == MOVESTATE_FORWARD) 
					{
						...
						//else if ((moveStateGoal == MOVESTATE_REVERSE) && (getMoveType() != MOVETYPE_AIR)) //Magic 68
						else if (moveStateGoal == MOVESTATE_REVERSE) //Magic 08112011
						{
							//---------------------------------------------------
						...
						//else if ((moveStateGoal == MOVESTATE_PIVOT_REVERSE) && (getMoveType() != MOVETYPE_AIR)) //Magic 68
						else if (moveStateGoal == MOVESTATE_PIVOT_REVERSE) //Magic 08112011
						{
							//---------------------------------------------------
							// Stop, and pivot fully reverse to next path step...
							//((MechActor*)appearance)->forceStop();
					...
					//else if ((moveState == MOVESTATE_REVERSE)  && (getMoveType() != MOVETYPE_AIR)) //Magic 68
					{
						if (moveStateGoal == MOVESTATE_FORWARD) 
						{
							//---------------------------------------------------
