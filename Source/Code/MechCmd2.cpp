...
#include <new.h> //magic 27032011
//------------------------------------------------------------------------------------------------------------
// MechCmdr2 Global Instances of Things
//UserHeapPtr systemHeap = NULL;//magic 02102011 disabled
//UserHeapPtr guiHeap = NULL; //magic 03102011 disabled
...
long resolution = 0;
//long logisticResolution = 0; //magic 16092011
long renderer = 0;
...
//magic values below changed 30122010
unsigned long elementHeapSize = 8192000;//1024000;
unsigned long maxElements = 8192; //2048;
unsigned long maxGroups = 2048; //1024;

unsigned long systemHeapSize = 1048576000; //524288000; //8192000;
unsigned long guiHeapSize = 16384000;//1023999;
unsigned long logisticsHeapSize = 131071999;//4095999;
unsigned long missionHeapSize = 131072000;//3072000;
unsigned long spriteDataHeapSize = 16384000;//2048000;
unsigned long spriteHeapSize = 65536000;//8192000;
unsigned long polyHeapSize = 16384000;//1024000;
...
//long ObjectTextureSize = 256; //magic 11022011 was 128;
long ObjectTextureSize = 512; //magic 15022012
...
#define	MAX_KILL_AT_START	200 //magic 25072011 //nije problem
...
void InitializeGameEngine()
{
	...
	_set_new_mode(1); //magic 27032011
	...
	/*
		...
		//systemHeap->init(systemHeapSize,"SYSTEM"); //magic 30032011 **WARNING 200 MAP CRUSH ON EXIT**, supp vehicle null alloc NO
		systemHeap->init(systemHeapSize,"SYSTEM", true); //magic 17032011
...
//		systemHeap->startHeapMallocLog();
		...
		
		{
			...
			gosASSERT(prefsBlockResult == NO_ERR);
			{
				...
				*/
				...
				if (result != NO_ERR)
					//TERRAIN_TXM_SIZE = 64;
					TERRAIN_TXM_SIZE = 128; //magic 20022012
	
				result = prefsFile->readIdLong("ObjectTextureRes",ObjectTextureSize);
				if (result != NO_ERR)
					//ObjectTextureSize = 256; //magic 11022011 was 128;
					ObjectTextureSize = 512; //magic 15022012 was 128;
		...
		*/
		...
		//MidLevelRenderer::InitializeClasses(8192*4,8192,0,0,true); //magic 101 disabled
		MidLevelRenderer::InitializeClasses(16384*4,16384,0,0,true);
		//MidLevelRenderer::InitializeClasses(32768*4,32768,0,0,true);//magic 06022011
		gosFX::InitializeClasses();
		
		gos_PushCurrentHeap(MidLevelRenderer::Heap);
	
		//MidLevelRenderer::TGAFilePool *pool = new MidLevelRenderer::TGAFilePool("data\\tgl\\128\\");
		//MidLevelRenderer::TGAFilePool *pool = new MidLevelRenderer::TGAFilePool("data\\tgl\\256\\"); //magic 31012012
		MidLevelRenderer::TGAFilePool *pool = new MidLevelRenderer::TGAFilePool("data\\tgl\\512\\"); //magic 15022012
		...
		MemoryPtr effectsData = (MemoryPtr)gos_Malloc(effectsSize); //magic 02102011
		...
		//guiHeap->init(guiHeapSize,"GUI"); //magic 30032011 returned 200 OK, supp vehicle null alloc NO
		guiHeap->init(guiHeapSize,"GUI",true); //magic 17032011
...
//					logistics->setLogisticsState(log_DONE); // can't do this until screen animates out
	...
	else
	{
		...
		else
		{
			...
			{
				gos_SetScreenMode(1280,1024,16,0,0,0,0,false,0,true,0,0);//magic 16092011 
			}
			else
			//magic 16092011 end*/
				//gos_SetScreenMode(800,600,16,0,0,0,0,false,0,true,0,0);
				gos_SetScreenMode(Environment.screenWidth,Environment.screenHeight,16,0,0,0,0,false,0,true,0,0); //magic 16092011
...
void GetGameOSEnvironment( char* CommandLine )
{
	...
	Environment.applicationName			= "MechCommanderOmnitech\\1.0"; // MP012001";
	...
	//Environment.screenWidth				= 1280; //magic 16092011
	//Environment.screenHeight			= 1024; //magic 16092011
//magic 16092011 begin
	int logisticResolution = 0;
	DWORD desktopRes;
	...
	float resFactor = (float)XdeskRes/(float)YdeskRes;
		...
		else
		{
			...
			else
			{
				...
				{
				//magic 02102012 end
				Environment.screenWidth	= 1440;
				Environment.screenHeight = 900;
				//magic 02102012 begin
				}
				else
				{
