...
#include "Prefs.h"
#include "LogisticsDialog.h"//magic 09052012
...
/*	//magic 16092011 begin
	...
	if ( Environment.screenWidth == 1280 )
		strcat( path, "mcl_gn_deploymentteams_1280x1024.fit" );
	if ( Environment.screenWidth == 1440 )
		strcat( path, "mcl_gn_deploymentteams_1440x900.fit" );
	//magic 16092011 end*/
		...
		!iter.IsDone(); iter++ )
		{
			...
			if ( !(*iter)->getForceGroup() )
			{
				...
				for ( int i = 0; i < mechListBox.GetItemCount(); i++ )
				{
					...
					if ( ((MechListBoxItem*)mechListBox.GetItem(i))->getMech()->getID() == (*iter)->getID() ) //magic 21082012
					{
						mechListBox.incMechCount(((MechListBoxItem*)mechListBox.GetItem(i))->getMech());//M16
						bFound = true;
					}
				}
...
//	int newPurSel, newInvSel;
	...
	if ( rects[6].pointInside( mouseX, mouseY ) )
	{
		purFirst = true;
	}
	else
...
//M17 begin
	if (currentDropWeight >= maxDropWeight) 
		getButton( MB_MSG_DROPWEIGHT )->disable( true );
	else
		getButton( MB_MSG_DROPWEIGHT )->disable( false );

	if (LogisticsData::instance->getCBills() < 10000) 
		getButton( MB_MSG_ADDWEIGHT )->disable( true );
	else
		getButton( MB_MSG_ADDWEIGHT )->disable( false );
...
//M17 end
	...
	//magic 02062012 begin
	char tmpStr2[128];
	sprintf( tmpStr2, "%ld ", LogisticsData::instance->getCurrentRP() );
	textObjects[18].setText( tmpStr2 );

	long curResPoints = LogisticsData::instance->getCurrentRP();

	if (curResPoints < 2000) 
		getButton( MB_MSG_DROPRP )->disable( true );
	else
		getButton( MB_MSG_DROPRP )->disable( false );

	if (LogisticsData::instance->getCBills() < 2000) 
		getButton( MB_MSG_ADDRP )->disable( true );
	else
		getButton( MB_MSG_ADDRP )->disable( false );
	...
	if ( pDragMech )
	{
		...
		if ( userInput->leftMouseReleased() || userInput->isLeftDoubleClick() )
		{
			if ( dragLeft )
			{
				...
				{
					//if ( 0 == LogisticsData::instance->getVariantsInInventory( pDragMech->getVariant(), 0 ) )//M16 disabled
					//if ( 0 == mechListBox.getMechCount(pDragMech) )
					if ( mechListBox.getMechCount(pDragMech) > 0)
					{
						mechListBox.incMechCount(pDragMech);
					}
					else
					{ 
			...
			else
			{
				...
				else
				{
					...
					if ( 0 == mechListBox.getMechCount(pDragMech) )//M16
					{
						//MechListBoxItem* item = new MechListBoxItem( pDragMech, 0 );
						MechListBoxItem* item = new MechListBoxItem( pDragMech, 1 );
						int index = mechListBox.AddItem( item );
						mechListBox.SelectItem( index );
					}
					else
	...
	*/ //magic 20082012 disabled drag
	...
	for ( int i = 0; i < ICON_COUNT; i++ )
	{
		...
		//if ( bInside && userInput->isRightClick() && pIcons[i].getMech() ) //magic 20082012 OK-not for now
		{
			
			/*if (!pIcons[i].isSelected())
				pIcons[i].select(true);
				*/ //magic 20082012 OK in combination with the right click - too fast
			//Magic 73 begin
	...
	if ( newSel != -1 )
	{
		...
		mechListBox.SelectItem( -1 );//deselect all in mechlistbox
	...
	{
		//getButton( MB_MSG_CHANGE_LOADOUT )->disable( true ); //magic 12052012 should be visible when no purchase
	...
	if ( userInput->isLeftDoubleClick() && (pTmp) && purFirst)
	{
		handleMessage( 111, MB_MSG_ADD);
	}
	/*if ( userInput->isLeftDoubleClick() && (forceGroupMech))
	{
		handleMessage( 111, MB_MSG_REMOVE);
	}*/
	//Magic 72 end
	...
	if ( pCurMech  )
	{
	bool disableIntRepair = pCurMech->getInternalDamage() <= 0; 
	bool disableArmRepair = pCurMech->getArmorDamage() <= 0;  
	...
	//magic 12052012 end //works OK

}

void MechBayScreen::update3()
{
	int oldAvailSel, oldForceSel = -1;
	int newAvailSel, newForceSel = -1;
	bool forceFirst, availFirst;
	LogisticsMech* cSelectedMech;
...
//M17 end
	...
	//oldAvailSel = purcomponentListBox.GetSelectedItem(); // need to check this before we update the list box
	//Magic 73 begin instead of up
/*	for ( int i = 0; i < ICON_COUNT; i++ )
	{
		if ( pIcons[i].isSelected() )
			oldForceSel = i;
	...
	for ( int i = 0; i < ICON_COUNT; i++ )
	{
		...
		{
			//Magic 73 begin
			cSelectedMech = pIcons[i].getMech();
...
/*	if ( newForceSel != -1 )
	...
	//oldForceSel = invcomponentListBox.GetSelectedItem(); // M19
/*	if ( newForceSel != -1 )
	{
		if ( oldForceSel != -1 )
			pIcons[oldForceSel].select( 0 );
	...
	oldAvailSel = mechListBox.GetSelectedItem();
...
*/
	...
	mechListBox.update(); //M20
	newAvailSel = mechListBox.GetSelectedItem(); // M20
	//M22

//M22
	//M20

	if ( oldAvailSel != newAvailSel)
	{
		if ( newAvailSel != -1 )
		{
			cSelectedMech = mechListBox.getCurrentMech();
			getButton( MB_MSG_REMOVE )->disable( 1 );
			getButton( MB_MSG_ADD )->disable( 0 );
		}
		else
	...
	else if (availFirst && (newAvailSel != -1))
	{
		cSelectedMech = mechListBox.getCurrentMech();
		getButton( MB_MSG_REMOVE )->disable( 1 );
		getButton( MB_MSG_ADD )->disable( 0 );
	...
	//M20
	if ( oldAvailSel != newAvailSel && newAvailSel != -1)
	{
		unselectDeploymentTeam();
	}
		// component list box
...
/*	newForceSel = oldForceSel;
	...
	//newForceSel = invcomponentListBox.GetSelectedItem(); // M20
	//M20
	bool bInside = false;
	if ( oldForceSel != newForceSel)
	{
		if (newForceSel != -1 )
		{
			//bInside = pIcons[newForceSel].pointInside( userInput->getMouseX(), userInput->getMouseY() );
			//cSelectedMech = pIcons[newForceSel].getMech();
			getButton( MB_MSG_REMOVE )->disable( 0 );
			getButton( MB_MSG_ADD )->disable( 1 );
		}
		else
		{
			//bInside = pIcons[oldForceSel].pointInside( userInput->getMouseX(), userInput->getMouseY() );
			//cSelectedMech = pIcons[oldForceSel].getMech();
			getButton( MB_MSG_REMOVE )->disable( 1 );
			getButton( MB_MSG_ADD )->disable( 1 );
		}
	}
	else if (forceFirst && (newForceSel != -1))
	{
		//bInside = pIcons[newForceSel].pointInside( userInput->getMouseX(), userInput->getMouseY() );
		//cSelectedMech = pIcons[newForceSel].getMech();
		getButton( MB_MSG_REMOVE )->disable( 0 );
	...
	if ( !cSelectedMech )
	{
		cSelectedMech = getFGSelMech();
	}
	if (cSelectedMech)
	{
		//setMech( cSelectedMech, 0);
		pCurMech = cSelectedMech;
		mechCamera->update();
	}

	//Magic 64 begin
	if (( userInput->isLeftDoubleClick() && availFirst ) && (cSelectedMech))
	{
		handleMessage( 111, MB_MSG_ADD);
	}
	if (( userInput->isLeftDoubleClick() && forceFirst ) && (cSelectedMech))
	{
		handleMessage( 111, MB_MSG_REMOVE);
	}
	if ( !pCurMech ) 
...
int	MechBayScreen::handleMessage( unsigned long message, unsigned long who )
{
	...
	case MB_MSG_NEXT:
		//status = NEXT;
		status = PILOTSELECTION; //magic 20022012
		break;

	case MB_MSG_PREV:
		//status = PREVIOUS;
		status = BRIEFINGSCREEN; //magic 20022012
		break;
	...
	case MB_MSG_ADD:
		...
		//reLoadMechListBox(); //magic 19032011
		break;
	...
	case MB_MSG_BUY:
		//status = UP;
		status = MECHPURCHASE; //magic 20022012
		break;
	...
	case MB_MSG_ADDWEIGHT:
		LogisticsData::instance->incrementMaxDropWeight(5);
		break;

	case MB_MSG_DROPWEIGHT:
		LogisticsData::instance->decrementMaxDropWeight(5);
		break;

	//magic 02062012 begin
	case MB_MSG_ADDRP:
		LogisticsData::instance->addRP(2000);
		break;
	...
	/*case MB_MSG_REPAIR__ALL:
		{
			//if ((float)pCurMech->getInternalDamage() > 1.0)
				pCurMech->repairInternalPoints();
			pCurMech->repairArmorPoints(); 
		}
		break;*/
...
{
	LogisticsMech* zMech = mechListBox.getCurrentMech();
	...
	//reinitMechs(); //M17 no good
	//mechListBox.update(); //izbacuje iz programa kad je prazna lista
	reLoadMechListBox();
}*/
...
void MechBayScreen::addSelectedMech()
{
	LogisticsMech* zMech = mechListBox.getCurrentMech();
	//if ( mechListBox.getCurrentMech() )
	if ( zMech ) //magic 09052012-2
	{
		//if ( !LogisticsData::instance->canAddMechToForceGroup( mechListBox.getCurrentMech() ) )
		if ( !LogisticsData::instance->canAddMechToForceGroup( zMech ))  //magic 09052012-2
			return;
		//magic 09052012 begin
		//if (isInternalDamaged(mechListBox.getCurrentMech()))
		if (isInternalDamaged(zMech))  //magic 09052012-2
		{
				//LogisticsOneButtonDialog::instance()->setText( IDS_MC_INSUFFICIENT_CBILLS, IDS_DIALOG_OK,IDS_DIALOG_OK );
				LogisticsOneButtonDialog::instance()->setText( "This mech has damaged internal structure - can not be added to force group" );
		...
		//if ( NO_ERR != LogisticsData::instance->addMechToForceGroup( mechListBox.getCurrentMech(), forceGroupCount + 1 ) )
		if ( NO_ERR != LogisticsData::instance->addMechToForceGroup( zMech, forceGroupCount + 1 ) )  //magic 09052012-2
		{
			// its already in the force group, add probably called 2x'2
			return;
		}
...
void MechBayScreen::removeSelectedMech()
{
	for ( int i = 0; i < ICON_COUNT; i++ )
	{
		if ( pIcons[i].isSelected() )
		{
			...
			if ( !pMech )
				return;
			//if ( 0 == LogisticsData::instance->getVariantsInInventory( pMech->getVariant(), 0 ) )//M16
			/*
			if ( 0 == mechListBox.getMechCount(pMech) )//M16
			{
			*/ //magic 21082012 disabled
			...
			{
				mechListBox.incMechCount(pMech);
			}
			*/ //magic 21082012 disabled
			LogisticsData::instance->removeMechFromForceGroup( pMech, true );	
			//mechListBox.update(); //magic 03012011
	...
	reLoadMechListBox(); //magic 03012011
}
...
void MechBayScreen::setMech( LogisticsMech* pMech,  bool bCommandFromLB )
{
	...
	if ( pCurMech )
	{
		...
		//textObjects[11].setText( pCurMech->getChassisName() ); //magic 31102011 disabled
		//magic 31102011 begin
		//EString vNameTxt = pCurMech->getName();
		//int blankPos = vNameTxt.Find(" ");
		//textObjects[11].setText(vNameTxt.SubString(0,blankPos));
		textObjects[11].setText(pMech->getRealChassisName());
		//magic 31102011 end
...
{	
	...
	if ( mechListBox.pointInside( userInput->getMouseDragX(), userInput->getMouseDragY() ) )
	{
		...
		dragLeft = 0;
		//M16 begin
		if (mechListBox.getMechCount(pMech) < 2)
		{
			int index = mechListBox.GetSelectedItem();
			aListItem* pItem = mechListBox.GetItem( index );
			mechListBox.RemoveItem( pItem, true );
		}
		else
			mechListBox.decMechCount(pMech);
	...
	for ( EList< LogisticsMech*, LogisticsMech* >::EIterator iter = mechList.Begin();
		!iter.IsDone(); iter++ )
		{
			if ( (*iter)->getForceGroup() )
			{
				...
				//if ( FG <= maxUnits )
				if (( FG <= maxUnits ) && !isInternalDamaged((*iter))) //magic 09052012 do not add mechs with internal damedge to force group
				{
...
void MechBayScreen::reLoadMechListBox()
{
	mechListBox.removeAllItems(true);
		...
		!iter.IsDone(); iter++ )
	{
			...
			for ( int i = 0; i < mechListBox.GetItemCount(); i++ )
			{
				...
				{
					bFound = true;
					mechListBox.incMechCount(((MechListBoxItem*)mechListBox.GetItem(i))->getMech());
				}
			}
...
//magic 09052012 begin
bool	MechBayScreen::isInternalDamaged(LogisticsMech* pMech)
{
