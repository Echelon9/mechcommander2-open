									   ...
									   long nGates) 
{
	...
	//objList = new GameObjectPtr[(getMaxObjects() + 1)]; //magic 25032012
	...
	//watchList = new GameObjectPtr[ (getMaxObjects() + 1)]; //magic 25032012
	...
	if (maxMechs > 0) {
		...
		//mechs = new BattleMechPtr[ maxMechs]; //magic 25032012 //not showing appearances for all mech in roster
		//mechs = (BattleMechPtr*)gos_Malloc(sizeof(BattleMechPtr) * maxMechs);  //magic 25032012
		if (!mechs)
	...
	if (maxVehicles > 0) {
		...
		//vehicles = new GroundVehiclePtr[ maxVehicles]; //magic 25032012
		if (!vehicles)
	...
	if (numTerrainObjects > 0) {
		...
		//terrainObjects = new TerrainObjectPtr[ numTerrainObjects]; //magic 25032012
		if (!terrainObjects)
	...
	if (numBuildings > 0) {
		...
		//buildings = new BuildingPtr[ numBuildings]; //magic 25032012
		if (!buildings)
	...
	if (numTurrets > 0)
	{
		...
		//turrets = new TurretPtr[ numTurrets]; //magic 25032012
		if ( !turrets )
	...
	if (numGates > 0)
	{
		...
		//gates = new GatePtr[ numGates]; //magic 25032012
		if ( !gates )
	...
	if (numWeapons > 0) {
		...
		//weapons = new WeaponBoltPtr[ numWeapons]; //magic 25032012
		if (!weapons)
	...
	if (numCarnage > 0) {
		...
		//carnage = new CarnagePtr[ numCarnage]; //magic 25032012
		if (!carnage)
	...
	if (numArtillery > 0) {
		...
		//artillery = new ArtilleryPtr[ numArtillery]; //magic 25032012
		if (!artillery)
	...
	moverLineOfSightTable = (char*)gos_Malloc(maxMovers * maxMovers); //magic 02102011
...
//---------------------------------------------------------------------------
#define	MAX_TERRAIN_OBJECTS		10000 //magic 06032012 was 3000 
...
void GameObjectManager::countTerrainObjects (PacketFile* terrainFile, long firstHandle) 
{
	...
	if (totalObjCount)
	{
		...
		objData = new ObjDataLoader[totalObjCount]; //magic 02102011
		...
		if (obj) {
			...
			if (obj->isTerrainObject()) {
				...
				if (obj->getObjectClass() == BUILDING) {
					...
					long* curCoord = ((GatePtr)obj)->cellsCovered; //magic 07032012
...
void GameObjectManager::destroy (void) 
{
	...
	if (mechs && maxMechs > 0) 
	{
		...
		//delete [] mechs; //magic 25032012
		//gos_Free(mechs); //magic 25032012
	}
	mechs = NULL;
	...
	if (vehicles && maxVehicles > 0) 
	{
		...
		//delete [] vehicles; //magic 25032012
	}
	vehicles = NULL;
	...
	if (terrainObjects && numTerrainObjects > 0) 
	{
		...
		//delete [] terrainObjects; //magic 25032012
	}
	terrainObjects = NULL;
	...
	if (buildings && numBuildings > 0) 
	{
		...
		//delete [] buildings; //magic 25032012
	}
	buildings = NULL;
	...
	if (artillery && numArtillery > 0) 
	{
		...
		//delete [] artillery; //magic 25032012
	}
	artillery = NULL;
	...
	gos_Free(moverLineOfSightTable); //magic 02102011
...
long GameObjectManager::buildCollidableList (void) 
{
	...
	if (collidableList) 
	{
		...
		//delete [] collidableList; //magic 25032012
		collidableList = NULL;
	}
	...
	//collidableList = new GameObjectPtr[ numCollidables]; //magic 25032012
...
long GameObjectManager::Save (PacketFilePtr file, long packetNum)
{
	...
	file->writePacket(packetNum,(MemoryPtr)watchSave,sizeof(long) * (getMaxObjects() + 1),STORAGE_TYPE_RAW); //magic 11032012
	packetNum++;
...
long GameObjectManager::Load (PacketFilePtr file, long packetNum)
{
	...
	{
		mechs = (BattleMechPtr*)ObjectTypeManager::objectCache->Malloc(sizeof(BattleMechPtr) * maxMechs);
		//mechs = new BattleMechPtr[ maxMechs]; //magic 25032012
		//mechs = (BattleMechPtr*)gos_Malloc(sizeof(BattleMechPtr) * maxMechs);  //magic 25032012
		if (!mechs)
			Fatal(maxMechs, " GameObjectManager.setNumObjects: cannot malloc mechs ");
	}
	...
	for (i=0;i<=getMaxObjects();i++)
	{
		...
		//else if ((packetSize == sizeof(TerrainObjectData)) || (packetSize == sizeof(TerrainObjectDataOld))) //magic 09032012
		{
			//We have a TerrainObject.
			// Get its data.
			TerrainObjectData data;
			//TerrainObjectDataOld dataOld; //magic 09032012
			//if (packetSize == sizeof(TerrainObjectData)) //magic 09032012
				file->readPacket(packetNum,(MemoryPtr)(&data));
			/*//magic 09032012 begin
			else
			if (packetSize == sizeof(TerrainObjectDataOld)) //magic 09032012
				file->readPacket(packetNum,(MemoryPtr)(&dataOld));

			//OldToNew(data, dataOld);
			//magic 09022012 end*/
			packetNum++;
