...
bool GateType::handleCollision (GameObjectPtr collidee, GameObjectPtr collider)
{
	...
	switch (collider->getObjectClass())
	{
		...
		case ELEMENTAL:
		{
			...
			if ((colliderRange < closestRange) && (!collider->isDisabled() && !collider->isDestroyed()))
			{
...
void Gate::render (void)
{
	if (appearance->canBeSeen())
	{
		...
		if (((GateTypePtr)getObjectType())->gateTypeName != 0 )
		{
			char tmpString[255];
			cLoadString(((GateTypePtr)getObjectType())->gateTypeName, tmpString, 254);
			appearance->setObjectName(tmpString);
		}
		else
			appearance->setObjectName(((GateTypePtr)getObjectType())->buildingName);
		//magic 09092011 end
		windowsVisible = turn;
...
void Gate::Save (PacketFilePtr file, long packetNum)
{
	...
	file->writePacket(packetNum,(MemoryPtr)&data,sizeof(GateData),STORAGE_TYPE_RAW); //magic 11032012
}
...
void Gate::setName (const char *objName) //magic 09092011 new function
{
		strcpy_s(((GateTypePtr)getObjectType())->buildingName, objName);//magic 09092011
		((GateTypePtr)getObjectType())->gateTypeName = 0;
}
