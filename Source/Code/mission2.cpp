...
void Mission::initBareMinimum()
{
	...
	mcTextureManager->startVertices(3000000); //magic 29052012 - 26112011 ovde definise totalVertices 500000 previse - probati sa 500000
	...
	unsigned long spriteHeapSize = 8192000; //magic 03122011
...
{
	//---------------------------------------------------------
	//unsigned long tglHeapSize = 4 * 1024 * 1024; //magic disabled 03102011
	...
	//{ //magic 03102011 disabled
		...
		//TG_Shape::tglHeap->init(tglHeapSize,"TinyGeom"); //magic 03102011 disabled
		//TG_Shape::tglHeap->init(tglHeapSize,"TinyGeom",true); //magic 27032011
		
		//Start up the TGL RAM pools.
		colorPool 		= new TG_VertexPool;
		colorPool->init(40000); //magic 27112011 4000
		
		vertexPool 		= new TG_GOSVertexPool;
		vertexPool->init(160000); //magic 27112011 4000
		
		facePool 		= new TG_DWORDPool;
		facePool->init(320000); //magic 27112011 8000
		
		shadowPool 		= new TG_ShadowPool;
		shadowPool->init(160000); //magic 27112011 4000
		
		trianglePool 	= new TG_TrianglePool;
		trianglePool->init(160000); //magic 27112011 4000
	//} //magic 03102011 disabled
}
