...
void OptionsXScreen::init(FitIniFile* file)
{
	...
	// 1920: offset x = 448, y = 312 u mission
	// 1366: offset x = 283, y = 54
...
/*//Magic 74 begin (?)
	case 1920:
		xOffset = 560; //560 po racunu od gore ? 448
		yOffset = 210; //210 po racunu od gore ? 312
		break;
//Magic 74 end*/
...
ResModes resModes[20] = {
	...
	1440,900, 16,
	1440,900, 32,
	1366,768, 16, //magic 08022012
	1366,768, 32 //magic 08022012
};

bool availableMode[20] = {
...
void OptionsGraphics::init(long xOffset, long yOffset)
{
	...
	//for ( int i = IDS_RESOLUTION0; i < IDS_RESOLUTION9 + 1; i++ )//Magic 75 disabled
	for ( int i = IDS_RESOLUTION0; i < IDS_RESOLUTION9 + 11; i++ )//Magic 75 (+5) magic 08022011 +9
	{
		...
		else
		{
			...
			else if (i == (IDS_RESOLUTION9 + 1))
			{
				sprintf(cstr,"1920x1080x16");
				resolutionList.AddItem( cstr, 0xffffffff );
			}
			else if (i == (IDS_RESOLUTION9 + 2))
			{
				sprintf(cstr,"1920x1080x32");
				resolutionList.AddItem( cstr, 0xffffffff );
			}
			//Magic 76 end
			//magic 08022011 begin
			else if (i == (IDS_RESOLUTION9 + 3))
			{
				sprintf(cstr,"1920x1200x16");
				resolutionList.AddItem( cstr, 0xffffffff );
			}
			else if (i == (IDS_RESOLUTION9 + 4))
			{
				sprintf(cstr,"1920x1200x32");
				resolutionList.AddItem( cstr, 0xffffffff );
			}
			else if (i == (IDS_RESOLUTION9 + 5))
			{
				sprintf(cstr,"1680x1050x16");
				resolutionList.AddItem( cstr, 0xffffffff );
			}
			else if (i == (IDS_RESOLUTION9 + 6))
			{
				sprintf(cstr,"1680x1050x32");
				resolutionList.AddItem( cstr, 0xffffffff );
			}
			else if (i == (IDS_RESOLUTION9 + 7))
			{
				sprintf(cstr,"1440x900x16");
				resolutionList.AddItem( cstr, 0xffffffff );
			}
			else if (i == (IDS_RESOLUTION9 + 8))
			{
				sprintf(cstr,"1440x900x32");
				resolutionList.AddItem( cstr, 0xffffffff );
			}
			//magic 08022012 begin
			else if (i == (IDS_RESOLUTION9 + 9))
			{
				sprintf(cstr,"1366x768x16");
				resolutionList.AddItem( cstr, 0xffffffff );
			}
			else if (i == (IDS_RESOLUTION9 + 10))
			{
				sprintf(cstr,"1366x768x32");
				resolutionList.AddItem( cstr, 0xffffffff );
			}
			//magic 08022012 end
	//magic 08022011 end
	...
	/*//Magic 67 begin
	if (gos_GetMachineInformation( gos_Info_ValidMode, Environment.FullScreenDevice, 1920, 1080, 16))
		{
			char cstr[32];
			sprintf(cstr,"1920x1080x16");
			resolutionList.AddItem( cstr, 0xffffffff );
	...
	{
		availableMode[10] = false;
	}
	if (gos_GetMachineInformation( gos_Info_ValidMode, Environment.FullScreenDevice, 1920, 1080, 32))
		{
			char cstr[32];
			sprintf(cstr,"1920x1080x32");
			resolutionList.AddItem( cstr, 0xffffffff );
...
void OptionsGraphics::end()
{
	...
	if ( sel > -1 )
	{
		long actualSel = -1;
		//for ( int i = IDS_RESOLUTION0; i < IDS_RESOLUTION9 + 1; i++ )//Magic 75 disabled
		for ( int i = IDS_RESOLUTION0; i < IDS_RESOLUTION9 + 11; i++ )
		{
			...
			if (actualSel == sel)
			{
				switch (resModes[i-IDS_RESOLUTION0].xRes)
				{
					...
					//Magic 67 begin
					case 1920:
						if (resModes[i-IDS_RESOLUTION0].yRes >= 1200)
						{
							prefs.resolution = 6;
						}
						else
