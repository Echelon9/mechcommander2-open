...
int	MissionBegin::logisticResolution = 0;
...
void MissionBegin::begin()
{
	...
	/*//Magic16 begin load mechs and components from purchase file
	LogisticsData::instance->initPurVariants();
	//Magic16 end*/
...
//magic 16092011 begin
	//ovde ispitati desktop res i u zavisnosti od toga da li je wide ili ne ucitati 1280 ili 1440
	//a u zavisnosti od toga inicijalizovati i screenove dole - odgovarajucim fit-om
	logisticResolution = 0;
	DWORD desktopRes;
	...
	float resFactor = (float)XdeskRes/(float)YdeskRes;
		...
		{
			if (XdeskRes < 1280)
				logisticResolution = 0; //800x600
			else
				logisticResolution = 1; //1280x1024
		}
		else
		{
			if (XdeskRes <1440)
				...
				logisticResolution = 2; //1440x900
		}
		//u zavisnosti od logisticResolution dole otvarati odgovarajuce fit-ove
		//a umesto if ( Environment.screenWidth > 800 ) staviti switch (logisticResolution)
//magic 16092011 end
	//if ( Environment.screenWidth > 800 ) //magic 16092011
	//magic 16092011 begin
	switch (logisticResolution)
	{
				case 0:
				...
				case 1:
					...
					{
					if (prefs.renderer == 3)
						gos_SetScreenMode(1280,1024,16,0,0,0,true,localFullScreen,0,localWindow,0,localRenderer);
					else if (prefs.bitDepth)
						gos_SetScreenMode(1280,1024,32,prefs.renderer,0,0,0,localFullScreen,0,localWindow,0,localRenderer);
					else
						gos_SetScreenMode(1280,1024,16,prefs.renderer,0,0,0,localFullScreen,0,localWindow,0,localRenderer);
					}
					break;
				case 2:
					...
					{
					if (prefs.renderer == 3)
						gos_SetScreenMode(1440,900,16,0,0,0,true,localFullScreen,0,localWindow,0,localRenderer);
					else if (prefs.bitDepth)
						gos_SetScreenMode(1440,900,32,prefs.renderer,0,0,0,localFullScreen,0,localWindow,0,localRenderer);
					else
						gos_SetScreenMode(1440,900,16,prefs.renderer,0,0,0,localFullScreen,0,localWindow,0,localRenderer);
					}
					break;
				default: //magic 26092011
					if ( Environment.screenWidth != 800 )
					{
					if (prefs.renderer == 3)
						gos_SetScreenMode(800,600,16,0,0,0,true,localFullScreen,0,localWindow,0,localRenderer);
	...
	//Magic18 begin
		ComponentPurchaseScreen* pComponentPurchaseScreen;
	//Magic18 end
	...
	switch (logisticResolution)
	{
			case 0:			//800by600
				...
				break;
			case 1:			//1280by1024
				strcat( path, "mcl_mm_1280x1024.fit" );
				break;
			case 2:			//1440by900
				strcat( path, "mcl_mm_1440x900.fit" );
				break;
			default:
				strcat( path, "mcl_mm.fit" ); //magic 26092011
	}
	//magic 16092011 end
	FitIniFile file;
	...
	mainMenu->init( file );
	mainMenu->setDrawBackground( false ); //MAGIC changed from - true
	mainMenu->begin();
	file.close();
	...
	switch (logisticResolution)
	{
			case 0:			//800by600
				...
				break;
			case 1:			//1280by1024
				strcat( path, "MCL_CM_Layout_1280x1024.fit" );
				break;
			case 2:			//1440by900
				strcat( path, "mcl_cm_layout_1440x900.fit" );
				break;
			default:
				strcat( path, "mcl_cm_layout.fit" ); //magic 26092011
	}
	//magic 16092011 end
	if ( NO_ERR != file.open( path ) )
	{
	...
	switch (logisticResolution)
	{
			case 0:			//800by600
				...
				break;
			case 1:			//1280by1024
				strcat( path, "mcl_mn_1280x1024.fit" );
				break;
			case 2:			//1440by900
				strcat( path, "mcl_mn_1440x900.fit" );
				break;
			default:
				strcat( path, "mcl_mn.fit" ); //magic 26092011
	}
	//magic 16092011 end	
	if ( NO_ERR != file.open( path ) )
	{
	...
	switch (logisticResolution)
	{
			case 0:			//800by600
				...
				break;
			case 1:			//1280by1024
				strcat( path, "MCL_MB_Layout_1280x1024.fit" );
				break;
			case 2:			//1440by900
				strcat( path, "mcl_mb_layout_1440x900.fit" );
				break;
			default:
				strcat( path, "mcl_mb_layout.fit" ); //magic 26092011
	}
	//magic 16092011 end	
	...
	switch (logisticResolution)
	{
			case 0:			//800by600
				...
				break;
			case 1:			//1280by1024
				strcat( path, "mcl_pr_layout_1280x1024.fit" );
				break;
			case 2:			//1440by900
				strcat( path, "mcl_pr_layout_1440x900.fit" );
				break;
			default:
				strcat( path, "mcl_pr_layout.fit" ); //magic 26092011
	}
	//magic 16092011 end
	if ( NO_ERR != file.open( path ) )
	{
	...
	switch (logisticResolution)
	{
			case 0:			//800by600
				...
				break;
			case 1:			//1280by1024
				strcat( path, "mcl_m$_1280x1024.fit" );
				break;
			case 2:			//1440by900
				strcat( path, "mcl_m$_1440x900.fit" );
				break;
			default:  //magic 26092011
				strcat( path, "mcl_m$.fit" ); //magic 26092011
	...
	switch (logisticResolution)
	{
			case 0:			//800by600
				...
				break;
			case 1:			//1280by1024
				strcat( path, "mcl_mc_1280x1024.fit" );
				break;
			case 2:			//1440by900
				strcat( path, "mcl_mc_1280x1024.fit" );
				break;
			default:  //magic 26092011
				strcat( path, "mcl_mc.fit" ); //magic 26092011
	}
	//magic 16092011 end
	if ( NO_ERR != file.open( path ) )
	{
	...
	switch (logisticResolution)
	{
			case 0:			//800by600
				...
				break;
			case 1:			//1280by1024
				strcat( path, "mcl_loadingscreen_1280.fit" );
				break;
			case 2:			///1440by900
				strcat( path, "mcl_loadingscreen_1440.fit" );
				break;
			default:  //magic 26092011
				strcat( path, "mcl_loadingscreen.fit" ); //magic 26092011
	}
	//magic 16092011 end
	if ( NO_ERR != file.open( path ) )
	{
	...
	//M19 begin
	// initialize ComponentPurchaseScreen
	pComponentPurchaseScreen = new ComponentPurchaseScreen;

	strcpy( path, artPath );
	//strcat( path, "mcl_c$.fit" );
	//magic 16092011 begin
	switch (logisticResolution)
	{
			case 0:			//800by600
				...
				break;
			case 1:			//1280by1024
				strcat( path, "mcl_c$_1280x1024.fit" );
				break;
			case 2:			//1440by900
				strcat( path, "mcl_c$_1440x900.fit" );
				break;
			default:  //magic 26092011
				strcat( path, "mcl_c$.fit" ); //magic 26092011
	}
	//magic 16092011 end
	if ( NO_ERR != file.open( path ) )
	{
	...
	pComponentPurchaseScreen->init( file );
	file.close();
	...
	singlePlayerScreens[1][0] = pComponentPurchaseScreen;
	singlePlayerScreens[2][2] = pMechLabScreen;
	singlePlayerScreens[4][1] = pLoadScreen;

	//for ( int i = 0; i < 4; i++ )
	for ( int i = 0; i < 5; i++ ) //magic 29112011
	{
		//for ( int j = 0; j < 4; j++ )
		for ( int j = 0; j < 3; j++ ) //magic 29112011
		{
			if ( singlePlayerScreens[i][j] )
			{
...
long MissionBegin::getCurrentScreenId()
{
	...
	//singlePlayerScreens[1][0] = pComponentPurchaseScreen;		ID 10
		...
		if ( pCurScreen->getStatus() != LogisticsScreen::RUNNING )
		{
			...
			if ( pCurScreen->getStatus() == LogisticsScreen::NEXT )
			{
				...
				if ( curScreenX < 4 )
				{
					...
					//leftAnim.begin(); //magic 21012012 disabled
					curScreenX++;
			...
			else if ( pCurScreen->getStatus() == LogisticsScreen::PREVIOUS )
			{
				...
				//rightAnim.begin(); //magic 21012012 disabled
				curScreenX--;
				soundSystem->playDigitalSample( LOG_NEXTBACKBUTTONS );
				animJustBegun = true;
			}
			else if ( pCurScreen->getStatus() == LogisticsScreen::DOWN )
			{
				...
				//upAnim.begin(); //magic 21012012 disabled
				curScreenY++;
				soundSystem->playDigitalSample( LOG_NEXTBACKBUTTONS );
				animJustBegun = true;
			}
			else if ( pCurScreen->getStatus() == LogisticsScreen::UP )
			{
				...
				if (screens[curScreenX][curScreenY-1]) 
					screens[curScreenX][curScreenY-1]->begin();
				//downAnim.begin(); //magic 21012012 disabled
				curScreenY--;
				soundSystem->playDigitalSample( LOG_NEXTBACKBUTTONS );
				animJustBegun = true;
			}
			...
			else if ( pCurScreen->getStatus() == LogisticsScreen::COMPONENTPURCHASE )
			{
				pCurScreen->end();
				if (screens[1][0] ) 
					screens[1][0]->begin();
			...
			else if ( pCurScreen->getStatus() == LogisticsScreen::MECHBAY )
			{
				pCurScreen->end();
				if (screens[2][1] ) 
					screens[2][1]->begin();
			...
			else if ( pCurScreen->getStatus() == LogisticsScreen::PILOTSELECTION )
			{
				pCurScreen->end();
				if (screens[3][1] ) 
					screens[3][1]->begin();
			...
			else if ( pCurScreen->getStatus() == LogisticsScreen::MISSIONSELECTION )
			{
				pCurScreen->end();
				if (screens[0][1] ) 
					screens[0][1]->begin();
			...
			else if ( pCurScreen->getStatus() == LogisticsScreen::BRIEFINGSCREEN )
			{
				pCurScreen->end();
				if (screens[1][1] ) 
					screens[1][1]->begin();
			...
			else if ( pCurScreen->getStatus() == LogisticsScreen::MECHPURCHASE )
			{
				pCurScreen->end();
				if (screens[2][0] ) 
					screens[2][0]->begin();
			...
			else if ( pCurScreen->getStatus() == LogisticsScreen::MECHLAB )
			{
				pCurScreen->end();
				if (screens[2][2] ) 
					screens[2][2]->begin();
			...
			else if ( pCurScreen->getStatus() == LogisticsScreen::LOADSCREEN )
			{
				pCurScreen->end();
				if (screens[4][1] ) 
					screens[4][1]->begin();
	...
	if ( pCurScreen /*&& pCurScreen->getStatus() == LogisticsScreen::RUNNING*/ )
	{
		if ( leftAnim.isAnimating() && !leftAnim.isDone() )
		{
			...
			case 0:			//800by600
				...
				break;
			case 1:			//1280by1024
				xOffset = leftAnim.getXDelta() + 1280;
				break;
			case 2:			//1440by900
				xOffset = leftAnim.getXDelta() + 1440;
				break;
			default:
				xOffset = leftAnim.getXDelta() + 800;
			}
			//magic 16092011 end
			//xOffset = leftAnim.getXDelta() + 800;			
			//xOffset = leftAnim.getXDelta() + 1280; //magic 16092011
			yOffset = leftAnim.getYDelta();
			//xOtherOffset = xOffset - 800;
			//xOtherOffset = xOffset - 1280; //magic 16092011
			//magic 16092011 begin
			switch (logisticResolution)
			{
			case 0:			//800by600
				...
				break;
			case 1:			//1280by1024
				xOtherOffset = xOffset - 1280;
				break;
			case 2:			//1440by900
				xOtherOffset = xOffset - 1440;
				break;
			default :
				xOtherOffset = xOffset - 800;
			}
		...
		else if ( downAnim.isAnimating() && !downAnim.isDone() )
		{
			...
			//yOtherOffset = yOffset + 1024; //magic 16092011
			//magic 16092011 begin
			switch (logisticResolution)
			{
			case 0:			//800by600
				...
				break;
			case 1:			//1280by1024
				yOtherOffset = yOffset + 1024;
				break;
			case 2:			//1440by900
				yOtherOffset = yOffset + 900;
				break;
			default:
				yOtherOffset = yOffset + 600;
			}
		...
		else if ( rightAnim.isAnimating() && !rightAnim.isDone() )
		{
			...
			//xOffset = rightAnim.getXDelta() - 1280; //magic 16092011
			//magic 16092011 begin
			switch (logisticResolution)
			{
			case 0:			//800by600
				...
				break;
			case 1:			//1280by1024
				xOffset = rightAnim.getXDelta() - 1280;
				break;
			case 2:			//1440by900
				xOffset = rightAnim.getXDelta() - 1440;
				break;
			default:
				xOffset = rightAnim.getXDelta() - 800;
			}
...
void MissionBegin::beginSplash( const char* playerName)
{
	...
	if ( mainMenu )
	{
		...
		mainMenu->setDrawBackground( false ); //magic 16092011
		mainMenu->begin();
...
void MissionBegin::setToComponentPurchasing()
{
	bReadyToLoad = 0;
