...
void Mission::save (const char *saveFileName)
{
	...
	//saveFile.reserve(30000); //magic 05082011 no good
	...
	FullPathFileName MySaveName;
	MySaveName.init(savePath,tmpString,".fit");

	FitIniFile MytempFile;
	result = MytempFile.create(MySaveName);
	...
	{ //magic 04082011
		land->save( &tempFile );
		land->save( &MytempFile ); //M
	} //magic 04082011
	loadProgress = 25.0f;

	tempFile.writeBlock( "MissionSettings" );
MytempFile.writeBlock( "MissionSettings" );
	tempFile.writeIdString( "MissionName"						, missionFileName);
MytempFile.writeIdString( "MissionName"						, missionFileName);
	tempFile.writeIdFloat( "TimeLimit"							, m_timeLimit );
MytempFile.writeIdFloat( "TimeLimit"							, m_timeLimit );
	tempFile.writeIdLong( "ResourcePoints"						, LogisticsData::instance->getResourcePoints() );
MytempFile.writeIdLong( "ResourcePoints"						, LogisticsData::instance->getResourcePoints() );
	tempFile.writeIdUChar( "scenarioTuneNum"					, missionTuneNum );
MytempFile.writeIdUChar( "scenarioTuneNum"					, missionTuneNum );
	tempFile.writeIdLong("OperationId"							, operationId);
MytempFile.writeIdLong("OperationId"							, operationId);
	tempFile.writeIdLong("MissionId"							, missionId);
MytempFile.writeIdLong("MissionId"							, missionId);
	tempFile.writeIdLong("ABLScriptHandle"						, missionBrain->getRealId());
MytempFile.writeIdLong("ABLScriptHandle"						, missionBrain->getRealId());
	tempFile.writeIdBoolean("Active"							, active);
MytempFile.writeIdBoolean("Active"							, active);
	tempFile.writeIdFloat("ActualTime"							, actualTime);
MytempFile.writeIdFloat("ActualTime"							, actualTime);
	tempFile.writeIdFloat("RunningTime"							, runningTime);
MytempFile.writeIdFloat("RunningTime"							, runningTime);
	tempFile.writeIdFloat("ScenarioTime"						, scenarioTime);
MytempFile.writeIdFloat("ScenarioTime"						, scenarioTime);
	tempFile.writeIdFloat("DropZoneX"							, dropZone.x);
MytempFile.writeIdFloat("DropZoneX"							, dropZone.x);
	tempFile.writeIdFloat("DropZoneY"							, dropZone.y);
MytempFile.writeIdFloat("DropZoneY"							, dropZone.y);
	tempFile.writeIdFloat("DropZoneZ"							, dropZone.z);
MytempFile.writeIdFloat("DropZoneZ"							, dropZone.z);

//magic 04092012 begin
LogisticsData::instance->quickSaveSalvageComponents(tempFile);
LogisticsData::instance->quickSaveSalvageComponents(MytempFile);
//magic 04092012 end

	tempFile.writeBlock("Parts");
MytempFile.writeBlock("Parts");
	tempFile.writeIdULong("NumParts",numParts);
MytempFile.writeIdULong("NumParts",numParts); //magic 04082011
	...
	missionInterface->Save( &MytempFile );
	...
	{ //magic 04082011
		...
		weather->save( &MytempFile );
	} //magic 04082011
	loadProgress = 43.0f;

	tempFile.writeBlock("TheSky");
	MytempFile.writeBlock("TheSky");
	tempFile.writeIdLong("SkyNumber"							, theSkyNumber);
	MytempFile.writeIdLong("SkyNumber"							, theSkyNumber);
	
	Team::home->objectives.Save( &tempFile );
	Team::home->objectives.Save( &MytempFile );

	loadProgress = 50.0f;

	tempFile.writeBlock( "Script" );
	MytempFile.writeBlock( "Script" );
	tempFile.writeIdString( "ScenarioScript"					, missionScriptName );
	MytempFile.writeIdString( "ScenarioScript"					, missionScriptName );
...
MytempFile.close();
	...
	saveFile.writePacket(currentPacket,tmpRAM,fileSz,STORAGE_TYPE_RAW); //magic 04082011
	currentPacket++;
...
//	currentPacket++;
	...
	//saveFile.writePacket(currentPacket,tmpRAM,fileSz,STORAGE_TYPE_ZLIB);
	saveFile.writePacket(currentPacket,tmpRAM,fileSz,STORAGE_TYPE_RAW);
	currentPacket++;
...
void Mission::load (const char *loadFileName)
{
	...
	char *campName = (char*)malloc(loadFile.getPacketSize()+1);
	//magic 15072011 end
	...
	*/
	...
	//delete [] campName;
	if (campName) //magic 15072011
		free(campName);//magic 15072011
	campName = NULL;
	...
	missionHeap->init(missionHeapSize,"MISSION", true); //magic 24022011 works - not a solution for 180 and 200 crush
...
//magic 04092012 begin
LogisticsData::instance->quickLoadSalvageComponents(missionFile);
//LogisticsData::instance->quickSaveSalvageComponents(MytempFile);
//magic 04092012 end
	...
	else
	{
		...
		if ((theSkyNumber < 1) || (theSkyNumber > 36)) //magic 04072011
			theSkyNumber = DEFAULT_SKY;
	}
	...
	ObjectManager->init("object2", 716799, 4096000);  //magic 05082011
	gosASSERT(result == NO_ERR);
	...
	land->getColorMapName(&missionFile); //magic 30092011
