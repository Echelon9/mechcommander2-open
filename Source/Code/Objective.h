...
enum action_species_type {
	...
	MAKE_NEW_COMPONENT_SALVAGE, //magic 03092012
	_REMOVE_STRUCTURE,
...
static const char *g_actionSpeciesStringArray[] = {
	...
	"MakeNewSalvageComponents", //magic 03092012
	"_RemoveStructure",
};
...
class CMakeNewSalvageComponents: public CObjectiveAction
{
private:
	EString m_purchaseFilePathname;
public:
	CMakeNewSalvageComponents(int alignment) : CObjectiveAction(alignment) {}
	action_species_type Species() { return MAKE_NEW_COMPONENT_SALVAGE; }
	bool Init() { return true; }
	bool Read( FitIniFile* missionFile );
	bool Save( FitIniFile* file );
	int Execute();
	...
	EString InstanceDescription();
	void CastAndCopy(const CObjectiveAction *pMaster) { (*this) = (*(dynamic_cast<const CMakeNewSalvageComponents *>(pMaster))); }
};
//magic 03092012 end
