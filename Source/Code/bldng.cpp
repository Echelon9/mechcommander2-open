...
//magic 01062011 begin
DWORD WallType[24] = 
{
	26,125,152,154,156,158,160,161,162,202,204,206,231,232,233,286,287,288,801,802,803,804,2534,2535
};
//magic 01062011 end
...
//magic 29072011 end
	unsigned long dmgLevel;
/*//magic 29072011 begin
	if (resourcePoints > 20)
		dmgLevel = resourcePoints / 20;
	else
	{
//magic 29072011 end */ //magic 08092012 disabled
		result = bldgFile.readIdULong("DmgLevel",dmgLevel);
		if (result != NO_ERR)
			return(result);
	//} //magic 29072011 //magic 08092012 disabled
...
*/
	...
	/* magic long objectNameID = buildingTypeName;
	...
	//for (long i=0;i<18;i++)
	for (long i=0;i<24;i++) //magic 01062011
	{
...
long BuildingType::init (FilePtr objFile, unsigned long fileSize, const char* builName) {
...
*/
	...
	//magic 18122010 begin
	char s[255];
	//result = bldgFile.readIdString("Name",s,254);
	result = bldgFile.readIdString("Name",s,254); //magic 19032011
	//buildingName = s; //magic 19032011
					...
					case 4:		//DO NOTHING!!  We are in position.
						{
							...
							else
							{
								...
								appearance->setGesture(appearance->getCurrentGestureId() + 4); //magic 23082011
							}
						}
						break;
					...
					case 0:
						if (!appearance->getInTransition())
						{
							...
							if (refitMech) {
								...
								//if (dist <= 128.0f)
								//magic 23082011 begin
								if ((dist <= 128.0f) && (refitMech->getStatus() == OBJECT_STATUS_SHUTDOWN))
								//magic 23082011 end
								{
...
long Building::setTeamId (long _teamId, bool setup) 
{
	...
	else
	{
		...
		//else if (_teamId > 2)					//magic 06072011 disabled
		//	teamId = 1;		//Not ally.  ENEMY!!	//magic 06072011 disabled
		else
...
void Building::render (void) {
	...
	if (appearance->canBeSeen())
	{
		...
		//cLoadString(appearance->getObjectNameId(), tmpString, 254);
		if (((BuildingTypePtr)getObjectType())->buildingTypeName != 0 )
		{
			char tmpString[255];
			cLoadString(((BuildingTypePtr)getObjectType())->buildingTypeName, tmpString, 254);
			appearance->setObjectName(tmpString);
		}
		else
			appearance->setObjectName(((BuildingTypePtr)getObjectType())->buildingName);
		//magic 09092011 end
...
void Building::setDamage (float newDamage) 
{
	...
	if (damage >= getDamageLevel())
	{
		...
		//appearance->setObjStatus(OBJECT_STATUS_DESTROYED); //magic 09112011 disabled
		//magic 09112011 begin
		BuildingTypePtr type = (BuildingTypePtr)getObjectType();
		appearance->setObjStatus(OBJECT_STATUS_DESTROYED, !type->marksImpassableWhenDestroyed); //magic 09112011
		//magic 09112011 end
	}
}
...
void Building::init (bool create, ObjectTypePtr objType) {
	...
	//refPoints = type->resourcePoints / 20.0f; //magic 02082011
...
void Building::Save (PacketFilePtr file, long packetNum)
{
	...
	file->writePacket(packetNum,(MemoryPtr)&data,sizeof(BuildingData),STORAGE_TYPE_RAW); //magic 11032012
}
...
{																	   
	...
	//data->refPoints = refPoints; //magic 02082011
...
void Building::Load (BuildingData *data)
{
	...
	//refPoints = data->refPoints; //magic 02082011
...
{
	float newDamageLevel = pointsToBurn + getDamage(); //magic 08072011
	...
	if (newDamageLevel >= getDamageLevel())
	{
		...
		if (getFlag(OBJECT_FLAG_MECHBAY))
		{
			setFlag(OBJECT_FLAG_MECHBAY, false);
		}
		setDamage(0);
		//magic 07072011 end
		return(false);
...
void Building::setName (const char *objName) //magic 09092011 new function
{
		strcpy_s(((BuildingTypePtr)getObjectType())->buildingName, objName);//magic 09092011
		((BuildingTypePtr)getObjectType())->buildingTypeName = 0;
}
...
/*bool Building::burnRefitPoints(float pointsToBurn) 
{
	...
	return true;
}*/ //magic 08072011 original function
